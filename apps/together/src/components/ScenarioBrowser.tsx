...
export function ScenarioBrowser({ worldId, gender = 'any', onboarding = false, limit, query: externalQuery, initialScenarioId }: { worldId: string; gender?: string; onboarding?: boolean; limit?: number; query?: string; initialScenarioId?:string }) {
  const {scenarios,loading:catalogLoading,error:catalogError,retry:retryCatalog}=useScenarioCatalog();
  const snapshot = useTogether(s => s.snapshot), scope = snapshot?.activeContinuity?.id;
  const [selected, setSelected] = useState<Scenario | null>(()=>scenarios.find(s=>s.id===initialScenarioId&&s.worldId===worldId)??null), [query, setQuery] = useState('');
  useEffect(()=>{if(initialScenarioId)setSelected(scenarios.find(s=>s.id===initialScenarioId&&s.worldId===worldId)??null);},[initialScenarioId,worldId,scenarios]);
  ...
  useFocusEffect(useCallback(() => {
    let live = true;
    setSessions([]); setSelected(initialScenarioId?scenarios.find(s=>s.id===initialScenarioId&&s.worldId===worldId)??null:null); setError(''); setLoadError(false); setLoading(!onboarding);
    if (!onboarding) void manageScenario<{ sessions: ScenarioSession[] }>({ action: 'list' })
      .then(r => { if (live) setSessions(r.sessions); })
      .catch(() => { if (live) setLoadError(true); })
      .finally(() => { if (live) setLoading(false); });
    return () => { live = false; };
  }, [scope, onboarding, reload,initialScenarioId,worldId,scenarios]));
  ...
  const actionLabel = existing?.status === 'completed' ? (selected?.chapterCount?'Read completed story':'Reopen scenario') : existing ? 'Continue scenario' : 'Start scenario';
  const start = async () => {
  ...
  return <View style={styles.browser}>
    {catalogLoading?<ActivityIndicator accessibilityLabel="Loading scenarios" color={colors.rose}/>:null}
    {catalogError?<Pressable accessibilityRole="button" onPress={retryCatalog} style={styles.retry}><Text style={styles.error}>Scenarios couldn’t load · Retry</Text></Pressable>:null}
    ...
    <View onLayout={e => setGridWidth(e.nativeEvent.layout.width)} style={styles.grid}>{[...choices].sort((a,b)=>Number(Boolean(sessions.find(s=>s.scenario_id===b.id&&s.status!=='completed')))-Number(Boolean(sessions.find(s=>s.scenario_id===a.id&&s.status!=='completed')))||Number(Boolean(b.chapterCount))-Number(Boolean(a.chapterCount))).slice(0, limit ?? choices.length).map(s => {
      ...
      return <Pressable key={s.id} accessibilityRole="button" accessibilityLabel={`Scenario: ${s.title}. ${s.leadName}. ${s.locationName}${session ? `. ${statusLabel(session)}` : locked ? '. Locked' : ''}`} onPress={() => { setSelected(s); setError(''); }} onHoverIn={() => setHoveredId(s.id)} onHoverOut={() => setHoveredId(null)} style={({ pressed }) => [styles.card, cardWidth ? { width: cardWidth } : styles.cardFallback, hoveredId === s.id && styles.cardHovered, pressed && styles.pressed]}>
        ...
        <View style={styles.copy}>
          <Text style={styles.themes} numberOfLines={1}>{s.chapterCount?`${s.chapterCount} chapters · ${s.themes[0]}`:s.themes.slice(0, 2).join(' · ')}</Text>
    ...
    <CreatorModal visible={Boolean(selected)} title={selected?.title ?? 'Scenario'} onClose={closePreview} footer={selected ? <>
      ...
      {selected ? <View style={styles.previewContent}>
        ...
        {selected.chapterCount?<Text style={styles.lead}>{selected.arcTitle} · {selected.chapterCount} chapters · Your choices are saved</Text>:null}
        ...
        {!available ? <View style={styles.lockNotice}><LockKeyhole size={18} color={colors.warm} /><Text style={styles.lockText}>This scenario requires access to its world and any story discovery shown for {selected.leadName}.</Text></View> : null}
      </View> : null}
    </CreatorModal>
  </View>;
}
