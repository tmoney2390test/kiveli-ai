...
export function VideoModelPicker({ routes, selectedRouteId, duration, resolution, sound, disabled, onSelect }: { routes: VideoRouteOption[]; selectedRouteId: string; duration: number; resolution: VideoResolution; sound: boolean; disabled?: boolean; onSelect: (route: VideoRouteOption) => void }) {
  ...
  return <View accessibilityRole="radiogroup" accessibilityLabel="Video quality" style={s.row}>{routes.map(route => {
    ...
    return <Pressable key={route.id} accessibilityRole="radio" accessibilityLabel={`${route.displayName}, ${quote.credits} credits`} aria-checked={selected} accessibilityState={{checked:selected,disabled}} disabled={disabled} onPress={() => onSelect(route)} style={[s.card,premium && s.premium,selected && s.active]}>
...
const s = StyleSheet.create({row:{width:'100%',flexDirection:'row',gap:10},card:{flex:1,minWidth:0,padding:12,borderRadius:12,borderWidth:1,borderColor:colors.border,backgroundColor:'rgba(255,255,255,.025)',gap:5},premium:{borderColor:VIDEO_ACCENT},active:{borderColor:VIDEO_ACCENT,backgroundColor:VIDEO_ACCENT_FILL},titleRow:{flexDirection:'row',justifyContent:'space-between',gap:4},title:{color:colors.text,fontSize:14,fontWeight:'600'},description:{color:colors.textSecondary,fontSize:12,lineHeight:17},price:{color:VIDEO_ACCENT,fontSize:12,fontWeight:'600'}});
