...
export function ProactiveSettings({value,onChange,entitled,disabled,name,scenarioActive,onUpgrade,defaultQuiet,accountQuiet,accountFrequency}:{value:ProactiveDraft;onChange:(value:ProactiveDraft)=>void;entitled:boolean;disabled:boolean;name:string;scenarioActive:boolean;onUpgrade:()=>void;defaultQuiet:QuietHours;accountQuiet:QuietHours;accountFrequency:ProactiveFrequency}) {
  ...
  const selectedFrequency=!entitled?'off':value.frequency==='default'?(accountFrequency==='default'?'off':accountFrequency):value.frequency;
  ...
  return <View style={styles.root}>
    ...
    <View accessibilityRole="radiogroup" accessibilityLabel="Proactive message frequency" style={styles.options}>
      {proactiveFrequencyOptions.filter(option=>option.value!=='default').map(option=><Pressable key={option.value} accessibilityRole="radio" aria-checked={selectedFrequency===option.value} accessibilityLabel={option.label} accessibilityState={{checked:(selectedFrequency===option.value),disabled:locked}} disabled={locked} onPress={()=>onChange({...value,frequency:option.value,frequencyDirty:true})} style={[styles.option,(selectedFrequency===option.value)&&styles.selected,locked&&styles.disabled]}><View style={styles.grow}><Text style={styles.label}>{option.label}</Text><Text style={styles.copy}>{option.detail}</Text></View>{(selectedFrequency===option.value)?<Check size={18} color={colors.violet}/>:null}</Pressable>)}
    </View>
