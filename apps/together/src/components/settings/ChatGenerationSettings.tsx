...
export function ChatGenerationSettings({mode,chatDynamism,reasoningPreference,tier,disabled=false,onChatDynamismChange,onReasoningPreferenceChange,onUpgrade}:Props){
  ...
  return <View style={styles.wrapper}>
    <SettingRow ref={dynamismRef} testID="chat-dynamism-setting" icon={<Sparkles size={18} color={colors.violet}/>} label="Chat Dynamism" value={chatDynamismLabel(chatDynamism)} helper={mode==='group'?"Controls how grounded or spontaneous the group’s replies feel.":'Controls how grounded or spontaneous the conversation feels.'} disabled={disabled} onPress={()=>setPicker('dynamism')}/>
    ...
    <ThemedSettingPicker visible={picker==='dynamism'} title="Chat Dynamism" description={mode==='group'?"Applies to the whole group while preserving each character’s individual voice.":'Applies to this conversation.'} choices={chatDynamismChoices} selected={chatDynamism} disabled={disabled} onSelect={onChatDynamismChange} onClose={()=>setPicker(null)} returnFocusRef={dynamismRef} testIDPrefix="chat-dynamism-option"/>
