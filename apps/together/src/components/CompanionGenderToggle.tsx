...
export function CompanionGenderToggle({ value, onChange }: { value: FeaturedGenderFilter; onChange: (value: FeaturedGenderFilter) => void }) {
  ...
  return <View accessibilityLabel="Companion gender filter" style={[styles.container, open && styles.containerOpen]}>
    <FrostedSurface intensity={82} style={styles.triggerShell}>
      <Pressable accessibilityRole="button" accessibilityLabel={`Showing ${selected.label.toLowerCase()} companions. Change filter.`} aria-expanded={open} accessibilityState={{ expanded: open }} onPress={() => setOpen((current) => !current)} style={({ pressed }) => [styles.trigger, pressed && styles.triggerPressed]}>
    ...
    <CreatorModal visible={open} title="Choose companion gender" onClose={()=>setOpen(false)}>
      {options.map((option) => {
...
const styles = StyleSheet.create({
  ...
  option: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 10, borderRadius: 12, borderWidth: 1, borderColor: 'transparent' },
