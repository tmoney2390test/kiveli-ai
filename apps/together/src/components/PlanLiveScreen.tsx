...
import { endPlanExperience, getPlanExperience, joinCommitment } from '../lib/commitments';
...
export function PlanLiveScreen({ planId }: { planId: string }) {
  ...
  const character = snapshot?.characters.find((item) => item.id === (experience?.plan.character_instance_id ?? snapshotPlan?.character_instance_id));
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const value = await getPlanExperience(planId, character.id);
      setExperience(value);
      setLoadedAt(Date.now());
      if (value.scene && !value.scene.ended_at) await resolveActions(value.scene, value.scene.conversation_id ?? value.plan.source_conversation_id);
      setError('');
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Together Now could not load.'); }
    finally { setLoading(false); }
  }, [character, planId, snapshot]);

  const resolveActions = useCallback(async (scene: SceneSession, conversationId?: string | null) => {
    const actionConversation = (conversationId ? snapshot?.conversations.find((item) => item.id === conversationId) : null) ?? conversation;
    if (!character || !actionConversation) return;
    try {
      const result = await manageInteraction<InteractionResponse>({ action: 'resolve', characterInstanceId: character.id, conversationId: actionConversation.id });
  ...
  if (!snapshot || loading || !experience || !character) return <View style={styles.center}><ActivityIndicator color={colors.rose}/><Text style={styles.muted}>Loading the shared experience…</Text></View>;
  ...
  const endPlan = async () => {
    ...
    try {
      ...
      setExperience(value); setLoadedAt(Date.now()); setShowEndConfirm(false); await refresh();
  ...
  return <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      {activeScene && experience.plan.ends_at ? <View style={styles.timeRow}><Clock3 size={14} color={colors.warm}/><Text style={styles.timeText}>Planned until {formatTime(experience.plan.ends_at, experience.plan.world_timezone)} · {endLabel}</Text></View> : null}
      {early ? <WaitingCard title={experience.companion.arrived ? `${companionName} is already here` : `${companionName} should be here around ${formatTime(experience.plan.starts_at, experience.plan.world_timezone)}`} body={experience.companion.arrived ? 'Go inside together when you’re ready.' : `${planLocation?.name ?? 'The planned place'} · ${experience.plan.title}`} action={experience.companion.arrived ? 'Go inside together' : 'Wait here'} onPress={experience.companion.arrived ? () => void load() : () => void load()} /> : null}
...
function formatCompletionMeta(experience: PlanExperience) {
