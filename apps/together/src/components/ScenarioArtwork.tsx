...
export function ScenarioArtwork({ scenario, priority = 'normal' }: { scenario: Scenario; priority?: 'normal' | 'high' }) {
  ...
  return <View style={styles.frame}>
    ...
    <Image source={scenarioAssets[scenario.id]??locationHeroAsset(scenario.worldSlug,scenario.locationSlug)} accessibilityLabel={`${scenario.title}, with ${scenario.leadName} at ${scenario.locationName}`} contentFit="cover" cachePolicy="memory-disk" priority={priority} recyclingKey={scenario.id} onLoad={() => setState('loaded')} onError={() => setState('error')} style={StyleSheet.absoluteFill} />
    {!scenarioAssets[scenario.id]?<View style={{position:'absolute',right:14,bottom:14}}><CharacterAvatar slug={scenario.characterSlug} name={scenario.leadName} size={68}/></View>:null}
  </View>;
}
