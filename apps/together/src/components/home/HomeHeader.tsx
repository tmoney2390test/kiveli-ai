...
export function HomeHeader({ status, personaName, onCredits, onProfile }: { status: SubscriptionStatus | null; personaName: string; onCredits: () => void; onProfile: () => void }) {
  ...
  return <View style={styles.header}>
    <Text accessibilityRole="header" style={styles.brand}>Kivelle<Text style={styles.brandAccent}>.AI</Text></Text>
...
const styles = StyleSheet.create({
  ...
  brand: { color: colors.text, fontFamily: typography.display, fontSize: 27, fontWeight: '700', letterSpacing: -.7 },
  brandAccent: { color: colors.rose },
