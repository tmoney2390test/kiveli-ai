...
export function HomeWorldSection({ wide, upcoming, relationship, hook, memory, upcomingSource, relationshipSource, onUpcoming, onRelationship, embedded=false, compact=false }: {
  wide: boolean;
  ...
  return <View style={styles.section}>
    <SectionTitle title={embedded?'Between you':'Your world'} compact={compact} />
    <Text style={styles.hook}>{hook}</Text>
    <View style={[styles.grid, !wide && styles.gridStack]}>
      <WorldImageCard source={upcomingSource} eyebrow={upcoming.eyebrow} title={upcoming.title} meta={upcoming.meta} icon={<Sparkles size={15} color={colors.warm} />} compact={compact} onPress={onUpcoming} />
      <WorldImageCard source={relationshipSource} eyebrow={relationship.eyebrow} title={relationship.title} meta={relationship.meta} icon={<Heart size={15} color="#FFB2CB" />} memory={memory} compact={compact} onPress={onRelationship} />
    </View>
  </View>;
}
...
function WorldImageCard({ source, eyebrow, title, meta, icon, memory, onPress, compact }: { source?: ImageSource | number; eyebrow: string; title: string; meta: string; icon: React.ReactNode; memory?: { eyebrow: string; text: string }; onPress: () => void; compact:boolean }) {
  return <Pressable accessibilityRole="button" accessibilityLabel={`${eyebrow}, ${title}`} onPress={onPress} style={({ pressed }) => [styles.card,compact&&styles.cardCompact, pressed && styles.pressed]}>
    ...
    <View style={styles.copy}><Text numberOfLines={3} style={[styles.title,compact&&styles.titleCompact]}>{title}</Text><Text numberOfLines={2} style={styles.meta}>{meta}</Text>{memory ? <View style={styles.memory}><Text style={styles.memoryLabel}>{memory.eyebrow}</Text><Text numberOfLines={2} style={styles.memoryText}>{memory.text}</Text></View> : null}</View>
  </Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  cardCompact:{minHeight:224,padding:15},
  ...
  titleCompact:{fontSize:23,lineHeight:26},
