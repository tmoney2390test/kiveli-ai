...
function TimelineEvent({ item, onPress }: { item: HomeTimelineItem; onPress: () => void }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`${item.time}, ${item.title}${item.detail ? `, ${item.detail}` : ''}`} onPress={onPress} style={({ pressed }) => [styles.row, item.current && styles.rowCurrent, pressed && styles.pressed]}>
    <View style={[styles.node, item.current && styles.nodeCurrent]}>{icon}</View>
    <View style={styles.body}><Text style={[styles.time, item.current && styles.timeCurrent]}>{item.current ? 'NOW' : item.time.toUpperCase()}</Text><Text numberOfLines={2} style={[styles.title, item.current && styles.titleCurrent]}>{item.title}</Text>{item.detail ? <Text numberOfLines={2} style={styles.detail}>{item.detail}</Text> : null}</View>
  </Pressable>;
}
