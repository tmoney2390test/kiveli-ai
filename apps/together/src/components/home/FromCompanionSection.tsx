...
function MediaCard({ item, onPress }: { item: CompanionMediaItem; onPress: () => void }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`${item.locked ? 'Locked media' : item.title}, ${item.subtitle}`} onPress={onPress} style={({ pressed }) => [styles.card, pressed && styles.cardPressed]}>
    ...
    <View style={styles.cardCopy}><Text style={styles.kicker}>{item.context ?? 'FROM HER'} Â· {date.toUpperCase()}</Text><Text numberOfLines={2} style={styles.title}>{item.title}</Text><Text numberOfLines={1} style={styles.subtitle}>{item.locked ? 'Something for you' : item.subtitle}</Text></View>
  </Pressable>;
}

export function SectionTitle({ title, action, onAction }: { title: string; action?: string; onAction?: () => void }) {
  return <View style={styles.sectionTitleRow}><Text accessibilityRole="header" style={styles.sectionTitle}>{title}</Text>{action && onAction ? <Pressable accessibilityRole="button" onPress={onAction} style={({ pressed }) => pressed && styles.actionPressed}><Text style={styles.action}>{action} â†’</Text></Pressable> : null}</View>;
}
