...
export function MediaRequestModal({visible,character,conversationId,onPhotoRequest,onShare,onVideoCreated,onBuyCredits,onClose}:Props){
  ...
  useEffect(()=>{
    ...
    void getDirectVideoGenerationOptions(character.id).then((value)=>{if(!active)return;setOptions(value);setRouteId((current)=>value.routes.some((route)=>route.id===current)?current:value.defaultRouteId??value.routes[0]?.id??'');}).catch((cause)=>{if(active)setError(cause instanceof Error?cause.message:'Video options could not be loaded.');}).finally(()=>{if(active)setLoading(false);});
    return()=>{active=false;};
      ...
      <KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':Platform.OS==='android'?'height':undefined} style={styles.frame}>
        <FrostedSurface intensity={82} style={styles.modal}>
          ...
          <View accessibilityRole="radiogroup" style={styles.modeToggle}>
            <Pressable accessibilityRole="radio" accessibilityState={{selected:mode==='photo'}} onPress={()=>setMode('photo')} style={[styles.modeOption,mode==='photo'&&styles.modeSelected]}><Camera size={16} color={mode==='photo'?'#fff':colors.muted}/><Text style={[styles.modeText,mode==='photo'&&styles.modeTextSelected]}>Photo</Text></Pressable>
            <Pressable accessibilityRole="radio" accessibilityState={{selected:mode==='video'}} onPress={()=>setMode('video')} style={[styles.modeOption,mode==='video'&&styles.modeSelected]}><Film size={16} color={mode==='video'?'#fff':colors.muted}/><Text style={[styles.modeText,mode==='video'&&styles.modeTextSelected]}>Video</Text></Pressable>
          </View>
          <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
            ...
            <Text style={styles.title}>See this moment</Text>
            {mode==='photo'?<PhotoComposer name={name} description={description} setDescription={setDescription} onRequest={onPhotoRequest} onSubmit={submitPhoto} onShare={onShare}/>:<>
              ...
              {options?.routes.length?<>
                ...
                <View accessibilityRole="radiogroup" style={styles.routeList}>{options.routes.map((item)=><Pressable key={item.id} accessibilityRole="radio" accessibilityState={{selected:item.id===routeId}} onPress={()=>setRouteId(item.id)} style={[styles.route,item.id===routeId&&styles.routeSelected]}><View style={styles.routeTop}><Text style={styles.routeName}>{item.displayName}</Text>{item.id===routeId?<Check size={15} color={colors.rose}/>:null}</View><Text style={styles.routeCopy}>{item.description}</Text></Pressable>)}</View>
                <Text style={styles.label}>DESCRIBE THE VIDEO</Text>
                <TextInput accessibilityLabel="Describe the video you want" value={description} onChangeText={setDescription} placeholder={`What should ${name} do in this video?`} placeholderTextColor={colors.dimmed} maxLength={400} multiline style={styles.videoInput}/>
                ...
                <View accessibilityRole="radiogroup" style={styles.choiceRow}>{route?.allowedDurations.map((duration)=><Choice key={duration} label={`${duration} sec`} selected={durationSeconds===duration} onPress={()=>setDurationSeconds(duration)}/>)}</View>
...
function PhotoComposer({name,description,setDescription,onRequest,onSubmit,onShare}:{name:string;description:string;setDescription:(value:string)=>void;onRequest:(request:string)=>void;onSubmit:()=>void;onShare:()=>void}){return <>
  <Text style={styles.copy}>Ask {name} for a photo grounded in where they are and what they’re doing right now.</Text>
  ...
  <Pressable accessibilityLabel="Share your own photo" onPress={onShare} style={styles.share}><ImagePlus size={15} color={colors.rose}/><Text style={styles.shareText}>Share your own photo</Text></Pressable>
</>}
