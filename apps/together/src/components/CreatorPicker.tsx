...
export function CreatorPicker({ label, title, value, options, onChange, custom = false }: { label: string; title?: string; value: string; options: Array<{ value: string; label: string; image?: ImageSource }>; onChange: (value: string) => void; custom?: boolean }) {
  ...
  return <View style={styles.field}>
    ...
    <Pressable accessibilityRole="button" accessibilityLabel={`${label}: ${selected?.label || value || 'Choose'}`} accessibilityState={{ expanded: open }} onPress={() => { setCustomValue(selected ? '' : value); setOpen(true); }} style={styles.trigger}>
    ...
    <CreatorModal visible={open} title={title || `Choose ${label.toLowerCase().replace(' *', '')}`} onClose={() => setOpen(false)}>
      <View style={visual ? styles.grid : styles.list}>{options.map((option) => <Pressable key={option.value} accessibilityRole="radio" accessibilityLabel={option.label} accessibilityState={{ checked: value === option.value }} onPress={() => choose(option.value)} style={[styles.option, visual && styles.world, value === option.value && styles.selected]}>
        {option.image ? <><Image source={option.image} style={StyleSheet.absoluteFill} contentFit="cover" /><View style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(0,0,0,.38)' }]} /></> : null}<Text style={[styles.value, visual && styles.worldName]}>{option.label}</Text>{value === option.value ? <Check size={20} color={visual ? '#fff' : colors.rose} /> : null}
      </Pressable>)}</View>
