...
import { Check, ChevronDown, ChevronLeft, ChevronRight, Edit3, MapPin, RefreshCw, X } from 'lucide-react-native';
...
import { companionPick, companionPickQuote, defaultPlanTimeFields, isVenueProgramTime, localPlanDateValue, parseCustomPlanTime, previewPlanTiming, recommendPlanOptions, type PlanDiscoveryIntent, type PlanOption, type PlanTimingChoice, type PlanTimingSelection } from '../lib/plans';
...
export function PlanSelection({ snapshot, character, scopedLocationId, currentLocationId, initialActivityKey, repeatPlanId, proposal, initialTimingChoice, mode='create', currentPlan, interests, busy, error, onPlan, onClose }: Props) {
  ...
  const scopedCandidate = snapshot.locations.find((item) => item.id === scopedLocationId && item.id !== currentLocationId);
  ...
  const planContext = {
    ...
    excludedLocationId: currentLocationId,
  ...
  const availability = availabilityCopy(character);
  ...
  const canConfirm=Boolean(timingChoice&&(timingChoice!=='custom'||selectedDateTime));
  ...
  return <View style={styles.surface} accessibilityViewIsModal={Platform.OS === 'web' ? undefined : true}>
    <View style={styles.header}>
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.subtitle}>{mode==='switch'?`Choose what replaces ${currentPlan?.title??'your current plan'}.`:choice ? 'When should you two go?' : scoped && !elsewhere ? `What sounds good at ${scoped.name}?` : 'Pick an idea, then we’ll find the right time.'}</Text>
      </View>
        ...
        <ScrollView ref={carousel} horizontal showsHorizontalScrollIndicator={false} snapToInterval={heroCardWidth + 12} decelerationRate="fast" contentContainerStyle={styles.carouselContent} onMomentumScrollEnd={(event) => { const index = Math.round(event.nativeEvent.contentOffset.x / (heroCardWidth + 12)); if (heroOptions[index]) selectHero(index); }}>
          {heroOptions.map((option, index) => { const isActive = index === activeHeroIndex; const optionLocation = snapshot.locations.find((item) => item.id === option.locationId); const optionWorld = optionLocation ? worldForLocation(snapshot, option.locationId) : selectedWorld; return <View key={option.id} style={[styles.hero, { width: heroCardWidth }, isActive && styles.heroActive]}>
            ...
            <View style={styles.heroContent}>
              <View style={styles.heroTop}><Text style={styles.eyebrow}>{activityLabel(option.activityKey)}</Text><Text style={styles.heroStatus}>{index === 0 ? `${companionName}'s pick` : `● ${isActive ? availability : `${companionName} should be free`}`}</Text></View>
