...
export function PlanSelection({ snapshot, character, scopedLocationId, currentLocationId, initialActivityKey, repeatPlanId, proposal, initialTimingChoice, mode='create', currentPlan, interests, companionLabel, pluralCompanions=false, participants, plannerWorldId, plannerConversationId, hideViewAllPlaces=false, busy, error, onPlan, onClose }: Props) {
  ...
  const scenarioPlanning=mode==='create'&&Boolean(character.scenario_state||participants?.some(participant=>participant.scenario_state));
  ...
  const timingPreview=timingChoice==='now'?(scenarioPlanning?'Plan for now · scenario continues':'Start now'):timingChoice==='in_one_hour'?`In 1 hour · ${formatPlanDate(previewPlanTiming('in_one_hour'),viewerTimezone)}`:selectedDateTime?formatPlanDate(new Date(selectedDateTime),viewerTimezone):'Choose a time above';
  ...
  return <View style={styles.surface} accessibilityViewIsModal={Platform.OS === 'web' ? undefined : true}>
    ...
    {chooserOpen || !choice ? <>
      ...
      <View style={styles.timingRow}>
        <TimingOption label="NOW" detail={scenarioPlanning?'Save for now; join when ready':'Start together immediately'} selected={timingChoice==='now'} disabled={busy} onPress={()=>selectTiming('now')}/>
