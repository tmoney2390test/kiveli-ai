...
export const PORT_VERVELLE_PHOTOGRAPHED_LOCATION_SLUGS=[
  ...
  'casa-del-mare','farmacia-vervelle','fiore-and-fig','forno-bellini','harbor-steps','la-sirena',
...
function seed(input:SeedInput):Location{
  ...
  return{
    ...
    canonical_lore:{summary:input.description,stableFacts:[`${input.name} is in ${district}.`]},
    sort_order:input.index*10,
    metadata:{tags:input.activities,district:districtNode?true:district,photoStatus:photographedLocationSlugs.has(input.slug)?'ready':'pending'},
  };
}
...
export const portVervelleLocations:Location[]=[
  ...
  seed({index:40,parent:6,district:'Capo Vervelle',name:'Hôtel Celeste',slug:'hotel-celeste',description:'A boutique cliffside hotel with a pool, gardens, visiting guests, hospitality work, and a restaurant locals still use.',category:'hotel',type:'residence',activities:['stay','pool','garden','dinner'],hours:{open:'00:00',close:'23:59'}}),
