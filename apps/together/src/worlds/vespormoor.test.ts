...
describe('Vespormoor prepared world shell',()=>{
  it('publishes the world as a location-rich preview while its resident roster is pending',()=>{
    ...
    expect(vespormoorWorld.metadata.contentStatus).toBe('location_catalog_preview');
    ...
    expect(vespormoorWorld.metadata.residentRosterStatus).toBe('pending');
  });
  ...
  it('packages six districts and all 45 authored sub-locations with prompt-ready context',()=>{
    ...
    expect(vespormoorLocations.every((location)=>location.metadata?.photoStatus==='world_fallback')).toBe(true);
  });

  it('packages the supplied artwork as the world and future location fallback',()=>{
    expect(vespormoorWorld.hero_asset_key).toBe('vespormoor-hero');
    expect(vespormoorWorld.metadata.photoStatus).toBe('hero_ready');
    expect(vespormoorWorld.metadata.locationPhotoStatus).toBe('world_fallback');
  });
