...
export function AuthProvider({children}:PropsWithChildren){
  ...
  const value=useMemo<AuthValue>(()=>({session,loading,socialAuth,
    ...
    signOut:async()=>{const{error}=await supabase.auth.signOut({scope:'local'});if(error)throw readableAuthError(error);},signOutOthers:async()=>{const{error}=await supabase.auth.signOut({scope:'others'});if(error)throw readableAuthError(error);},updateEmail:async(email)=>{const{error}=await supabase.auth.updateUser({email});if(error)throw readableAuthError(error);},updatePassword:async(password)=>{const{error}=await supabase.auth.updateUser({password});if(error)throw readableAuthError(error);},resendEmailVerification:async()=>{if(!session?.user.email)throw new Error('No email is available for this account.');const{error}=await supabase.auth.resend({type:'email_change',email:session.user.email,options:{emailRedirectTo:authRedirectUrl()}});if(error)throw readableAuthError(error);}}),[session,loading]);
