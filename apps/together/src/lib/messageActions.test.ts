...
describe('message actions', () => {
  ...
  it('only continues from the latest visible companion reply', () => {
    const first = message({ id: 'a' }), control = message({ id: 'b', role: 'user', provider_metadata: { uiHidden: true } }), latest = message({ id: 'c' }), newerUserMessage = message({ id: 'd', role: 'user' }), generatedPhoto = message({ id: 'e', content: '[Photo]', provider_metadata: { mediaOnly: true } });
    expect(canContinueMessage(first, [first, control, latest])).toBe(false);
    expect(canContinueMessage(latest, [first, control, latest, newerUserMessage, generatedPhoto])).toBe(true);
  });
});
