...
export type SubscriptionPlan={tier:SubscriptionTier;displayName:string;monthlyPriceUsd:number;annualPriceUsd:number|null;maxActiveConversations:number;dailyMessageLimit:number|null;includedCompanionPhotoDailyLimit:number;includedDatePhotoMonthlyLimit:number;intelligenceProfile:'core'|'deep'|'director';memoryRetrievalBudget:number;historyRetrievalBudget:number;maxLives:number;maxCustomCompanions:number;worldAccess:'free'|'all_standard';earlyWorldAccess:boolean;monthlyCreditGrant:number;subscriptionCreditRolloverCap:number;mediaQueue:'standard'|'priority'|'highest'};
...
export type CreditActivityEvent={contextAction?:boolean;contextStatus?:string;contextChargedCredits?:number;id:string;eventType:string;permanentDelta:number;subscriptionDelta:number;createdAt:string;adjustmentReason?:'tier_cap_reduced'|'tier_cap_restored'|'post_subscription_grace_expired'|'refund'|'dispute'|'chargeback'|null};
