...
function sceneActivity(scene:{activity_key?:string|null;state:Record<string,unknown>}){const explicit=typeof scene.state.activityLabel==='string'?scene.state.activityLabel.trim():'';if(explicit)return naturalizeCharacterActivity(explicit,{activityKey:scene.activity_key});const key=String(scene.state.currentActivityKey??scene.activity_key??'together');return key&&key!=='together'?naturalizeCharacterActivity(key,{activityKey:key}):'Spending time together';}
...
function humanizeActivity(value:unknown){if(typeof value!=='string')return undefined;return value&&value!=='together'?naturalizeCharacterActivity(value):'Spending time together';}
