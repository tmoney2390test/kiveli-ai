...
export function resolveGroupPlanAvailability(input:{participants:CharacterInstance[];start:Date;durationMinutes:number;schedules:ScheduleItem[];plans:SharedPlan[];dates:DateSession[];immediate?:boolean;excludePlanId?:string;replacingActivePlan?:boolean}):GroupPlanAvailability[]{
  ...
  return input.participants.map((participant)=>{
    ...
    const schedule=input.schedules.find((candidate)=>candidate.character_version_id===participant.character_version_id
      && scheduleRunsOnDate(candidate.metadata,input.start,Intl.DateTimeFormat().resolvedOptions().timeZone)&& candidate.day_of_week===input.start.getDay()&&candidate.availability==='busy'
