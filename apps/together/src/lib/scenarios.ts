...
export function scenarioAvailable(scenario:Scenario,snapshot:Snapshot):boolean{
 return snapshot.worlds.some(w=>w.id===scenario.worldId&&w.published)&&
  (snapshot.discoverableCharacters.some(c=>c.id===scenario.characterTemplateId)||snapshot.characters.some(c=>c.character_template_id===scenario.characterTemplateId));
}
