...
export class XaiRealtimeVoiceClient implements RealtimeVoiceClient {
  ...
  private readonly audio=new PlatformRealtimeAudioEngine();private socket:WebSocket|null=null;private intentionalClose=false;
  ...
  constructor(private readonly callbacks:RealtimeVoiceCallbacks){this.speakerControlAvailable=this.audio.speakerControlAvailable;}
  ...
  async connect(credentials:RealtimeVoiceCredentials):Promise<void>{
    await this.disconnect();this.intentionalClose=false;
    ...
    return await new Promise<void>((resolve,reject)=>{
      ...
      const fail=(error:Error,recoverable:boolean)=>{this.callbacks.onError(error,recoverable);if(!settled){settled=true;clearTimeout(timeout);reject(error);}};
      ...
      socket.onclose=()=>{clearTimeout(timeout);const current=this.socket===socket;if(current){this.socket=null;void this.audio.close();}if(current&&!this.intentionalClose){if(!settled){settled=true;reject(new Error('The realtime voice connection closed.'));}this.callbacks.onClosed();}};
    });
  }
  async disconnect(){this.intentionalClose=true;const socket=this.socket;this.socket=null;await shutdownRealtimeTransport({audio:this.audio,socket,activeOutputTurn:this.activeOutputTurn,onSilenced:()=>{this.callbacks.onSpeaking('assistant',false);this.callbacks.onSpeaking('user',false);}});}
