...
export const updateCreatorDraftSections = (input:{draftId:string;sections:Partial<Record<'identity'|'appearance'|'personality'|'communication'|'connection'|'life'|'routine',Record<string,unknown>>>;expectedRevision:number;currentStep?:CreatorStep;relationshipGoal?:'friendship'|'romance'|'either'}) => manageCreator<{draft:CreatorDraft;readiness?:{ready:boolean;missing:string[]}}>({action:'update_draft_sections',...input});
