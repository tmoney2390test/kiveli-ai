...
describe('photo sharing access',()=>{
  ...
  it('opens the subscription prompt without opening the picker for a non-subscriber',()=>{const openPicker=vi.fn(),openPaywall=vi.fn();expect(handlePhotoSharingTap(false,{openPicker,openPaywall})).toBe('paywall');expect(openPaywall).toHaveBeenCalledOnce();expect(openPicker).not.toHaveBeenCalled();});
});
