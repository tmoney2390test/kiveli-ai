...
type ContextBudgetInspector={ceilingTokens:number;estimatedTokens:number;utilization:number;sections:Array<{key:string;included:boolean;required:boolean;selectedVariant:string;estimatedTokens:number;relevance:number;freshness:number;recordIds:string[];droppedRecordIds:string[];reasonCodes:string[];decision:string}>};
type ContextInspector={memoryContext?:{callbackAllowance?:number;debug?:Array<{id:string;activation:number;mode:string;reasonCodes:string[]}>};emotionalResidue?:{tone?:string;intensity?:number}|null;userPatterns?:Array<{summary?:string;confidence?:number}>;recentEpisodes?:Array<{title?:string;significance?:number}>;debug?:{contextBudget?:ContextBudgetInspector}};
...
export default function Debug(){
  ...
  const[memoryInspector,setMemoryInspector]=useState<ContextInspector|null>(null);
  ...
  return <Screen>
    ...
    {memoryInspector?.debug?.contextBudget?<><SectionHeader title="Context budget"/><Data label="Prompt utilization" value={`${memoryInspector.debug.contextBudget.estimatedTokens.toLocaleString()} / ${memoryInspector.debug.contextBudget.ceilingTokens.toLocaleString()} estimated tokens · ${Math.round(memoryInspector.debug.contextBudget.utilization*100)}%`}/>{memoryInspector.debug.contextBudget.sections.map((section)=><View key={section.key} style={styles.media}><View style={{flex:1}}><Text style={styles.mediaTitle}>{section.key} · {section.included?section.selectedVariant:'dropped'} · {section.estimatedTokens} tokens</Text><Text style={styles.mediaMeta}>relevance {section.relevance.toFixed(2)} · freshness {section.freshness.toFixed(2)} · {section.decision}</Text><Text style={styles.mediaMeta}>{section.reasonCodes.join(', ')}{section.droppedRecordIds.length?` · dropped ${section.droppedRecordIds.length} record(s)`:''}</Text></View></View>)}</>:null}
