...
export default function Debug(){
  ...
  return <Screen>
    ...
    <Data label="Active scene" value={scene?`${scene.source} Â· ${scene.activity_key??'together'}`:'No active shared scene'}/>
    ...
    {interactionCandidates.map((candidate)=><View key={candidate.id} style={styles.media}><View style={{flex:1}}><Text style={styles.mediaTitle}>{candidate.interactionKey} Â· {candidate.score.toFixed(2)}</Text><Text style={styles.mediaMeta}>{candidate.label} Â· {candidate.family}</Text><Text style={styles.mediaMeta}>{candidate.reasonCodes.join(', ')}</Text></View></View>)}
    <SectionHeader title="Schedule inspector"/>
    {schedule.length?schedule.map((event)=><View key={event.id} style={styles.media}><View style={{flex:1}}><Text style={styles.mediaTitle}>{new Date(event.starts_at).toLocaleString()} Â· {event.title}</Text><Text style={styles.mediaMeta}>{snapshot?.locations.find((item)=>item.id===event.location_id)?.name??'Travel'} Â· {event.source} Â· {event.priority} Â· {event.visibility} Â· {event.interruptibility}</Text><Text style={styles.mediaMeta}>score={String(event.metadata?.score??'n/a')} Â· {Array.isArray(event.metadata?.reasons)?event.metadata.reasons.join(', '):String(event.metadata?.reason??'deterministic selection')}</Text></View></View>):<Data label="Schedule" value="Generate from Home or Resolve one hour"/>}
    ...
    {media.length?media.map((item)=><View key={item.id} style={styles.media}><View style={{flex:1}}><Text style={styles.mediaTitle}>{item.status.toUpperCase()} Â· {String(item.metadata?.source??'unknown')}</Text><Text style={styles.mediaMeta}>{mediaPlace(item.metadata)}</Text><Text style={styles.mediaMeta}>{String(item.metadata?.photoOpportunitySlug??'no opportunity')} Â· {String(item.metadata?.shotType??'unknown')} Â· {String(item.metadata?.resolvedContentLevel??item.content_level)}</Text></View>{item.status==='failed'?<Pressable onPress={()=>void retry(item.id)} style={styles.retry}><RefreshCw size={15} color={colors.rose}/></Pressable>:null}</View>):<Data label="Media" value="No media rows"/>}
...
function Data({label,value}:{label:string;value?:string}){return <View style={styles.data}><Text style={styles.label}>{label.replaceAll('_',' ')}</Text><Text style={styles.value}>{value??'â€”'}</Text></View>}
