...
export default function MediaViewer(){
  const {id,gallery,character:galleryCharacter,returnTo}=useLocalSearchParams<{id:string;gallery?:MediaCarouselMode;character?:string;returnTo?:string}>();
  const{snapshot,refresh,removeMedia,upsertMedia}=useTogether();
  const[animating,setAnimating]=useState(false),[removing,setRemoving]=useState(false),[editOpen,setEditOpen]=useState(false),[lightboxOpen,setLightboxOpen]=useState(false),[videoFullscreenOpen,setVideoFullscreenOpen]=useState(false),[videoOpen,setVideoOpen]=useState(false),[videoRequestId,setVideoRequestId]=useState(''),[videoOptions,setVideoOptions]=useState<VideoGenerationOptions|null>(null),[videoError,setVideoError]=useState<string|null>(null),[videoProgress,setVideoProgress]=useState<string|null>(null),[diagnostics,setDiagnostics]=useState<VideoDiagnostics|null>(null),[editing,setEditing]=useState(false),[instruction,setInstruction]=useState(''),[videoReady,setVideoReady]=useState(false),[videoRefreshing,setVideoRefreshing]=useState(false),[videoPlaybackError,setVideoPlaybackError]=useState<string|null>(null),[videoPlaybackUrl,setVideoPlaybackUrl]=useState<string|null>(null),[videoPosterUrl,setVideoPosterUrl]=useState<string|null>(null),[stageSize,setStageSize]=useState({width:0,height:0});
