...
export default function MediaViewer(){
  ...
  useEffect(()=>{
    ...
    const subscription=player.addListener('statusChange',({status})=>{
      ...
      if(status==='readyToPlay'){
        ...
        if(!videoFullscreenOpen){player.muted=true;player.play();}
      }
    });
    return()=>subscription.remove();
  },[player,videoFullscreenOpen]);
  ...
  const animate=async(input:{routeId:string;durationSeconds:number;resolution:VideoResolution;sound:boolean;prompt:string;requestId:string;expectedCredits:number})=>{if(animating)return;setAnimating(true);setVideoError(null);try{const result=await animateMedia(media.id,{model:input.routeId,sound:input.sound,resolution:input.resolution,duration:input.durationSeconds,expectedCredits:input.expectedCredits},input.prompt,input.requestId);upsertMedia(result.media);setVideoOpen(false);navigateMedia(result.media.id,'replace');}catch(error){setVideoError(error instanceof Error?error.message:'The video could not be started.');}finally{setAnimating(false);}};
  ...
  const openVideoFullscreen=()=>{if(videoRefreshing||!videoPlaybackUrl)return;setVideoFullscreenOpen(true);};
  ...
  return <View style={styles.screen}>
    ...
    {Platform.OS!=='web'&&media.media_type==='video'&&media.status==='ready'&&videoPlaybackUrl?<VideoLightbox visible={videoFullscreenOpen} uri={videoPlaybackUrl} posterUri={videoPosterUrl} aspectRatio={videoRatio} audioBehavior={videoAudioBehavior} sharedPlayer={player} onClose={()=>setVideoFullscreenOpen(false)}/>:null}
    {media.media_type==='image'&&media.signed_url?<VideoGenerationSheet visible={videoOpen} sourceMediaId={media.id} sourceUrl={media.signed_url} characterName={character?.together_character_templates.name} requestId={videoRequestId} initialOptions={videoOptions} submitting={animating} error={videoError} onCancel={()=>{if(!animating)setVideoOpen(false);}} onEvent={(event,routeId)=>{void trackVideoSelectorEvent(media.id,event,routeId).catch(()=>undefined);}} onConfirm={(value)=>void animate(value)}/>:null}
