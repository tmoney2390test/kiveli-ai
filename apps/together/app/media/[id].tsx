...
export default function MediaViewer(){
  ...
  const refreshVideoPlayback=useCallback(async():Promise<string|null>=>{
    ...
    try{
      ...
      const nextUrl=result.media.status==='ready'?privateMediaPlaybackUrl(result.media.signed_url,Platform.OS,supabaseUrl):null;
  ...
  const parentMedia=media.media_type==='video'&&media.parent_media_id?snapshot?.generatedMedia?.find((item)=>item.id===media.parent_media_id):undefined;
  const videoPosterUrl=privateMediaPlaybackUrl(parentMedia?.signed_url,Platform.OS,supabaseUrl);
