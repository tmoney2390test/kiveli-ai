...
import type { GeneratedMedia, VideoDiagnostics, VideoGenerationOptions, VideoMotionPreset,VideoResolution } from '../../src/types';
...
export default function MediaViewer(){
  ...
  const playbackRecorded=useRef(false),playbackRefreshRequest=useRef(0),routeMediaRequest=useRef(0);
  ...
  const animate=async(input:{routeId:string;motionPreset:VideoMotionPreset;durationSeconds:number;resolution:VideoResolution;sound:boolean;requestId:string})=>{if(animating)return;setAnimating(true);setVideoError(null);try{const result=await animateMedia(media.id,{model:input.routeId,sound:input.sound,resolution:input.resolution,duration:input.durationSeconds},input.motionPreset,input.requestId);upsertMedia(result.media);setVideoOpen(false);navigateMedia(result.media.id,'replace');}catch(error){setVideoError(error instanceof Error?error.message:'The video could not be started.');}finally{setAnimating(false);}};
...
function VideoDetails({media,progressState,diagnostics}:{media:GeneratedMedia;progressState:string|null;diagnostics:VideoDiagnostics|null}){
  const[feedbackOpen,setFeedbackOpen]=useState(false),[reasons,setReasons]=useState<string[]>([]),[saving,setSaving]=useState(false),routeName=diagnostics?.routeDisplayName??'Selected video model',motion=String(media.motion_preset??'subtle'),audio=media.actual_audio_behavior??diagnostics?.actualAudioBehavior??'unknown';
