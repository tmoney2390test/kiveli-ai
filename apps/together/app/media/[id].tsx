...
import { ErrorState, FrostedBackdrop, FrostedSurface, ImageLightbox, LoadingSkeleton, MediaFeedbackControls, VideoGenerationSheet, VideoLightbox } from '../../src/components';
...
export default function MediaViewer(){
  ...
  useEffect(()=>{playbackRecorded.current=false;if(media?.media_type!=='video'||media.status!=='ready')return;const subscription=player.addListener('playingChange',({isPlaying})=>{if(!isPlaying||playbackRecorded.current)return;playbackRecorded.current=true;void recordVideoPlayback(media.id).catch(()=>{playbackRecorded.current=false;});});return()=>subscription.remove();},[media?.id,media?.media_type,media?.status,player]);
