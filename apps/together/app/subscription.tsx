...
import { ArrowLeft, Brain, CalendarDays, Camera, Check, ChevronDown, ChevronRight, CircleAlert, CreditCard, ExternalLink, Gift, Globe2, Heart, History, LockKeyhole, RefreshCw, ShieldCheck, Sparkles, UserRound, Zap } from 'lucide-react-native';
...
import { annualSavingsPercentage, billingStatusPresentation, checkoutBackoffDelay, creditActivityPresentation, managementActionLabel, membershipBenefits, membershipMetrics, membershipPageMode, membershipPricePresentation, normalizeSubscriptionIntent, safeSubscriptionReturnTo, subscriptionIntentPresentation } from '../src/lib/subscriptionPresentation';
...
export default function Subscription() {
  ...
  return (
    <Screen scrollRef={scrollRef} contentStyle={styles.content}>
      ...
      {intent !== 'plans' ? <IntentCallout eyebrow={intro.eyebrow} title={intro.title} body={intro.body} /> : null}

      {mode === 'discovery' ? <>
        <DiscoveryHero compact={compact} onExplore={() => scrollTo(plansY)} onCompare={() => { setCompareOpen(true); scrollTo(compareY); }} />
        <DiscoveryWalletBand state={state} compact={compact} onActivity={() => scrollTo(creditsY)} />
...
function DiscoveryHero({ compact, onExplore, onCompare }: { compact: boolean; onExplore: () => void; onCompare: () => void }) {
  return <View style={[styles.heroCard, compact && styles.heroCardCompact]}><PortalArt compact={compact} /><View style={[styles.heroContent, compact && styles.heroContentCompact]}><StatusPill label="NO ACTIVE MEMBERSHIP" tone="neutral" /><Text accessibilityRole="header" style={[styles.heroTitle, compact && styles.heroTitleCompact]}>Your next world is waiting.</Text><Text style={styles.heroCopy}>Restore deeper memories, receive daily images, meet more companions, and get priority access across every world.</Text><View style={styles.heroActions}><Pressable accessibilityRole="button" onPress={onExplore} style={({ pressed }) => [styles.primaryButton, pressed && styles.pressed]}><Text style={styles.primaryButtonText}>Explore memberships</Text><ChevronRight size={19} color="#fff" /></Pressable><Pressable accessibilityRole="button" onPress={onCompare} style={({ pressed }) => [styles.secondaryButton, pressed && styles.pressed]}><Text style={styles.secondaryButtonText}>Compare benefits</Text><ChevronRight size={17} color={colors.rose} /></Pressable></View><View style={styles.safetyLine}><ShieldCheck size={18} color={colors.success} /><Text style={styles.safetyText}>Your permanent Credits, companions, and memories stay safe.</Text></View></View></View>;
}

function PortalArt({ compact = false }: { compact?: boolean }) {
  return <View pointerEvents="none" style={[styles.portalArt, compact && styles.portalArtCompact]}><View style={styles.portalGlow} /><View style={styles.portalRingOuter}><View style={styles.portalRingMiddle}><View style={styles.portalRingInner}><Heart size={72} strokeWidth={1.2} color="#FF62C7" fill="rgba(216,62,234,.13)" /></View></View></View><Sparkles size={24} color="#F4A36C" style={styles.portalSparkTop} /><Sparkles size={18} color="#D86EFA" style={styles.portalSparkBottom} /></View>;
}

function DiscoveryWalletBand({ state, compact, onActivity }: { state: SubscriptionStatus; compact: boolean; onActivity: () => void }) {
  return <View style={[styles.walletBand, compact && styles.stack]}><View style={styles.walletBandCell}><Text style={styles.statLabel}>AVAILABLE CREDITS</Text><View style={styles.bigBalance}><Text style={styles.bigBalanceText}>{state.creditBalance.total.toLocaleString()}</Text><KivelleCreditIcon size={38} /></View><Text style={styles.successNote}>Permanent Credits do not expire</Text></View><View style={[styles.walletBandDivider, compact && styles.walletBandDividerCompact]} /><View style={styles.walletBandCell}><Text style={styles.statLabel}>MONTHLY PLAN GRANT</Text><View style={styles.pausedRow}><View style={styles.pauseIcon}><Text style={styles.pauseText}>Ⅱ</Text></View><View><Text style={styles.pausedTitle}>Paused</Text><Text style={styles.statDetail}>Starts when you subscribe</Text></View></View></View><Pressable accessibilityRole="button" onPress={onActivity} style={({ pressed }) => [styles.activityLink, pressed && styles.pressed]}><Text style={styles.activityLinkText}>View credit activity</Text><ChevronRight size={17} color={colors.text} /></Pressable></View>;
}
...
function MemberHero({ state, plan, compact, busy, onManage, onBuyCredits }: { state: SubscriptionStatus; plan: SubscriptionPlan; compact: boolean; busy: boolean; onManage?: () => void; onBuyCredits?: () => void }) {
  ...
  const subscriptionShare = state.creditBalance.total > 0 ? Math.min(100, Math.max(0, state.creditBalance.subscriptionBalance / state.creditBalance.total * 100)) : 0;
  return <View style={[styles.memberHero, compact && styles.memberHeroCompact]}><PortalArt compact={compact} /><View style={styles.memberPlanColumn}><StatusPill label={status.label.toUpperCase()} tone={status.tone} /><Text accessibilityRole="header" style={styles.memberPlanName}>{plan.displayName}</Text><View style={styles.priceRow}><Text style={styles.memberPrice}>{price.primary}</Text><Text style={styles.memberPeriod}>{price.period}</Text></View><Text style={styles.memberBilling}>{state.management.mode === 'kivelle' ? status.detail : `${price.detail} · ${state.management.label}`}</Text>{status.date && status.dateLabel ? <Text style={styles.renewalLine}>{status.dateLabel} {formatDate(status.date)}</Text> : null}<View style={styles.memberActions}>{onBuyCredits ? <Pressable accessibilityRole="button" onPress={onBuyCredits} style={({ pressed }) => [styles.primaryButton, styles.memberActionButton, pressed && styles.pressed]}><KivelleCreditIcon size={19} /><Text style={styles.primaryButtonText}>Buy more Credits</Text></Pressable> : null}{onManage ? <Pressable accessibilityRole="button" disabled={busy} onPress={onManage} style={({ pressed }) => [styles.secondaryButton, styles.memberActionButton, busy && styles.disabled, pressed && styles.pressed]}><Text style={styles.secondaryButtonText}>{busy ? 'Opening billing…' : managementActionLabel(state.management)}</Text><ExternalLink size={16} color={colors.text} /></Pressable> : null}</View></View><View style={[styles.memberDivider, compact && styles.memberDividerCompact]} /><View style={[styles.memberCreditColumn, compact && styles.memberCreditColumnCompact]}><Text style={styles.statLabel}>AVAILABLE CREDITS</Text><View style={styles.memberBalanceRow}><Text style={styles.memberBalance}>{state.creditBalance.total.toLocaleString()}</Text><KivelleCreditIcon size={44} /></View><View style={styles.balanceBar}><View style={[styles.balanceBarSubscription, { width: `${subscriptionShare}%` }]} /></View><View style={styles.balanceLegend}><BalanceLegend color="#F05C9E" value={state.creditBalance.permanentBalance} label="permanent" /><BalanceLegend color="#9B63D7" value={state.creditBalance.subscriptionBalance} label="plan Credits" /></View><View style={styles.nextGrant}><CalendarDays size={18} color={colors.text} /><Text style={styles.nextGrantText}>{state.nextCreditGrantAt ? `Next monthly grant: ${plan.monthlyCreditGrant.toLocaleString()} Credits on ${formatDate(state.nextCreditGrantAt)}` : `${plan.monthlyCreditGrant.toLocaleString()} Credits included each month`}</Text></View></View></View>;
}

function BalanceLegend({ color, value, label }: { color: string; value: number; label: string }) { return <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: color }]} /><View><Text style={styles.legendValue}>{value.toLocaleString()}</Text><Text style={styles.legendLabel}>{label}</Text></View></View>; }
...
function BenefitsCard({ plan }: { plan: SubscriptionPlan }) {
  ...
  return <View style={[styles.dashboardCard, styles.benefitsCard]}><Text style={styles.dashboardTitle}>Everything included with {plan.displayName}</Text><View style={styles.benefitList}>{membershipBenefits(plan).map((benefit, index) => { const Icon = icons[index] ?? Check; return <View key={benefit} style={styles.benefitRow}><View style={styles.benefitIcon}><Icon size={19} color="#F2C8EA" /></View><Text style={styles.benefitText}>{benefit}</Text><Check size={19} color={colors.success} /></View>; })}</View></View>;
}
...
const styles = StyleSheet.create({
  ...
  heroCard: { position: 'relative', minHeight: 286, overflow: 'hidden', justifyContent: 'center', padding: 32, borderRadius: radius.xl, backgroundColor: '#160B19', borderWidth: 1, borderColor: 'rgba(235,93,184,.46)' }, heroCardCompact: { minHeight: 390, padding: 20 }, heroContent: { zIndex: 2, maxWidth: 610, gap: 14 }, heroContentCompact: { maxWidth: '100%' }, heroTitle: { color: '#FFF5EC', fontFamily: 'Georgia', fontSize: 41, lineHeight: 48 }, heroTitleCompact: { fontSize: 34, lineHeight: 40 }, heroCopy: { maxWidth: 560, color: '#D9CCD5', fontSize: 15, lineHeight: 23 }, heroActions: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginTop: 3 }, primaryButton: { minHeight: 50, minWidth: 190, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, paddingHorizontal: 20, borderRadius: radius.md, backgroundColor: '#C62DB9', shadowColor: colors.rose, shadowOpacity: .28, shadowRadius: 17, shadowOffset: { width: 0, height: 7 }, elevation: 4 }, primaryButtonText: { color: '#fff', fontSize: 14, fontWeight: '900' }, secondaryButton: { minHeight: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingHorizontal: 16, borderRadius: radius.md, backgroundColor: 'rgba(255,255,255,.025)', borderWidth: 1, borderColor: 'rgba(255,255,255,.22)' }, secondaryButtonText: { color: colors.text, fontSize: 13, fontWeight: '900' }, safetyLine: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingTop: 10, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: 'rgba(255,255,255,.12)' }, safetyText: { flex: 1, color: colors.muted, fontSize: 11, lineHeight: 17 },
  portalArt: { position: 'absolute', top: 0, right: 0, bottom: 0, width: '48%', minWidth: 330, alignItems: 'center', justifyContent: 'center', opacity: .96 }, portalArtCompact: { width: '100%', minWidth: 0, opacity: .25 }, portalGlow: { position: 'absolute', width: 290, height: 290, borderRadius: 145, backgroundColor: 'rgba(194,29,186,.16)', shadowColor: '#EE35D4', shadowOpacity: .7, shadowRadius: 52 }, portalRingOuter: { width: 222, height: 246, borderRadius: 111, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(244,99,206,.35)', transform: [{ rotate: '4deg' }] }, portalRingMiddle: { width: 180, height: 210, borderRadius: 90, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: 'rgba(219,76,230,.5)', transform: [{ rotate: '-8deg' }] }, portalRingInner: { width: 130, height: 166, borderRadius: 65, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: 'rgba(255,138,215,.72)', backgroundColor: 'rgba(52,11,54,.65)', transform: [{ rotate: '4deg' }] }, portalSparkTop: { position: 'absolute', top: 32, right: 42 }, portalSparkBottom: { position: 'absolute', bottom: 30, left: 56 },
  ...
  walletBand: { flexDirection: 'row', alignItems: 'stretch', gap: 18, padding: 18, borderRadius: radius.xl, backgroundColor: '#15101B', borderWidth: 1, borderColor: 'rgba(244,124,181,.29)' }, walletBandCell: { flex: 1, minWidth: 210, justifyContent: 'center', gap: 6 }, walletBandDivider: { width: 1, backgroundColor: colors.border }, walletBandDividerCompact: { width: '100%', height: 1 }, statLabel: { color: colors.dimmed, fontSize: 9, fontWeight: '900', letterSpacing: 1.05 }, bigBalance: { flexDirection: 'row', alignItems: 'center', gap: 10 }, bigBalanceText: { color: '#FFF5EA', fontFamily: 'Georgia', fontSize: 45, lineHeight: 50 }, successNote: { color: colors.success, fontSize: 10, fontWeight: '800' }, pausedRow: { flexDirection: 'row', alignItems: 'center', gap: 12 }, pauseIcon: { width: 46, height: 46, borderRadius: 23, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: colors.violet }, pauseText: { color: colors.violet, fontSize: 20, fontWeight: '900' }, pausedTitle: { color: colors.text, fontFamily: 'Georgia', fontSize: 22 }, statDetail: { color: colors.muted, fontSize: 10, marginTop: 2 }, activityLink: { minHeight: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, paddingHorizontal: 12 }, activityLinkText: { color: colors.text, fontSize: 11, fontWeight: '800', textDecorationLine: 'underline' },
  ...
  memberHero: { position: 'relative', overflow: 'hidden', flexDirection: 'row', gap: 24, padding: 30, borderRadius: radius.xl, backgroundColor: '#180B1A', borderWidth: 1, borderColor: 'rgba(244,124,181,.5)' }, memberHeroCompact: { flexDirection: 'column', padding: 20 }, memberPlanColumn: { zIndex: 2, flex: 1, minWidth: 280, gap: 8 }, memberPlanName: { color: '#FFF5EA', fontFamily: 'Georgia', fontSize: 39, lineHeight: 46 }, memberPrice: { color: '#FFF5EA', fontFamily: 'Georgia', fontSize: 30 }, memberPeriod: { color: colors.muted, fontSize: 12 }, memberBilling: { color: colors.muted, fontSize: 11, lineHeight: 17 }, renewalLine: { color: '#DCCBD8', fontSize: 11 }, memberActions: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 10 }, memberActionButton: { minWidth: 170 }, memberDivider: { zIndex: 2, width: 1, backgroundColor: 'rgba(255,255,255,.13)' }, memberDividerCompact: { width: '100%', height: 1 }, memberCreditColumn: { zIndex: 2, flex: 1, minWidth: 300, justifyContent: 'center', gap: 10, paddingRight: 120 }, memberCreditColumnCompact: { minWidth: 0, paddingRight: 0 }, memberBalanceRow: { flexDirection: 'row', alignItems: 'center', gap: 12 }, memberBalance: { color: '#FFF5EA', fontFamily: 'Georgia', fontSize: 62, lineHeight: 68 }, balanceBar: { height: 9, overflow: 'hidden', borderRadius: 6, backgroundColor: '#F05C9E' }, balanceBarSubscription: { height: '100%', alignSelf: 'flex-end', backgroundColor: colors.violet }, balanceLegend: { flexDirection: 'row', flexWrap: 'wrap', gap: 22 }, legendItem: { flexDirection: 'row', alignItems: 'center', gap: 8 }, legendDot: { width: 8, height: 8, borderRadius: 4 }, legendValue: { color: colors.text, fontSize: 14, fontWeight: '900' }, legendLabel: { color: colors.muted, fontSize: 9 }, nextGrant: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 5 }, nextGrantText: { flex: 1, color: colors.text, fontSize: 10, lineHeight: 15 },
