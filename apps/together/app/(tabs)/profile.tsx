...
export default function CompanionProfile() {
  ...
  if (!life) return <EmptyState title="Your companion is waiting" body="Choose someone in Discover to begin your shared story." />;
  ...
  return <Screen contentStyle={{ paddingTop: 0 }}>
    ...
    <Pressable disabled={!location} onPress={() => location && router.push(`/location/${location.slug}` as never)} accessibilityRole={location ? 'button' : undefined}>
      <GlassCard style={styles.status}>
        ...
        <View style={{ flex: 1 }}><Text style={styles.statusLabel}>RIGHT NOW</Text><Text style={styles.statusTitle}>{location?.name ?? 'Current place'}</Text><Text style={styles.statusCopy}>{companion.current_activity}</Text></View>
