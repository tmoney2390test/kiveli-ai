...
export default function MomentsTab() {
  const snapshot = useTogether((state) => state.snapshot);const moments=snapshot?.moments??[];const media=snapshot?.generatedMedia??[];
  const [filter, setFilter] = useState<Filter>('All');
  const visible = useMemo(() => moments.filter((moment) => filter === 'All' || filter === 'Photos' ? (filter !== 'Photos' || moment.media.length > 0 || media.some((item)=>item.moment_id===moment.id&&item.status==='ready')) : filter === 'Dates' ? /date/i.test(moment.moment_type) : filter === 'Milestones' ? /milestone|relationship|introduction/i.test(moment.moment_type) : /memory|conversation/i.test(moment.moment_type)), [filter, moments, media]);
