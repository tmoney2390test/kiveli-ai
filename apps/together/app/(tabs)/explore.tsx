...
import{AlertCircle,Check,ChevronDown,ChevronLeft,ChevronRight,Clock3,LockKeyhole,MapPin,Search,Sparkles,UsersRound,X}from'lucide-react-native';
...
export default function Explore(){
  ...
  const requestedWorld=accessibleWorlds.find((item)=>item.slug===params.world);
  const preferredWorld=accessibleWorlds.find((item)=>item.slug===preference.worldSlug);
  const selectedWorld=requestedWorld??preferredWorld??accessibleWorlds.find((item)=>item.id===browsedWorldId)??accessibleWorlds.find((item)=>item.id===companionWorld?.id)??accessibleWorlds[0];
  ...
  const chooseWorld=(world:World)=>{
    if(!canAccessWorld(snapshot,world)){
      setWorldPickerOpen(false);
      nav.push(subscriptionHref({intent:'worlds',returnTo:`/(tabs)/explore?world=${encodeURIComponent(world.slug)}`}));
      return;
    }
...
function WorldCard({world,width,accessible,onPress}:{world:World;width:number;accessible:boolean;onPress:()=>void}){const early=isSubscriberEarlyAccessWorld(world.metadata);return <Pressable accessibilityRole="button" accessibilityLabel={`${accessible?'Explore':'Unlock'} ${world.name}${early?', subscriber early access':''}`} onPress={onPress} style={({pressed})=>[styles.worldCard,{width},Platform.OS==='web'&&webWorldSnapStyle,pressed&&styles.pressed]}><Image source={worldHeroAsset(world.slug)} style={StyleSheet.absoluteFill} contentFit="cover" loading="lazy" priority="low"/><View pointerEvents="none" style={styles.worldCardShade}/><View pointerEvents="none" style={styles.worldCardBottomShade}/>{early?<View style={styles.worldEarlyBadge}><LockKeyhole size={10} color="#FFF4FD"/><Text style={styles.worldEarlyBadgeText}>EARLY ACCESS</Text></View>:null}<View style={styles.worldCardCopy}><Text style={styles.worldCardName}>{world.name}</Text><Text style={styles.worldCardMeta}>{accessible?worldRole(world):'Subscribers'}</Text></View></Pressable>;}
...
function WorldSwitcherSheet({visible,worlds,selectedWorldId,accessibleWorldIds,homeWorldId,loading,error,desktop,reducedMotion,returnFocusRef,onRetry,onSelect,onClose}:{visible:boolean;worlds:World[];selectedWorldId:string;accessibleWorldIds:Set<string>;homeWorldId?:string;loading:boolean;error:string;desktop:boolean;reducedMotion:boolean;returnFocusRef:RefObject<View|null>;onRetry:()=>void;onSelect:(world:World)=>void;onClose:()=>void}){
  ...
  return <Modal visible={visible} transparent statusBarTranslucent navigationBarTranslucent animationType={reducedMotion?'none':mobile?'slide':'fade'} onRequestClose={onClose}>
    <View style={[styles.worldSheetBackdrop,mobile?styles.worldSheetBackdropMobile:styles.worldSheetBackdropDesktop]}>
      ...
      <View nativeID="explore-world-dialog" accessibilityViewIsModal style={[styles.worldSheet,mobile?styles.worldSheetMobile:styles.worldSheetDesktop,{maxHeight:Math.min(height-(mobile?20:48),760),paddingBottom:Math.max(mobile?18:20,safeArea.bottom+12)}]}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.worldSheetGrid}>{worlds.length?worlds.map((world)=>{const selected=world.id===selectedWorldId,home=world.id===homeWorldId,accessible=accessibleWorldIds.has(world.id),early=isSubscriberEarlyAccessWorld(world.metadata);return <Pressable key={world.id} accessibilityRole="button" accessibilityLabel={`${accessible?'Choose':'Unlock'} ${world.name}${home?', home world':''}${early?', subscriber early access':''}`} accessibilityState={{selected}} {...webSelectedState(selected)} onPress={()=>onSelect(world)} style={({pressed})=>[styles.worldSheetCard,{width:cardWidth},selected&&styles.worldSheetCardSelected,pressed&&styles.pressed]}><Image source={worldHeroAsset(world.slug)} style={StyleSheet.absoluteFill} contentFit="cover" loading="lazy" priority="low"/><View pointerEvents="none" style={styles.worldSheetCardShade}/><View style={styles.worldSheetCardTop}>{home?<View style={styles.homeWorldBadge}><Text style={styles.homeWorldBadgeText}>HOME WORLD</Text></View>:early?<View style={styles.homeWorldBadge}><Text style={styles.homeWorldBadgeText}>EARLY ACCESS</Text></View>:<View/>}{selected?<View style={styles.worldSelectedBadge}><Check size={15} strokeWidth={3} color="#281527"/></View>:!accessible?<View style={styles.worldLockedBadge}><LockKeyhole size={13} color="#FFF4FD"/></View>:null}</View><View style={styles.worldSheetCardCopy}><Text numberOfLines={2} style={styles.worldSheetCardName}>{world.name}</Text>{!accessible?<Text style={styles.worldSheetCardAccess}>SUBSCRIBERS</Text>:null}</View></Pressable>}):!loading?<View style={styles.worldSheetEmpty}><Text style={styles.worldSheetEmptyTitle}>No worlds are available yet</Text><Text style={styles.worldSheetEmptyCopy}>Check back when the next world opens.</Text></View>:null}</ScrollView>
      </View>
    </View>
  </Modal>;
}
