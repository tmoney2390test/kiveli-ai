import {scenarios} from '../../src/lib/scenarioCatalog';
...
import{WorldChoiceCard}from'../../src/components/WorldChoiceCard';
...
export default function Explore(){
  ...
  const matchingScenarios=scenarios.filter(s=>s.worldId===selectedWorld.id&&matchesSearch(normalizedQuery,s.title,s.leadName,s.locationName,s.setup,...s.themes));
  const noSearchResults=searching&&!matchingPeople.length&&!matchingLocations.length&&!matchingWorlds.length&&!matchingScenarios.length;
  ...
  return <><Screen contentStyle={[styles.content,{paddingBottom:responsive.bottomClearance},Platform.OS==='web'&&styles.webContent]} scrollRef={scrollRef} onScroll={handleScroll} onContentSizeChange={restoreSavedScroll}>
    ...
    <View style={styles.searchShell}><Search size={19} color={colors.textSecondary}/><TextInput nativeID="explore-search" accessibilityLabel="Search people, places, worlds, and scenarios" value={query} onChangeText={setQuery} placeholder="Search people, places, worlds, and scenarios" placeholderTextColor={colors.muted} returnKeyType="search" style={styles.searchInput}/>{query?<Pressable accessibilityRole="button" accessibilityLabel="Clear Explore search" onPress={()=>setQuery('')} style={styles.clearSearch}><X size={17} color={colors.text}/></Pressable>:null}</View>
    ...
    </View>:<View style={styles.searchHeading}><Text accessibilityRole="header" style={styles.searchTitle}>Search results</Text><Text style={styles.searchMeta}>People, places, worlds, and scenarios matching “{query.trim()}”</Text></View>}
    ...
    {(!searching||matchingScenarios.length>0)?<Section title="Scenarios" subtitle={`Step into a story in ${selectedWorld.name}`} action="See all scenarios" onAction={()=>nav.push(`/scenarios?world=${selectedWorld.slug}`)}><ScenarioBrowser key={selectedWorld.id} worldId={selectedWorld.id} limit={3} query={query}/></Section>:null}
  ...
  return <Modal visible={visible} transparent statusBarTranslucent navigationBarTranslucent animationType={reducedMotion?'none':mobile?'slide':'fade'} onRequestClose={onClose}>
    <View style={[styles.worldSheetBackdrop,mobile?styles.worldSheetBackdropMobile:styles.worldSheetBackdropDesktop]}>
      ...
      <View nativeID="explore-world-dialog" accessibilityViewIsModal style={[styles.worldSheet,mobile?styles.worldSheetMobile:styles.worldSheetDesktop,{maxHeight:Math.min(height-(mobile?20:48),760),paddingBottom:Math.max(mobile?18:20,safeArea.bottom+12)}]}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.worldSheetGrid}>{worlds.length?worlds.map((world)=>{const selected=world.id===selectedWorldId,home=world.id===homeWorldId;return <WorldChoiceCard key={world.id} name={world.name} image={worldHeroAsset(world.slug)} selected={selected} home={home} width={cardWidth} onPress={()=>onSelect(world)}/>}):!loading?<View style={styles.worldSheetEmpty}><Text style={styles.worldSheetEmptyTitle}>No worlds are available yet</Text><Text style={styles.worldSheetEmptyCopy}>Check back when the next world opens.</Text></View>:null}</ScrollView>
      </View>
    </View>
  </Modal>;
}
