...
import{buildExploreContext,exploreCompanionBadge,locationsForExploreCategory,type ExploreCategoryId}from'../../src/lib/explore';
...
export default function Explore(){
  ...
  const refreshCatalog=useCallback(async(force=false)=>{
    ...
    try{
      ...
      setCoreState({worlds:result.worlds,locations:result.locations,characterWorldPresence:result.characterWorldPresence,discoverableCharacters:result.discoverableCharacters,favoriteCharacterTemplateIds:result.favoriteCharacterTemplateIds,lifeEvents:mergeLifeEvents(snapshot.lifeEvents,result.lifeEvents)});
  ...
  return <Screen contentStyle={styles.content} scrollRef={scrollRef} onScroll={handleScroll} onContentSizeChange={restoreSavedScroll}>
    ...
    {showEvents&&context.worldEvents.length?<Section title="Happening now" subtitle={`The parts of ${selectedWorld.name} that are moving right now`} action="Explore places" onAction={()=>nav.push(`/world/places?world=${selectedWorld.slug}`)}><View style={styles.eventStack}>{context.worldEvents.slice(0,selectedIntent==='tonight'?4:2).map((event,index)=>{const location=context.locations.find((item)=>item.id===event.location_id);return <Pressable key={event.id} accessibilityRole="button" accessibilityLabel={`${eventStatus(event)}: ${event.title}`} onPress={()=>discussionHandle?nav.push(`/(tabs)/chat-tab?character=${discussionHandle}&draft=${encodeURIComponent(`What's going on with ${event.title}?`)}`):location?openLocation(location):undefined} style={({pressed})=>[styles.eventCard,index===0&&styles.eventCardFeatured,pressed&&styles.pressed]}><View style={styles.eventIcon}><Sparkles size={16} color={colors.warm}/></View><View style={{flex:1}}><Text style={styles.eventKicker}>{eventStatus(event)}</Text><Text style={styles.eventTitle}>{event.title}</Text><Text numberOfLines={2} style={styles.eventCopy}>{event.narrative_summary}</Text>{location?<Text style={styles.eventLocation}>{location.name}</Text>:null}</View><ChevronRight size={17} color={colors.muted}/></Pressable>;})}</View></Section>:null}
...
function eventStatus(event:Snapshot['lifeEvents'][number]){const now=Date.now(),starts=new Date(event.starts_at).getTime(),ends=event.ends_at?new Date(event.ends_at).getTime():starts+2*60*60*1000;return starts<=now&&ends>now?'HAPPENING NOW':'UPCOMING'}
function mergeLifeEvents(current:Snapshot['lifeEvents'],fresh:Snapshot['lifeEvents']){const ids=new Set(fresh.map((event)=>event.id));return[...fresh,...current.filter((event)=>!ids.has(event.id))].sort((left,right)=>new Date(right.starts_at).getTime()-new Date(left.starts_at).getTime()).slice(0,40)}
