...
export default function Notifications(){
  ...
  const companionPatches=useRef<Record<string,InitiativeLevel|null>>({});
  ...
  useEffect(()=>{const next={...preferences?.companion_initiative_levels};for(const [id,level] of Object.entries(companionPatches.current)){if(level===null)delete next[id];else next[id]=level;}setOverrides(next);},[preferences?.companion_initiative_levels]);
  ...
  const save=async()=>{setBusy(true);try{
    ...
    await invoke('together-notifications',{action:'preferences',pushEnabled:push,characterInitiatedMessages:initiative!=='off',initiativeLevel:initiative,companionInitiativePatches:companionPatches.current,dateReminders:dates,worldEventUpdates:world,quietHoursStart:start,quietHoursEnd:end,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC'});companionPatches.current={};await refresh({force:true});Alert.alert('Notification preferences saved');
  }catch(error){Alert.alert('Could not save',error instanceof Error?error.message:'Try again.');}finally{setBusy(false);}};
  const setOverride=(value:InitiativeLevel|'default')=>{if(!selectedCompanion)return;companionPatches.current[selectedCompanion.id]=value==='default'?null:value;setOverrides((current)=>{const next={...current};if(value==='default')delete next[selectedCompanion.id];else next[selectedCompanion.id]=value;return next;});};
