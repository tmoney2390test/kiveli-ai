...
export default function ArchivedConversation() {
  ...
  return <Screen>
    ...
    {conversation.archived_at ? <View style={styles.notice}><Text style={styles.noticeTitle}>Archived conversation</Text><Text style={styles.noticeCopy}>This transcript is read-only. It is not automatically used as current chat context.</Text></View> : null}
    {conversation.archived_at && !hasActiveConversation && character ? <Pressable onPress={() => void startConversation()} style={styles.startButton}><Text style={styles.startButtonText}>Start a new conversation</Text></Pressable> : null}
  </Screen>;
}

const styles = StyleSheet.create({ header: { flexDirection: 'row', alignItems: 'center', gap: 13 }, archived: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 4 }, archivedText: { color: colors.violet, fontSize: 10, fontWeight: '800' }, messages: { gap: 8 }, bubble: { maxWidth: '86%', padding: 12, borderRadius: radius.md }, user: { alignSelf: 'flex-end', backgroundColor: '#B93467' }, companion: { alignSelf: 'flex-start', backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border }, match: { borderWidth: 2, borderColor: colors.warm }, text: { color: colors.text, fontSize: 14, lineHeight: 21 }, time: { color: 'rgba(255,255,255,.45)', fontSize: 9, marginTop: 5 }, load: { alignSelf: 'center', paddingHorizontal: 13, paddingVertical: 8, borderRadius: radius.pill, backgroundColor: colors.surface }, loadText: { color: colors.rose, fontSize: 11, fontWeight: '800' }, start: { color: colors.dimmed, fontSize: 10, textAlign: 'center' }, notice: { padding: 14, borderRadius: radius.md, backgroundColor: 'rgba(154,104,255,.08)', borderWidth: 1, borderColor: 'rgba(154,104,255,.2)' }, noticeTitle: { color: colors.text, fontWeight: '800' }, noticeCopy: { color: colors.muted, fontSize: 11, lineHeight: 16, marginTop: 4 }, startButton: { minHeight: 48, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: colors.rose }, startButtonText: { color: '#fff', fontWeight: '900' } });
