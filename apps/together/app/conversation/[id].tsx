...
export default function ArchivedConversation() {
  ...
  return <Screen>
    <View style={styles.header}><Pressable accessibilityLabel="Back" onPress={() => router.canGoBack() ? router.back() : router.replace('/chat-tab?inbox=1')}><ArrowLeft color={colors.text} /></Pressable><View style={{ flex: 1 }}><PageTitle>{conversation.title ?? 'Conversation'}</PageTitle><View style={styles.archived}><Archive size={13} color={colors.violet} /><Text style={styles.archivedText}>{conversation.archived_at ? 'Archived conversation' : 'Current conversation'}</Text></View></View></View>
