...
import { activeConversationFor, defaultDirectConversationTitle } from '../../src/lib/conversation';
...
export default function ArchivedConversation() {
  ...
  const startConversation = async () => { if (!character || restoring) return; setRestoring(true); try { await manageConversation({ action: 'ensure', characterInstanceId: character.id }); await refresh(); router.replace(`/chat?character=${character.together_character_templates.slug}` as never); } catch (caught) { setError(caught instanceof Error ? caught.message : 'This chat could not be opened.'); setRestoring(false); } };
  ...
  return <Screen>
    <View style={styles.header}><Pressable accessibilityLabel="Back" onPress={() => router.canGoBack() ? router.back() : router.replace('/chat-tab?inbox=1')}><ArrowLeft color={colors.text} /></Pressable><View style={{ flex: 1 }}><PageTitle>{conversation.title ?? defaultDirectConversationTitle(character?.together_character_templates.name)}</PageTitle><View style={styles.archived}><Archive size={13} color={colors.violet} /><Text style={styles.archivedText}>{conversation.archived_at ? 'Archived conversation' : 'Current conversation'}</Text></View></View></View>
