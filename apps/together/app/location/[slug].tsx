...
export default function LocationDetail() {
  ...
  const breadcrumb = [locationWorld?.name, ...ancestry.map((item) => item.name), location.name].filter(Boolean).join('  â€º  ');
  const now = new Date();
  ...
  return <Screen>
    ...
    {people.length ? <><SectionHeader title="Whoâ€™s here" /><GlassCard style={styles.list}>{people.map(({ person, event }) => {
