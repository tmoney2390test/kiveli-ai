...
export default function LocationDetail() {
  ...
  const lore = location.canonical_lore ?? {};
  const loreDetails = [...(lore.atmosphere ?? []), ...(lore.sensoryDetails ?? []), ...(lore.signatureDetails ?? [])].slice(0, 6);
  const crowdNow = lore.crowdRhythm?.[localDaypart(locationWorld?.timezone, now)];
  ...
  return <Screen>
    ...
    {programs.length ? <><SectionHeader title="Event nights" /><GlassCard style={styles.list}>{programs.map((program) => <Pressable key={program.activityKey} onPress={() => router.push(`/(tabs)/chat-tab?${handle ? `character=${handle}&` : ''}plan=1&location=${location.slug}&world=${locationWorld?.slug ?? ''}&activity=${encodeURIComponent(program.activityKey)}&draft=${encodeURIComponent(`Want to go to ${program.title} at ${location.name}?`)}` as never)} style={styles.programRow}><CalendarDays size={17} color={colors.warm} /><View style={styles.flex}><Text style={styles.rowTitle}>{program.title}</Text><Text style={styles.rowCopy}>{programDays(program.daysOfWeek)} · {formatClock(program.startTime)} · about {formatDuration(program.durationMinutes)}</Text>{program.scheduleNote ? <Text style={styles.programNote}>{program.scheduleNote}</Text> : null}</View><Text style={styles.programAction}>Plan</Text></Pressable>)}</GlassCard></> : null}
