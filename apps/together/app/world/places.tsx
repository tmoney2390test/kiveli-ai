...
import{locationChildren,locationsForWorld,worldBySlug}from'../../src/lib/place';
...
export default function Places(){
  ...
  const world=worldBySlug(snapshot,worldSlug??'')??snapshot.worlds.find((item)=>item.published);
  ...
  const visible=useMemo(()=>{const needle=query.trim().toLowerCase();if(!needle)return roots;return roots.filter((root)=>[root,...locationChildren(snapshot,root.id)].some((item)=>`${item.name} ${item.description} ${item.possible_activities.join(' ')}`.toLowerCase().includes(needle)));},[query,roots,snapshot]);
  const role=String((world as unknown as {world_role?:string}).world_role??'home');
  return <Screen>
    <View style={styles.header}><Pressable onPress={()=>router.canGoBack()?router.back():router.replace(`/(tabs)/worlds?world=${world.slug}`)} style={styles.back}><ArrowLeft color={colors.text}/></Pressable><View style={{flex:1}}><Text style={styles.kicker}>{role==='destination'?'DESTINATION WORLD':'HOME WORLD'}</Text><Text style={styles.title}>{world.name}</Text><Text style={styles.subtitle}>{locations.length} mapped places · {world.timezone}</Text></View></View>
