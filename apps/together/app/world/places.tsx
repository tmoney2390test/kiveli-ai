...
export default function Places(){
  const{world:worldSlug,character:characterKey,planning}=useLocalSearchParams<{world?:string;character?:string;planning?:string}>();
  ...
  const planningMode=planning==='1'||Boolean(characterKey);
  ...
  return <Screen contentStyle={styles.content}>
    ...
    {directory.sections.length?<View style={styles.sections}>{directory.sections.map((section)=>{
      ...
      return <DistrictSection key={section.id} section={section} world={world} isOpen={isOpen} onToggle={()=>toggle(section.id)} cardWidth={cardWidth} cardHeight={cardHeight} now={now} timezone={timezone} characterId={planningCharacter?.id}/>;
...
function DistrictSection({section,world,isOpen,onToggle,cardWidth,cardHeight,now,timezone,characterId}:{section:WorldPlaceDirectorySection;world:World;isOpen:boolean;onToggle:()=>void;cardWidth:number;cardHeight:number;now:Date;timezone:string;characterId?:string}){
  ...
  return <View style={styles.section}>
    ...
    {isOpen?<View style={styles.expanded}>
      {district?<View style={styles.districtActions}><Text style={styles.insideLabel}>INSIDE {district.name.toUpperCase()}</Text><Pressable accessibilityRole="link" onPress={()=>router.push(locationHref(district.slug,world.slug,characterId) as never)} style={styles.detailsLink}><Text style={styles.detailsLinkText}>District details</Text><ArrowUpRight size={14} color={colors.rose}/></Pressable></View>:<Text style={styles.citywideNote}>These places are visible now and can move into a district as the world map becomes more detailed.</Text>}
      {section.places.length?<View style={styles.placeGrid}>{section.places.map((place)=><PlaceCard key={place.id} place={place} world={world} width={cardWidth} height={cardHeight} districtName={district?.name} districtSlug={district?.slug} now={now} timezone={timezone} characterId={characterId}/>)}</View>:<View style={styles.emptyDistrict}><Text style={styles.emptyDistrictTitle}>No matching places here</Text><Text style={styles.emptyDistrictCopy}>Try another filter to explore this district.</Text></View>}
    </View>:null}
  </View>;
}

function PlaceCard({place,world,width,height,districtName,districtSlug,now,timezone,characterId}:{place:Location;world:World;width:number;height:number;districtName?:string;districtSlug?:string;now:Date;timezone:string;characterId?:string}){
  ...
  return <Pressable accessibilityRole="link" accessibilityLabel={`Open ${place.name}${districtName?` in ${districtName}`:''}. ${availability.statusLabel}.`} onPress={()=>router.push(locationHref(place.slug,world.slug,characterId) as never)} style={({pressed})=>[styles.placeCard,{width,height},pressed&&styles.placeCardPressed]}>
...
function locationHref(slug:string,worldSlug:string,characterId?:string){return`/location/${slug}?world=${encodeURIComponent(worldSlug)}${characterId?`&character=${encodeURIComponent(characterId)}&planning=1`:''}`;}
