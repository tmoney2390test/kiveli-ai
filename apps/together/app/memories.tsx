...
export default function Memories(){
  ...
  const categoryLabel=MEMORY_CATEGORY_OPTIONS.find((option)=>option.key===category)?.label??'All memories';
  const sortLabel=SORTS.find((option)=>option.key===sort)?.label??'Pinned first';
  const avatarSize=desktop?112:compact?82:96;
  const hero=<View style={[styles.hero,desktop&&styles.heroDesktop]}>
    <View style={[styles.heroIdentity,compact&&styles.heroIdentityCompact]}>
      {companion?<View style={[styles.portraitShell,{width:avatarSize,height:avatarSize,borderRadius:avatarSize/2}]}><CharacterAvatar slug={companion.together_character_templates.slug} name={name} template={companion.together_character_templates} version={companion.together_character_versions} size={avatarSize}/></View>:null}
      <View style={styles.heroCopy}>
        ...
        {privacyMode?<Text style={styles.heroSubtitle}>Review what companions retain across this Life.</Text>:null}
      </View>
    </View>
  </View>;
    ...
    <FlatList
      ...
      ListHeaderComponent={<View>
        ...
        <View style={styles.filterToolbar}>
          <Pressable accessibilityRole="button" accessibilityLabel={`Filter memories. ${categoryLabel} selected`} accessibilityState={{expanded:filterOpen}} onPress={()=>setFilterOpen(true)} style={({pressed})=>[styles.filterTrigger,category!=='all'&&styles.filterTriggerActive,pressed&&styles.pressed]}><SlidersHorizontal size={15} color={category!=='all'?'#281527':colors.textSecondary}/><Text numberOfLines={1} style={[styles.filterTriggerText,category!=='all'&&styles.filterTriggerTextActive]}>{categoryLabel}</Text><ChevronDown size={14} color={category!=='all'?'#281527':colors.textSecondary}/></Pressable>
          <Pressable accessibilityRole="button" accessibilityLabel={`Sort memories. ${sortLabel} selected`} accessibilityState={{expanded:sortOpen}} onPress={()=>setSortOpen(true)} style={({pressed})=>[styles.filterTrigger,sort!=='pinned'&&styles.filterTriggerActive,pressed&&styles.pressed]}><ArrowUpDown size={15} color={sort!=='pinned'?'#281527':colors.textSecondary}/><Text numberOfLines={1} style={[styles.filterTriggerText,sort!=='pinned'&&styles.filterTriggerTextActive]}>{sortLabel}</Text><ChevronDown size={14} color={sort!=='pinned'?'#281527':colors.textSecondary}/></Pressable>
          <Pressable accessibilityRole="button" accessibilityLabel="Search and manage memories" onPress={()=>setControlsOpen(true)} style={({pressed})=>[styles.filterTool,pressed&&styles.pressed]}><Search size={18} color={query?colors.rose:colors.textSecondary}/>{query?<View style={styles.activeDot}/>:null}</Pressable>
        </View>
      </View>}
...
function MemoryJournalControls({visible,desktop,query,canSelect,onClose,onQuery,onSelect,onPrivacy}:{visible:boolean;desktop:boolean;query:string;canSelect:boolean;onClose:()=>void;onQuery:(value:string)=>void;onSelect:()=>void;onPrivacy:()=>void}){
  ...
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}><View style={[styles.modalRoot,desktop?styles.modalCentered:styles.modalBottom]}><Pressable accessibilityLabel="Close memory controls" onPress={onClose} style={StyleSheet.absoluteFill}/><FrostedSurface intensity={96} style={[styles.controlsSheet,{paddingBottom:Math.max(20,insets.bottom)},desktop&&styles.controlsSheetDesktop]}><View style={styles.sheetHeader}><View><Text style={styles.sheetKicker}>MEMORY JOURNAL</Text><Text style={styles.sheetTitle}>Search and manage</Text></View><Pressable accessibilityLabel="Close" onPress={onClose} style={styles.sheetClose}><X size={20} color={colors.muted}/></Pressable></View><ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={styles.controlsContent}>
    ...
    <Pressable onPress={onClose} style={styles.showResults}><Text style={styles.showResultsText}>Done</Text></Pressable>
  </ScrollView></FrostedSurface></View></Modal>;
}

function MemoryOptionPicker({visible,desktop,title,eyebrow,options,value,onClose,onChoose}:{visible:boolean;desktop:boolean;title:string;eyebrow:string;options:Array<{key:string;label:string;count?:number}>;value:string;onClose:()=>void;onChoose:(value:string)=>void}){
  const insets=useSafeAreaInsets();
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}><View style={[styles.modalRoot,desktop?styles.modalCentered:styles.modalBottom]}><Pressable accessibilityLabel={`Close ${title.toLowerCase()}`} onPress={onClose} style={StyleSheet.absoluteFill}/><FrostedSurface intensity={96} style={[styles.pickerSheet,{paddingBottom:Math.max(20,insets.bottom)},desktop&&styles.pickerSheetDesktop]}><View style={styles.sheetHeader}><View><Text style={styles.sheetKicker}>{eyebrow}</Text><Text style={styles.sheetTitle}>{title}</Text></View><Pressable accessibilityLabel="Close" onPress={onClose} style={styles.sheetClose}><X size={20} color={colors.muted}/></Pressable></View><View accessibilityRole="radiogroup" style={styles.pickerOptions}>{options.map((option)=>{const selected=value===option.key;return <Pressable key={option.key} accessibilityRole="radio" accessibilityState={{checked:selected}} onPress={()=>onChoose(option.key)} style={({pressed})=>[styles.pickerOption,selected&&styles.pickerOptionActive,pressed&&styles.pressed]}><Text style={[styles.pickerOptionText,selected&&styles.pickerOptionTextActive]}>{option.label}</Text>{typeof option.count==='number'?<Text style={styles.pickerOptionCount}>{option.count}</Text>:null}{selected?<Check size={18} strokeWidth={2.6} color={colors.rose}/>:null}</Pressable>;})}</View></FrostedSurface></View></Modal>;
}
