...
import { ErrorState, GradientButton, GlassCard, LoadingSkeleton, Screen } from '../../../src/components';
...
import type { CreatorCommunicationConfig, CreatorConnectionConfig, CreatorDraft, CreatorIdentityConfig, CreatorLifeConfig, CreatorPersonalityConfig, CreatorRoutineBlock, CreatorStep } from '../../../src/types';
...
export default function CreatorStudioRoute() {
  ...
  const applyDraft = useCallback((next: CreatorDraft, initializeStep = false) => {
    setDraft(next); setIdentity(next.identity_config); setPersonality(next.personality_config); setCommunication(next.communication_config);
    setConnection(next.connection_config); setLife(next.life_config); setRoutine(next.routine_config?.blocks ?? []);
