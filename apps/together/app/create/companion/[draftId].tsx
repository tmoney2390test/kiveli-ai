...
import { ArrowLeft, ArrowRight, Camera, Check, ChevronDown, ChevronLeft, ChevronRight, ImagePlus, MapPin, Plus, RefreshCw, Sparkles, Trash2, Upload, UserRound, X } from 'lucide-react-native';
import { ErrorState, GradientButton, GlassCard, KivelleCreditIcon, LoadingSkeleton, Screen } from '../../../src/components';
...
function IdentityEditor({ value, onChange }: { value: CreatorIdentityConfig; onChange: (next: CreatorIdentityConfig) => void }) {
  return <View style={styles.form}>
    ...
    <View style={styles.twoColumn}><Field label="Gender" value={value.gender ?? ''} onChange={(gender) => onChange({ ...value, gender })} /><Field label="Pronouns" value={value.pronouns} placeholder="she/her" onChange={(pronouns) => onChange({ ...value, pronouns })} /><Field label="Job or role" value={value.occupation} onChange={(occupation) => onChange({ ...value, occupation })} /></View>
...
function AppearanceEditor({ draft, description, onDescription, busy, onBusy, onDraft, onGenerate, onChoose }: { draft: CreatorDraft; description: string; onDescription: (value: string) => void; busy: string; onBusy: (value: string) => void; onDraft: (draft: CreatorDraft) => void; onGenerate: () => void; onChoose: (id: string) => void }) {
  ...
  const [referenceOrigin, setReferenceOrigin] = useState<'fictional_ai' | 'authorized_real_person'>('fictional_ai');
  ...
  return <View style={styles.form}>
    <View style={styles.uploadHero}><ImagePlus size={26} color={colors.rose} /><View style={{ flex: 1 }}><Text style={styles.uploadTitle}>Upload their identity portrait</Text><Text style={styles.uploadCopy}>One clear portrait becomes the private reference used to keep their face consistent.</Text></View></View>
    <Pressable accessibilityRole="button" accessibilityState={{ expanded: guidelinesOpen }} accessibilityLabel="Photo guidelines" onPress={() => setGuidelinesOpen((value) => !value)} style={styles.guidelineHeader}><View style={styles.guidelineRule} /><Text style={styles.guidelineTitle}>Photo guidelines</Text><ChevronDown size={16} color={colors.muted} style={{ transform: [{ rotate: guidelinesOpen ? '180deg' : '0deg' }] }} /><View style={styles.guidelineRule} /></Pressable>
    {guidelinesOpen ? <View style={styles.guidelines}><Text style={styles.guidelineCopy}>Use a front-facing, shoulder-up portrait with both shoulders visible. Even light, a neutral expression, and at least 1024×1024 work best.</Text><View style={styles.guidelineCards}><View style={[styles.guidelineCard, styles.guidelineGood]}><Check size={17} color="#79e6b0" /><Text style={styles.guidelineCardTitle}>Best result</Text><Text style={styles.guidelineCardCopy}>One adult · clear face · natural angle · no filters</Text></View><View style={[styles.guidelineCard, styles.guidelineBad]}><X size={17} color="#ff8d9c" /><Text style={styles.guidelineCardTitle}>Avoid</Text><Text style={styles.guidelineCardCopy}>Groups · cropped forehead · sunglasses · explicit source photos</Text></View></View></View> : null}
    <View style={styles.field}><Text style={styles.fieldLabel}>Reference kind *</Text><Text style={styles.fieldHelp}>This protects media generation from treating a real person as a fictional adult identity.</Text><View style={styles.referenceChoices}><Pressable accessibilityRole="radio" accessibilityState={{ checked: referenceOrigin === 'fictional_ai' }} onPress={() => setReferenceOrigin('fictional_ai')} style={[styles.referenceChoice, referenceOrigin === 'fictional_ai' && styles.referenceChoiceSelected]}><Text style={styles.referenceTitle}>Original fictional / AI-created</Text><Text style={styles.referenceCopy}>May be used for eligible SFW and adult-capable media.</Text></Pressable><Pressable accessibilityRole="radio" accessibilityState={{ checked: referenceOrigin === 'authorized_real_person' }} onPress={() => setReferenceOrigin('authorized_real_person')} style={[styles.referenceChoice, referenceOrigin === 'authorized_real_person' && styles.referenceChoiceSelected]}><Text style={styles.referenceTitle}>Real adult with permission</Text><Text style={styles.referenceCopy}>Safe media only; never used for adult generation.</Text></Pressable></View></View>
    ...
    <View style={styles.uploadActions}><Pressable accessibilityRole="button" accessibilityLabel="Choose portrait from photo library" disabled={Boolean(busy)} onPress={() => void uploadPhoto('library')} style={[styles.uploadButton, Boolean(busy) && styles.disabled]}><Upload size={17} color="#fff" /><Text style={styles.uploadButtonText}>{busy === 'upload' ? 'Uploading…' : 'Choose photo'}</Text></Pressable>{Platform.OS !== 'web' ? <Pressable accessibilityRole="button" accessibilityLabel="Take a portrait photo" disabled={Boolean(busy)} onPress={() => void uploadPhoto('camera')} style={styles.takeButton}><Camera size={17} color={colors.text} /><Text style={styles.takeButtonText}>Take photo</Text></Pressable> : null}</View>
    <View style={styles.orRow}><View style={styles.guidelineRule} /><Text style={styles.orText}>OR GENERATE</Text><View style={styles.guidelineRule} /></View>
    <Pressable accessibilityRole="button" accessibilityLabel="Generate three appearance options for 40 Kivelle Credits" disabled={Boolean(busy)} onPress={onGenerate} style={[styles.generateButton, Boolean(busy) && styles.disabled]}><Sparkles size={18} color={colors.rose} /><View style={{ flex: 1 }}><Text style={styles.generateTitle}>{busy === 'appearance' ? 'Generating three identities…' : readyAssets.length ? 'Generate another set' : 'Generate three appearance options'}</Text><Text style={styles.generateDetail}>Original fictional adults · failed generations are refunded</Text></View><View style={styles.credit}><KivelleCreditIcon size={14} /><Text style={styles.creditText}>40</Text></View></Pressable>
    {draft.assets.some((asset) => asset.status === 'queued' || asset.status === 'generating') ? <GlassCard style={styles.emptyAppearance}><Sparkles size={30} color={colors.rose} /><Text style={styles.emptyTitle}>Preparing their identity…</Text><Text style={styles.emptyCopy}>You can keep editing while Kivelle prepares the portrait. This updates automatically.</Text></GlassCard> : null}
    {draft.assets.some((asset) => asset.status === 'failed') && !draft.assets.some((asset) => asset.status === 'queued' || asset.status === 'generating' || asset.status === 'ready') ? <GlassCard style={styles.emptyAppearance}><RefreshCw size={30} color={colors.muted} /><Text style={styles.emptyTitle}>That portrait could not be prepared</Text><Text style={styles.emptyCopy}>Try another clear JPEG, or generate an original identity.</Text></GlassCard> : null}
    {readyAssets.length ? <View style={styles.lookGrid}>{readyAssets.map((asset) => <Pressable key={asset.id} accessibilityRole="radio" accessibilityLabel={`${asset.label} appearance`} accessibilityState={{ checked: asset.selected }} onPress={() => onChoose(asset.id)} style={[styles.lookCard, asset.selected && styles.lookSelected]}>{asset.signedUrl ? <Image source={{ uri: asset.signedUrl }} style={styles.lookImage} contentFit="cover" contentPosition="top" /> : <View style={[styles.lookImage, styles.fallback]}><UserRound size={30} color={colors.rose} /></View>}<View style={styles.lookInfo}><Text style={styles.lookLabel}>{asset.label}</Text><Text style={styles.lookDescription} numberOfLines={3}>{asset.description}</Text>{asset.selected ? <View style={styles.selectedPill}><Check size={12} color="#fff" /><Text style={styles.selectedPillText}>IDENTITY SELECTED</Text></View> : <Text style={styles.chooseText}>{busy === `look:${asset.id}` ? 'Selecting…' : 'Use this identity'}</Text>}</View></Pressable>)}</View> : <GlassCard style={styles.emptyAppearance}><UserRound size={36} color={colors.violet} /><Text style={styles.emptyTitle}>No canonical portrait yet</Text><Text style={styles.emptyCopy}>Upload one clear portrait or generate an original identity before continuing.</Text></GlassCard>}
  </View>;
}
