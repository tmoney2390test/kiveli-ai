...
import { ArrowLeft, ArrowRight, Camera, Check, ChevronDown, ChevronLeft, ChevronRight, MapPin, Plus, RefreshCw, Sparkles, Trash2, UserRound, X } from 'lucide-react-native';
...
export default function CreatorStudioRoute() {
  ...
  return <Screen contentStyle={styles.screen}>
    <View style={styles.topbar}>
      ...
      <View style={{ flex: 1 }}><Text style={styles.kicker}>CREATE A COMPANION</Text><Text style={styles.title}>{identity.name}</Text><Text style={styles.subtitle}>{draft.world?.name ?? 'Kivelle'}</Text></View>
    ...
    <View style={styles.mobileProgress}><Text style={styles.progressLabel}>{stepIndex + 2} OF {steps.length + 1} · {activeStep.label.toUpperCase()}</Text><View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${((stepIndex + 2) / (steps.length + 1)) * 100}%` }]} /></View></View>
...
function AppearanceEditor({ draft, description, onDescription, busy, onBusy, onDraft, onGenerate, onChoose }: { draft: CreatorDraft; description: string; onDescription: (value: string) => void; busy: string; onBusy: (value: string) => void; onDraft: (draft: CreatorDraft) => void; onGenerate: () => void; onChoose: (id: string) => void }) {
  ...
  return <View style={styles.form}>
    ...
    <CreatorModal visible={pickerOpen} title={`Choose ${draft.identity_config.name}'s avatar`} onClose={() => setPickerOpen(false)} large>
      ...
      {readyAssets.length ? <View style={avatarStyles.grid}>{readyAssets.map((asset) => <Pressable key={asset.id} accessibilityRole="radio" accessibilityLabel={`${asset.label} avatar`} accessibilityState={{ checked: asset.selected, disabled: Boolean(busy) }} disabled={Boolean(busy)} onPress={() => onChoose(asset.id)} style={[avatarStyles.option, { width: width < 600 ? '100%' : '31%' }, asset.selected && avatarStyles.selected]}>
