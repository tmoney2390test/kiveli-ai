...
type PrivacyChoices = { aiDataConsent: { decision: 'accepted' | 'declined' | 'withdrawn' | 'unknown'; recordedAt: string | null }; privateTextPreference: 'standard' | 'mature' | 'explicit' | null; disclosure: { providers: string[]; dataCategories: string[]; version: string } };
...
export default function Privacy() {
  ...
  const savePrivacyChoices = async (next: { aiDataSharing?: boolean; privateTextPreference?: 'standard' | 'mature' | 'explicit' }) => {
    if (!privacyChoices || privacyChoiceBusy) return;
    setPrivacyChoiceBusy(true); setPrivacyChoiceError('');
    try {
      const updated = await manageAccount<PrivacyChoices>({ action: 'privacy_choices', aiDataSharing: next.aiDataSharing ?? privacyChoices.aiDataConsent.decision === 'accepted', privateTextPreference: next.privateTextPreference ?? privacyChoices.privateTextPreference ?? 'standard', source: 'privacy' });
      setPrivacyChoices(updated); await refresh();
    } catch (error) { setPrivacyChoiceError(error instanceof Error ? error.message : 'Your privacy choice could not be saved.'); }
    finally { setPrivacyChoiceBusy(false); }
  };
  ...
  return <Screen>
    ...
    <SectionHeader title="AI & private conversations" />
    <View style={styles.choiceCard}>
      <Text style={styles.rowTitle}>Private conversation level</Text>
      <Text style={styles.rowBody}>Choose the level you want in private chats. Public sharing, notifications, voice, and native visual media keep their separate limits.</Text>
      <View accessibilityRole="radiogroup" accessibilityLabel="Private conversation level" style={styles.choiceRow}>{(['standard','mature','explicit'] as const).map((choice)=><Pressable key={choice} accessibilityRole="radio" accessibilityState={{checked:privacyChoices?.privateTextPreference===choice,disabled:privacyChoiceBusy}} disabled={privacyChoiceBusy||!privacyChoices} onPress={()=>void savePrivacyChoices({privateTextPreference:choice})} style={[styles.choiceButton,privacyChoices?.privateTextPreference===choice&&styles.choiceButtonSelected]}><Text style={[styles.choiceText,privacyChoices?.privateTextPreference===choice&&styles.choiceTextSelected]}>{choice.charAt(0).toUpperCase()+choice.slice(1)}</Text></Pressable>)}</View>
    </View>
    <View style={styles.choiceCard}>
      <View style={styles.choiceToggleRow}><View style={{flex:1}}><Text style={styles.rowTitle}>Share data with AI providers</Text><Text style={styles.rowBody}>Required for AI chat and media. Depending on the feature, Kivelle may send conversation context, memories, uploaded photos, or audio to {privacyChoices?.disclosure?.providers?.join(', ') || 'the providers named in Kivelle’s current disclosure'}.</Text></View><Switch accessibilityLabel="Share data with AI providers" disabled={privacyChoiceBusy||!privacyChoices} value={privacyChoices?.aiDataConsent.decision==='accepted'} onValueChange={(value)=>void savePrivacyChoices({aiDataSharing:value})} trackColor={{false:colors.elevated,true:colors.rose}} /></View>
    </View>
...
const styles = StyleSheet.create({
  header: { flexDirection: 'row', alignItems: 'center', gap: 14 }, lead: { color: colors.muted, lineHeight: 20 }, row: { minHeight: 70, flexDirection: 'row', alignItems: 'center', gap: 12, padding: 13, borderRadius: radius.md, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border }, rowTitle: { color: colors.text, fontWeight: '800' }, rowBody: { color: colors.muted, fontSize: 12, lineHeight: 17, marginTop: 3 }, rowMeta: { color: colors.violet, fontSize: 10.5, fontWeight: '800', marginTop: 5 }, choiceCard:{gap:12,padding:14,borderRadius:radius.md,backgroundColor:colors.surface,borderWidth:1,borderColor:colors.border},choiceToggleRow:{flexDirection:'row',alignItems:'center',gap:12},choiceRow:{flexDirection:'row',flexWrap:'wrap',gap:8},choiceButton:{minHeight:44,minWidth:88,alignItems:'center',justifyContent:'center',paddingHorizontal:14,borderRadius:radius.md,borderWidth:1,borderColor:colors.border,backgroundColor:'rgba(255,255,255,.03)'},choiceButtonSelected:{borderColor:colors.rose,backgroundColor:'rgba(229,74,163,.14)'},choiceText:{color:colors.muted,fontWeight:'800'},choiceTextSelected:{color:colors.text}, dangerRow: { borderColor: 'rgba(255,107,121,.35)' }, note: { color: colors.muted, fontSize: 12, lineHeight: 18, padding: spacing.md, borderRadius: radius.md, backgroundColor: 'rgba(154,104,255,.07)' }, error: { color: colors.danger, fontSize: 12, lineHeight: 18, fontWeight: '700' },
