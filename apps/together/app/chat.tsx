...
function ChatSession() {
  ...
  const lastIncomingMessage=useRef<{conversationId:string;id:string|null}|null>(null);
  ...
  useEffect(()=>{
    ...
    const incoming=messages.findLast(message=>message.role==='assistant'&&message.delivery_status==='complete');
    const previous=lastIncomingMessage.current;
    if(previous?.conversationId===conversation.id&&incoming?.id&&previous.id&&previous.id!==incoming.id&&!keepPinnedToBottom.current)setHasNewMessages(true);
    lastIncomingMessage.current={conversationId:conversation.id,id:incoming?.id??null};
  },[messages,loading,conversation?.id]);
  useEffect(()=>{if(!showJumpToLatest)setHasNewMessages(false);},[showJumpToLatest]);
  ...
  const mobileLocationBackground=(activeScenario?scenarioAssets[activeScenario.id]:undefined)??sceneVisualSource(snapshot,character,chatContext);
  ...
  return <KeyboardAvoidingView style={[styles.screen,mobileViewportStyle]} behavior={Platform.OS === 'ios' ? 'padding' : Platform.OS === 'android' ? 'height' : undefined}>
    <View style={[styles.shell,desktopChat&&styles.shellDesktop]}>
      ...
      <View style={[styles.conversation,width<720&&styles.conversationWithLocation]}>
        ...
        {width<720?<MobileChatMediaHeader
          ...
          subtitle={activeScenario?'Scenario in progress':location.trim().toLowerCase()==='home'?'At home':`At ${location}`}
        ...
        />:<ChatHeader character={character} location={location} status={activeScenario?'Scenario in progress':undefined} mediaCount={chatGalleryItems.length} onBack={openMessagesInbox} onMedia={openChatGallery} onCall={()=>navigateChatSurface(`/call?character=${character.id}&conversation=${conversation.id}`)} onMenu={()=>setShowConversationMenu((value)=>!value)} />}
        ...
        <ScenarioConversationBanner key={conversation.id} conversationId={conversation.id} scope={snapshot.activeContinuity?.id??'default'} onScenarioChange={setActiveScenario}/>
        ...
        >
          ...
          {conversationReady&&!activeScenario?<SceneCard character={character} context={chatContext} snapshot={snapshot} roster={sharedSceneRoster} />:null}
        ...
        <JumpToLatestButton visible={!showPlans&&showJumpToLatest} bottom={composerHeight+12} hasNewMessages={hasNewMessages} onPress={jumpToLatest}/>
...
function Composer({compact,desktop,inputRef,conversationId,character,input,onChangeInput,onDictation,onDictationError,onDictationStart,pendingImage,photoUploadPhase,onAddPhoto,onRemovePhoto,sending,onSend,onMoment,autoDialogue,autoDialogueBusy,canSuggest,onSuggest,onSuggestOptions,onClearSuggestion,onFocus,onLayout}:{compact:boolean;desktop:boolean;inputRef:{current:TextInput|null};conversationId:string;character:CharacterInstance;input:string;onChangeInput:(value:string)=>void;onDictation:(text:string)=>void;onDictationError:(message:string)=>void;onDictationStart:()=>void;pendingImage:PendingImage|null;photoUploadPhase:PhotoUploadPhase;onAddPhoto:()=>void;onRemovePhoto:()=>void;sending:boolean;onSend:()=>void;onMoment:(mode:MediaMomentMode)=>void;autoDialogue:AutoDialogueSuggestion|null;autoDialogueBusy:boolean;canSuggest:boolean;onSuggest:()=>void;onSuggestOptions:()=>void;onClearSuggestion:()=>void;onFocus?:()=>void;onLayout?:(height:number)=>void}) {
  ...
  return <View onLayout={event=>onLayout?.(event.nativeEvent.layout.height)} style={[styles.composerWrap,compact&&styles.composerWrapCompact,{paddingBottom:Math.max(8,insets.bottom)}]}>
...
const styles=StyleSheet.create({
  ...
  ,composerTextInputMobile:{fontSize:16}
