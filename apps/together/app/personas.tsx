...
export default function Personas() {
  ...
  useEffect(() => {
    if (snapshot || loading || error || attemptedLoad.current) return;
    attemptedLoad.current = true;
    void refresh();
  }, [error, loading, refresh, snapshot]);
  if (!snapshot) return loading || (!error && !attemptedLoad.current)
    ? <LoadingSkeleton label="Loading your Kivelle Lives…" />
    : <ErrorState message={error ?? 'Your Lives could not be loaded.'} onRetry={() => void refresh({ force: true })} />;
  ...
  const removeLife = (life: KivelleContinuity) => confirmAction({
    ...
    message: 'This permanently removes this Alternate Life and its relationships, memories, plans, Dates, Moments, Stories, and media state. Your Main Life is untouched.',
...
const styles = StyleSheet.create({
  ...
  add: { minHeight: 54, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border }, addText: { color: colors.rose, fontWeight: '800' }, empty: { color: colors.muted, lineHeight: 19 },notice:{color:colors.success,fontSize:13},disabled:{opacity:.48},
});
