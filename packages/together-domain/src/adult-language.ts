...
// High-signal slang in this list (cock, ass, sexual emoji) routes as adult
// language immediately. Everyday collisions stay in LOW_CONFIDENCE and still
// need visual, possessive, exposure, size, or sexual context. SAFE_CONTEXT
// keeps golf balls, chicken breasts, cake racks, and similar phrases SFW.
...
const LOW_CONFIDENCE=new Set([
  'cat','kitty','kitty cat','flower','lotus','rosebud','peach','cookie','pie','pink','lips','slit','crease','folds','petals','button','bean','bag','box','head','hood','meat','stick','tool','rod','pole','girls','twins','cheeks','buns','cakes','cake','rear','bottom','ring','hole','bush','flower symbol','taco',
  'shaft','member','package','bulge','junk','goods','the goods','equipment','hardware','anatomy','wood','bone','tip','knob','dangle','sausage','salami','banana','eggplant','aubergine','boys','berries','jewels','sack','nuts','jugs','melons','cans','coconuts','puppies','chest','bust','bags','bumps','nip','behind','posterior','backside',
]);
...
const SAFE_CONTEXT:Partial<Record<AdultAnatomyCategory,RegExp>>={
  breasts:/\b(?:chicken|turkey|duck) breasts?\b|\bbreast (?:cancer|screening|exam|feeding|milk|stroke|pocket|plate)\b|\b(?:spice|wine|coat|hat|bike|dish|plate|magazine|clothing|server|cake) racks?\b|\brack of (?:lamb|ribs)\b|\b(?:cake|pies?|cookies?|dishes|plates|spices?)\s+on the rack\b/i,
  male_genitalia:/\b(?:golf|tennis|soccer|baseball|basketball|footballs?|billiard|pool) balls?\b|\bballs? (?:game|practice|field|court)\b|\bmeatballs?\b|\bnuts? (?:and bolts|allergy|allergies|roasted|recipe)\b|\bpackage (?:delivery|tracking|manager|installer)\b|\btool box\b|\b(?:chicken|rooster) cock\b|\b(?:elevator|drive|golf|crank|prop) shafts?\b/i,
...
const SEXUAL_CONTEXT=/\b(?:sex|sexual|fuck|suck|lick|touch|stroke|rub|masturbat|orgasm|cum|penetrat|oral|anal|horny|aroused|girth|circumference|grosor|circonférence|umfang|circunfer)\w*\b/i;
const ANATOMY_SIZE_CONTEXT=/\b(?:\d+(?:\.\d+)?\s*(?:["”]|inch(?:es)?|in\.?|cm|mm|centimet(?:er|re)s?|millimet(?:er|re)s?|pulgadas?|pouces?|zoll|pollici|polegadas?|インチ|인치|英寸)|(?:girth|circumference|grosor|circonférence|umfang|circunfer[eê]ncia|太さ|둘레|周长))\b/iu;
...
const MULTILINGUAL_ALIASES:Array<[RegExp,string]>=[
  ...
  [/\b(?:vergas?|pollas?|pijas?|pitos?)\b/giu,'penis'],
  [/\b(?:coños?|chochos?)\b/giu,'vagina'],
  [/\b(?:(?:une|la|ma|ta|sa)\s+bite|teubs?|zizis?)\b/giu,'penis'],
  [/\b(?:chattes?|minettes?)\b/giu,'vagina'],
  [/\b(?:cazzi|cazzo|minchia)\b/giu,'penis'],
  [/\b(?:fica|figa|fighe)\b/giu,'vagina'],
  [/\b(?:schw[aä]nze?|pimmel)\b/giu,'penis'],
  [/\b(?:muschis?|m[öo]sen?)\b/giu,'vagina'],
  [/\b(?:paus?|picas?|rolas?|caralhos?)\b/giu,'penis'],
  [/\b(?:bucetas?|xotas?)\b/giu,'vagina'],
  [/(?:ちんこ|ちんぽ|ちんちん)/gu,'penis'],
  [/(?:まんこ|おまんこ)/gu,'vagina'],
  [/(?:자지)/gu,'penis'],
  [/(?:보지)/gu,'vagina'],
  [/(?:鸡巴|屌)/gu,'penis'],
...
export function analyzeAdultLanguage(text:string):AdultLanguageAnalysis{
  ...
  if(ambiguous.length){
    const highSignal=ambiguous.filter(({term})=>!LOW_CONFIDENCE.has(term));
    const lowSignal=ambiguous.filter(({term})=>LOW_CONFIDENCE.has(term));
    const visual=VISUAL_REQUEST.test(normalized),exposure=EXPOSURE_CONTEXT.test(normalized),sexual=SEXUAL_CONTEXT.test(normalized)||ANATOMY_SIZE_CONTEXT.test(normalized),possessive=ambiguous.some(({term})=>hasPossessiveReference(normalized,term)),emoji=ambiguous.some(({term})=>term.endsWith(' symbol'));
    ...
    if(lowSignal.length){
      ...
      if(score>=85){
        lowSignal.forEach(({category,term})=>{categories.add(category);matched.add(term);});
        if(tier==='none')tier='ambiguous_euphemism';
      }
    }
  }
