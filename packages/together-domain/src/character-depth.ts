import { selectCharacterPerformance } from './character-performance.ts';
...
export type CharacterResponseMode='danger'|'casual'|'playful'|'supportive'|'vulnerable'|'conflicted'|'repair'|'practical'|'storytelling'|'affectionate';
...
export type CharacterVoiceCard={
  ...
  performance?:ReturnType<typeof selectCharacterPerformance>;
};
...
export function compileCharacterVoiceCard(input:{bible:unknown;characterName?:string;occupation?:string;message:string;mode:CharacterResponseMode;relationshipStage?:string;trust?:number;interactionMode?:string;interactionQuality?:string;recentAssistantMessages?:string[];contentMode?:string;age?:number|null;pressure?:ScenePressure}):CharacterVoiceCard{
  const bible=normalizeCharacterDepthBible(input.bible,row(input.bible));const pressure=input.pressure??assessScenePressure({message:input.message,...(input.interactionMode?{interactionMode:input.interactionMode}:{})});const performance=selectCharacterPerformance({bible:input.bible,mode:input.mode,pressure,...(input.occupation?{occupation:input.occupation}:{}),...(input.recentAssistantMessages?{recentAssistantMessages:input.recentAssistantMessages}:{})});const seed=`${input.characterName??''}|${input.message}|${input.mode}`;
  ...
  const candidates=usableShapes.filter((shape)=>!recentShapes.includes(shape));const shape=pressure.phase==='immediate'?'short_burst':pick(candidates.length?candidates:usableShapes,`${seed}|shape`);
  const anecdote=pressure.phase!=='none'?null:selectCharacterAnecdote({bible,message:input.message,...(input.relationshipStage!==undefined?{relationshipStage:input.relationshipStage}:{}),...(input.trust!==undefined?{trust:input.trust}:{}),...(input.recentAssistantMessages!==undefined?{recentAssistantMessages:input.recentAssistantMessages}:{}),...(input.contentMode!==undefined?{contentMode:input.contentMode}:{}),...(input.age!==undefined?{age:input.age}:{})});
  const texture=pressure.phase==='immediate'?[]:unique([...bible.voice.verbalTics,...bible.voice.petNames.map((item)=>`Pet name available sparingly: ${item}`)]).slice(0,5);
  return{
    ...
    perceptionLens:pick(lenses,`${seed}|lens`),conversationalMove:`${pick(moves,`${seed}|move`)} ${performance.behavior} Apply this through the supplied motivation and contradiction; never recite them.`,responseShape:shape,
    performance,
    ...
    adultContinuity:'Keep this exact personality and voice in mature romantic dialogue. Romantic intensity may change boldness, never identity, autonomy, relationship truth, or the non-sexual production ceiling.',
  };
}
...
function normalizeMoves(value:unknown):Partial<Record<CharacterResponseMode,string[]>>{const source=row(value);const result:Partial<Record<CharacterResponseMode,string[]>>={};for(const mode of ['danger','casual','playful','supportive','vulnerable','conflicted','repair','practical','storytelling','affectionate'] as CharacterResponseMode[]){const entries=strings(source[mode]).slice(0,5);if(entries.length)result[mode]=entries;}return result;}
