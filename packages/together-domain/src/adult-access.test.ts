...
describe('website adult access matrix',()=>{
  it('authorizes only a paid, eligible, server-authorized website request',()=>expect(adultPipelineAuthorized({...eligible,global_enabled:true})).toBe(true));
  it.each([
    ...
    ['website request not authorized by the server',{...eligible,adult_mode_enabled:false}],
