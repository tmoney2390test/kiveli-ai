...
export type DialogueGenerationProvider = 'openai' | 'xai' | 'gemini' | 'venice' | 'wavespeed';
...
export function resolveDialogueModelCapabilities(input: { provider: DialogueGenerationProvider; model: string }): DialogueModelCapabilities {
  ...
  if (input.provider === 'venice' || input.provider === 'wavespeed') return { supportedReasoningEfforts: ['none'], supportsTemperature: true, supportsTemperatureWithReasoning: false, maxOutputTokens: 8192 };
