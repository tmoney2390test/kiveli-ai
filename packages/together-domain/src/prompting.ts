...
export function compileIntimacyStance(input:{message:string;recentTurns?:Array<{role:string;content:string;providerMetadata?:Record<string,unknown>;provider_metadata?:Record<string,unknown>}>;relationship:RelationshipLike;personality?:Record<string,unknown>;interactionMode?:string;availability?:string;requestedMode?:DialogueContentMode}):IntimacyStance{
  ...
  const acceptedByStage=reasonCodes.includes('relationship_stage_default_reciprocity'),acceptedByLongTerm=reasonCodes.includes('long_term_default_reciprocity'),acceptedByUserInitiation=reasonCodes.includes('user_initiated_explicit_consent'),acceptedCncFantasy=reasonCodes.includes('consensual_nonconsent_fantasy');
  const willingReadiness=acceptedCncFantasy?'This is staged play between these two. Keep it between them. If they say stop, stop. Do not add extra people or real-world crime. Do not pause to ask permission or mention rules.':acceptedByUserInitiation?'This is already a yes this turn. Move the scene forward in character. Do not ask permission, do not mention rules, and do not pause unless they stop or pull back.':acceptedByLongTerm?'This pairing already has a strong default of willing sex. Go with the request unless they stop, refuse, or a real authored boundary is in the way.':acceptedByStage?`At the ${stage} stage this is already a yes. Advance one natural beat. Do not ask permission or mention rules.`:'Go with it. Treat the advance as wanted. Do not invent reluctance or a purity test.';
