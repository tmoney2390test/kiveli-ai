...
export interface BuildResponsesRequestBodyInput {model:string;prompt:string;maxOutputTokens:number;stream:boolean;promptCacheKey?:string;reasoningEffort?:EffectiveReasoningEffort;temperature?:number}
export function buildResponsesRequestBody(input:BuildResponsesRequestBodyInput){return{model:input.model,input:input.prompt,max_output_tokens:input.maxOutputTokens,reasoning:{effort:input.reasoningEffort??'none'},stream:input.stream,...(typeof input.temperature==='number'?{temperature:input.temperature}:{}),...(input.promptCacheKey?{prompt_cache_key:input.promptCacheKey}:{})};}
