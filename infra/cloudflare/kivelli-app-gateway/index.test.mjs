...
import worker, { appDocumentAssetPath, releaseFromHtml } from './index.js';
...
test('maps every client route to the single app document', () => {
  assert.equal(appDocumentAssetPath('/'), '/index.html');
  assert.equal(appDocumentAssetPath('/explore'), '/index.html');
  assert.equal(appDocumentAssetPath('/world/places'), '/index.html');
  assert.equal(appDocumentAssetPath('/settings/'), '/index.html');
  assert.equal(appDocumentAssetPath('/location/juniper-civic-arena'), '/index.html');
  assert.equal(appDocumentAssetPath('/media/2346aee9-3dd3-4e9c-99b7-7feacfc859cc'), '/index.html');
  assert.equal(appDocumentAssetPath('/favicon.ico'), null);
  assert.equal(appDocumentAssetPath('/_expo/static/js/web/entry-0123456789abcdef.js'), null);
});

test('serves the single app document internally while retaining the public route', async () => {
  ...
  assert.equal(new URL(assetRequestUrl).pathname, '/index.html');
  assert.equal(new URL(assetRequestUrl).search, '?world=juniper-city');
  assert.equal(response.headers.get('x-kivelli-app-document'), '/index.html');
