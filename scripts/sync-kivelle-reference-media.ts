...
async function main(){
  ...
  const context=await loadContext(db),summary={discovered:assets.length,unchanged:0,uploaded:0,reusedStorage:0,storageUploads:0,missingCanonicalEntity:0,missingSource:0};
  for(const asset of assets){
    const scope=resolveScope(asset,context);
    if(!scope){summary.missingCanonicalEntity+=1;console.warn(`skip missing canonical entity: ${asset.sourceKey}`);continue;}
    let bytes:Buffer;
    try{bytes=await readFile(asset.path);}catch(error){if((error as NodeJS.ErrnoException).code==='ENOENT'){summary.missingSource+=1;console.warn(`skip missing source image: ${asset.sourceKey}`);continue;}throw error;}
    const sha256=createHash('sha256').update(bytes).digest('hex'),extension=extname(asset.path).slice(1).toLowerCase(),contentType=extension==='png'?'image/png':extension==='webp'?'image/webp':'image/jpeg';
    const{data:existing,error}=await db.from('together_media_reference_assets').select('*').eq('source_key',asset.sourceKey).eq('asset_role',asset.role).order('revision',{ascending:false}).limit(1).maybeSingle();
    ...
    const{error:insertError}=await db.from('together_media_reference_assets').insert({asset_role:asset.role,source_key:asset.sourceKey,storage_bucket:storageBucket,storage_path:storagePath,content_type:contentType,byte_size:bytes.byteLength,sha256,revision,active:true,...dimensions,...scope,metadata:{syncedBy:'sync-kivelle-reference-media',sourcePath:asset.path.replace(root,'').replaceAll('\\','/'),storageReused:Boolean(reusable)}});
...
async function discoverAssets():Promise<Asset[]>{
  ...
  for(const worldSlug of['juniper-city','port-vervelle','neon-kyo']){
    ...
    for(const name of files.filter((item)=>/\.(jpe?g|png|webp)$/i.test(item))){
      ...
      if(slug===worldSlug){output.push({sourceKey:`world:${worldSlug}:canonical`,role:'world_canonical',path:join(directory,name),worldSlug});continue;}
      if(files.some((candidate)=>parse(candidate).name===slug&&/\.jpe?g$/i.test(candidate))&&!/\.jpe?g$/i.test(name))continue;
      output.push({sourceKey:`location:${worldSlug}:${slug}:canonical`,role:'location_canonical',path:join(directory,name),worldSlug,locationSlug:slug});
    }
  }
