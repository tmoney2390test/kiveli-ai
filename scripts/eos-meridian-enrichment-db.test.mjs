...
test('additive migration preserves adult material, custom introductions and user outcomes; rerun is idempotent',async()=>{
 ...
 try{
  await db.exec(`
   ...
   create table together_schedule_templates(character_version_id uuid references together_character_versions(id),day_of_week int,start_minute int,end_minute int,location_id uuid references together_locations(id),activity text,availability text,energy_delta int,mood_influence text,variation_weight numeric,metadata jsonb,week_index smallint generated always as (coalesce((metadata->>'weekIndex')::smallint,0)) stored,unique(character_version_id,day_of_week,start_minute,week_index));
