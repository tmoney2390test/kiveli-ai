-- Store transaction ownership and refund tombstones survive webhook retries.
create table public.together_billing_sandbox_testers (
  user_id uuid primary key references auth.users(id) on delete cascade,
  created_at timestamptz not null default now()
);
alter table public.together_billing_sandbox_testers enable row level security;
revoke all on public.together_billing_sandbox_testers from public,anon,authenticated;
grant all on public.together_billing_sandbox_testers to service_role;

create table public.together_store_credit_purchases (
  store text not null check (store in ('APP_STORE','PLAY_STORE')),
  environment text not null check (environment in ('SANDBOX','PRODUCTION')),
  transaction_id text not null,
  user_id uuid references auth.users(id) on delete set null,
  product_id text not null,
  credits integer not null check (credits in (100,300,800,2000)),
  status text not null check (status in ('pending','granted','refunded')),
  unrecovered_credits integer not null default 0,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  primary key (store,environment,transaction_id)
);
alter table public.together_store_credit_purchases enable row level security;
revoke all on public.together_store_credit_purchases from public,anon,authenticated;
grant all on public.together_store_credit_purchases to service_role;
create index on public.together_store_credit_purchases(user_id,created_at desc);

create function public.kivelle_apply_store_credit_purchase(
...
begin
  if p_store not in ('APP_STORE','PLAY_STORE') or p_environment not in ('SANDBOX','PRODUCTION')
    or length(p_transaction_id) not between 1 and 512 or p_credits not in (100,300,800,2000)
  ...
  if purchase.user_id is distinct from p_user_id or purchase.product_id<>p_product_id or purchase.credits<>p_credits then
