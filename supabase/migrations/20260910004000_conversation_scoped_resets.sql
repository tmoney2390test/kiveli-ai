-- Replace only the selected transcript; preserve the companion and relationship.
create table public.together_conversation_reset_receipts (
 user_id uuid not null references auth.users(id) on delete cascade,
 request_id uuid not null,continuity_id uuid not null references public.together_continuities(id) on delete cascade,
 conversation_id uuid not null,mode text not null check(mode in('conversation','scenario')),
 result jsonb not null,created_at timestamptz not null default now(),primary key(user_id,request_id)
);
alter table public.together_conversation_reset_receipts enable row level security;
revoke all on public.together_conversation_reset_receipts from public,anon,authenticated;
grant all on public.together_conversation_reset_receipts to service_role;
create function public.kivelli_reset_chat(p_user uuid,p_continuity uuid,p_conversation uuid,p_mode text,p_request uuid,p_scenario_session uuid default null,p_opening text default null)
returns jsonb language plpgsql security invoker set search_path=public,pg_temp as $$
declare old_chat public.together_conversations%rowtype; fresh public.together_conversations%rowtype; receipt public.together_conversation_reset_receipts%rowtype; target public.together_scenario_sessions%rowtype; saved record; paths text[]; result jsonb;
begin
