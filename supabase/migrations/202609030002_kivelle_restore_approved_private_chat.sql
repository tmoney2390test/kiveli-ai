...
-- The first website adult-chat pipeline stored approved private dialogue as
-- web_adult. The later private-text policy deliberately failed those legacy
-- rows closed because they did not yet carry its versioned metadata. Promote
-- only server-approved dialogue with the known server-authored bridge, an
-- adult account, and an all-adult private conversation. Media stays web-only.
with eligible_messages as (
  ...
  where message.role in('user','assistant')
    ...
    and message.moderation_version in('web-adult-v1','private-adult-text-v1')
...
-- Keep derived private text available to the same authorized context path.
...
where memory.source_message_id=message.id
  ...
  and message.provider_metadata->>'legacyPolicyUpgraded'='true';
