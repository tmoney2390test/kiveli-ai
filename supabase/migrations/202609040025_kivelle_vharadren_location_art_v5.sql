-- Complete the visually reviewed Verdant Reach location-art set.
begin;

create temporary table vharadren_location_art_v5(slug text primary key) on commit drop;
insert into vharadren_location_art_v5(slug) values
  ('briar-and-cup'),
  ('green-hart-lodge'),
  ('hartwood-hall'),
  ('hollow-crown-grove'),
  ('motherleaf-sanctuary'),
  ('rootmarket'),
  ('weeping-archive');
...
set
  metadata=coalesce(location.metadata,'{}'::jsonb)||jsonb_build_object(
    'assetStatus','ready',
    'photoStatus','ready',
    'locationArtPack','vharadren_location_art_v5',
    'locationReferenceSourceKey','location:vharadren:'||location.slug||':canonical'
  ),
  updated_at=now()
from vharadren_location_art_v5 artwork
...
begin
  ...
  join vharadren_location_art_v5 artwork on artwork.slug=location.slug
  where location.world_id='10000000-0000-4000-8000-000000000013'::uuid;
  ...
  if location_count<>7 or batch_asset_count<>7 or total_asset_count<>36 or ready_count<>36 then
    raise exception 'Vharadren location art v5 integration failed: locations %, batch assets %, total assets %, ready %',location_count,batch_asset_count,total_asset_count,ready_count;
  end if;
end $$;
