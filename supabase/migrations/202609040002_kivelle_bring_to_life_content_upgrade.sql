...
-- Bring-to-life may adopt a separately moderated prompt level. The resulting
-- row always keeps the higher of the source-photo and prompt policies, and an
-- adult-capable endpoint still requires a live authorized web-adult session.
create or replace function public.kivelle_reserve_video_generation_v7(
  ...
  p_aspect_ratio text,p_user_prompt text,p_content_level text,p_anonymous_adult_partner boolean,p_testing_selection boolean,p_route_concurrency_limit integer,
...
begin
  ...
  if p_content_level not in('standard','romance','suggestive','mature','explicit') or (coalesce(p_anonymous_adult_partner,false) and p_content_level not in('suggestive','mature','explicit')) then raise exception using errcode='22023',message='INVALID_VIDEO_CONTENT_POLICY';end if;
  ...
  if source_media.content_level not in('standard','romance','suggestive','mature','explicit') then raise exception using errcode='P0001',message='VIDEO_CONTENT_LEVEL_BLOCKED';end if;

  restricted_source:=source_media.content_level in('suggestive','mature','explicit') or source_media.content_rating='explicit' or source_media.visibility_scope='web_adult';
  source_level:=case when restricted_source and source_media.content_level in('standard','romance') then 'explicit' else source_media.content_level end;
  if array_position(array['standard','romance','suggestive','mature','explicit'],p_content_level)<array_position(array['standard','romance','suggestive','mature','explicit'],source_level) then raise exception using errcode='22023',message='INVALID_VIDEO_CONTENT_POLICY';end if;

  restricted_output:=p_content_level in('suggestive','mature','explicit') or restricted_source;
  adult_route:=p_route_id like '%-spicy';
  if restricted_output and not adult_route then raise exception using errcode='22023',message='INVALID_VIDEO_ROUTE_CONFIGURATION';end if;
  ...
  elsif coalesce(p_adult_authorized,false) or p_adult_web_session_id is not null or restricted_output then
  ...
  adult_metadata:=case when adult_route then jsonb_build_object('adultAuthorized',true,'adultWebSessionId',p_adult_web_session_id,'moderationVersion','web-adult-video-v1','adultCapableRoute',true) else '{}'::jsonb end
    ||case when coalesce(p_anonymous_adult_partner,false) or coalesce((source_media.metadata->>'anonymousAdultPartner')='true',false) then jsonb_build_object('anonymousAdultPartner',true,'expectedAdultSubjectCount',2) else '{}'::jsonb end;
  ...
  insert into public.together_generated_media(
    ...
    'video',p_content_level,case when adult_route or restricted_output then 'explicit' when p_content_level='romance' then 'suggestive' else 'safe' end,case when adult_route then 'web_adult' else 'all' end,case when adult_route then 'web-adult-video-v1' else coalesce(source_media.moderation_version,'video-source-v1') end,'queued',p_request_key,source_media.queue_priority,p_route_id,p_motion_preset,p_duration_seconds,p_resolution,p_audio_behavior,p_sound_requested,p_provider_audio_mode,p_requested_model,p_resolved_model,p_aspect_ratio,p_quote_usd,p_baseline_usd,p_testing_selection,'source_photo',trim(p_user_prompt),
    jsonb_build_object('source','user_request','videoSourceMode','source_photo','parentMediaId',source_media.id,'sourceContentLevel',source_level,'requestKey',p_request_key,'creditTransactionId',transaction_id,'creditCost',p_credit_cost,'creditAction','short_video','creditRefunded',false,'routeConcurrencyLimit',p_route_concurrency_limit,'generationIntent',jsonb_build_object('requestText',trim(p_user_prompt),'requestedContentLevel',p_content_level),'videoRequest',jsonb_build_object('routeId',p_route_id,'requestedModel',p_requested_model,'resolvedModel',p_resolved_model,'motionPreset',p_motion_preset,'durationSeconds',p_duration_seconds,'resolution',p_resolution,'soundRequested',p_sound_requested,'providerAudioMode',p_provider_audio_mode,'quotedCostUsd',p_quote_usd,'baselineCostUsd',p_baseline_usd,'aspectRatio',p_aspect_ratio,'testingSelection',p_testing_selection))||adult_metadata
  );
...
revoke all on function public.kivelle_reserve_video_generation_v7(uuid,uuid,uuid,text,text,text,text,text,text,text,integer,numeric,numeric,integer,text,text,boolean,text,text,text,text,boolean,boolean,integer,boolean,uuid) from public,anon,authenticated;
grant execute on function public.kivelle_reserve_video_generation_v7(uuid,uuid,uuid,text,text,text,text,text,text,text,integer,numeric,numeric,integer,text,text,boolean,text,text,text,text,boolean,boolean,integer,boolean,uuid) to service_role;
