-- Publish the fourth accepted Vharadren canonical portrait batch.
begin;

create temporary table vharadren_portrait_characters_v4(slug text primary key) on commit drop;
insert into vharadren_portrait_characters_v4(slug) values
  ('captain-seraphine-vale'),
  ('duchess-aveline-edevane'),
  ('duchess-mirelle-greymere'),
  ('freya-hart'),
  ('lord-cassian-greymere'),
  ('lord-rowan-edevane'),
  ('lyssa-bramble'),
  ('thalia-moss');

create temporary table vharadren_portrait_assets_v4 on commit drop as
select
  template.slug,
...
from vharadren_portrait_characters_v4 expected
...
set
  ...
  visual_identity=jsonb_set(
    coalesce(version.visual_identity,'{}'::jsonb)||jsonb_build_object(
      ...
      'portraitPack','vharadren_primary_portraits_v4',
  ...
  updated_at=now()
from vharadren_portrait_assets_v4 portrait
...
declare
  ...
  ready_count integer;
begin
  select count(*),count(primary_path) into linked_count,primary_count from vharadren_portrait_assets_v4;
  select count(*) into ready_count
  from public.together_character_versions version
  join vharadren_portrait_assets_v4 portrait on portrait.character_version_id=version.id
  ...
  if linked_count<>8 or primary_count<>8 or ready_count<>8 then
    raise exception 'Vharadren portrait v4 integration failed: linked %, primaries %, ready %',linked_count,primary_count,ready_count;
  end if;
end $$;
