-- Publish the final Vharadren canonical primary portrait batch.
begin;

create temporary table vharadren_portrait_characters_v6(slug text primary key) on commit drop;
insert into vharadren_portrait_characters_v6(slug) values
  ('catrin-brann'),
  ('liora-saintless'),
  ('maeve-redreed'),
  ('malrec-vale'),
...
create temporary table vharadren_portrait_assets_v6 on commit drop as
select
  template.slug,
...
from vharadren_portrait_characters_v6 expected
...
set
  ...
  visual_identity=jsonb_set(
    coalesce(version.visual_identity,'{}'::jsonb)||jsonb_build_object(
      ...
      'portraitPack','vharadren_primary_portraits_v6',
  ...
  updated_at=now()
from vharadren_portrait_assets_v6 portrait
...
begin
  select count(*),count(primary_path) into linked_count,primary_count from vharadren_portrait_assets_v6;
  select count(*) into ready_count
  from public.together_character_versions version
  join vharadren_portrait_assets_v6 portrait on portrait.character_version_id=version.id
  ...
  if linked_count<>9 or primary_count<>9 or ready_count<>9 or world_primary_count<>49 then
    raise exception 'Vharadren portrait v6 integration failed: linked %, primaries %, ready %, world primaries %',linked_count,primary_count,ready_count,world_primary_count;
  end if;
end $$;
