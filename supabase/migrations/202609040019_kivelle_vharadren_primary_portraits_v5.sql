-- Publish the fifth accepted Vharadren canonical portrait batch.
begin;

create temporary table vharadren_portrait_characters_v5(slug text primary key) on commit drop;
insert into vharadren_portrait_characters_v5(slug) values
  ('asha-ren'),
  ('bastian-crow'),
  ('delphine-lantern'),
  ('idris-salt'),
...
create temporary table vharadren_portrait_assets_v5 on commit drop as
select
  template.slug,
...
from vharadren_portrait_characters_v5 expected
...
set
  ...
  visual_identity=jsonb_set(
    coalesce(version.visual_identity,'{}'::jsonb)||jsonb_build_object(
      ...
      'portraitPack','vharadren_primary_portraits_v5',
  ...
  updated_at=now()
from vharadren_portrait_assets_v5 portrait
...
declare
  ...
  ready_count integer;
begin
  select count(*),count(primary_path) into linked_count,primary_count from vharadren_portrait_assets_v5;
  select count(*) into ready_count
  from public.together_character_versions version
  join vharadren_portrait_assets_v5 portrait on portrait.character_version_id=version.id
  ...
  if linked_count<>8 or primary_count<>8 or ready_count<>8 then
    raise exception 'Vharadren portrait v5 integration failed: linked %, primaries %, ready %',linked_count,primary_count,ready_count;
  end if;
end $$;
