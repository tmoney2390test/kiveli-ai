-- Publish the third accepted Vharadren canonical portrait batch.
begin;

create temporary table vharadren_portrait_characters_v3(slug text primary key) on commit drop;
insert into vharadren_portrait_characters_v3(slug) values
  ('brother-aldren'),
  ('celessa-vane'),
  ('ilyra-ashscale'),
  ('joren-ash'),
  ...
  ('mara-sable'),
  ('nerys-rowanleaf'),
  ('prince-aurel-ravaryn');

create temporary table vharadren_portrait_assets_v3 on commit drop as
select
  template.slug,
...
from vharadren_portrait_characters_v3 expected
...
set
  ...
  visual_identity=jsonb_set(
    coalesce(version.visual_identity,'{}'::jsonb)||jsonb_build_object(
      ...
      'portraitPack','vharadren_primary_portraits_v3',
  ...
  updated_at=now()
from vharadren_portrait_assets_v3 portrait
...
declare
  ...
  ready_count integer;
begin
  select count(*),count(primary_path) into linked_count,primary_count from vharadren_portrait_assets_v3;
  select count(*) into ready_count
  from public.together_character_versions version
  join vharadren_portrait_assets_v3 portrait on portrait.character_version_id=version.id
  ...
  if linked_count<>8 or primary_count<>8 or ready_count<>8 then
    raise exception 'Vharadren portrait v3 integration failed: linked %, primaries %, ready %',linked_count,primary_count,ready_count;
  end if;
end $$;
