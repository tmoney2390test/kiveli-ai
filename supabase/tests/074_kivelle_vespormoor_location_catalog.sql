...
select is((select count(*)::integer from public.together_locations where world_id='10000000-0000-4000-8000-000000000010' and nullif(canonical_visual_context->>'canonicalPrompt','') is not null and metadata->>'photoStatus'='slot_ready'),51,'Every Vespormoor location has prompt-ready individual art grounding');
