...
select like(pg_get_functiondef('public.kivelle_acquire_provider_slot(text,text,uuid,integer,integer)'::regprocedure),'%pg_advisory_xact_lock%','Provider admission is serialized per provider');
...
select like(pg_get_functiondef('public.kivelle_begin_dialogue_turn(uuid,uuid,uuid,text,text,boolean,integer)'::regprocedure),'%for update%','Turn acquisition serializes on the conversation row');
select like(pg_get_functiondef('public.kivelle_begin_dialogue_turn(uuid,uuid,uuid,text,text,boolean,integer)'::regprocedure),'%active_turn.state%planning%','An in-progress setup cannot be superseded');
...
select like(lower(pg_get_functiondef('public.kivelle_claim_media_jobs_v2(integer,integer)'::regprocedure)),'%row_number() over (partition by media.user_id%','Media claiming distributes a batch across users');
select like(pg_get_functiondef('public.kivelle_claim_media_jobs_v2(integer,integer)'::regprocedure),'%kivelle-media-global-claim%','Global media admission is serialized');
select like(pg_get_functiondef('public.kivelle_claim_media_provider_poll_jobs(text,integer,integer)'::regprocedure),'%for update skip locked%','Concurrent dispatchers cannot poll the same job');
select like(pg_get_functiondef('public.kivelle_claim_media_finalization(uuid,integer)'::regprocedure),'%finalization_lease_expires_at%','Webhook and polling finalization share one lease');
