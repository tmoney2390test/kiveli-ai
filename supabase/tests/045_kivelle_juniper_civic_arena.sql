...
select is((select parent.slug from public.together_locations location join public.together_locations parent on parent.id=location.parent_location_id where location.slug='juniper-civic-arena'),'alder-district','The arena has canonical Alder District ancestry');
