...
select like(pg_get_constraintdef(oid),'%truth_mode%canonical%disputed%rumor%secret%','truth mode is constrained') from pg_constraint where conname='together_world_facts_truth_mode_check';
select like(pg_get_constraintdef(oid),'%required_participant_count%maximum_participant_count%','scene participant bounds are constrained') from pg_constraint where conname='together_scene_interaction_beats_participant_bounds';
