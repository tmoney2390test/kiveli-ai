...
select ok(regexp_replace(lower(pg_get_functiondef('public.kivelle_claim_media_jobs(integer)'::regprocedure)),'\s+',' ','g') like '%order by queue_priority desc, created_at%','Media dispatcher claims higher-tier jobs before FIFO within a tier');
select * from finish();
rollback;
