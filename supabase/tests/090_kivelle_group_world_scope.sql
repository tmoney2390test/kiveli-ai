...
select like(pg_get_functiondef('public.kivelle_character_resident_world(uuid)'::regprocedure),'%presence.presence_type = ''resident''%','temporary visitors cannot define group scope');
select like(pg_get_functiondef('public.kivelle_character_resident_world(uuid)'::regprocedure),'%count(DISTINCT presence.world_id) = 1%','ambiguous resident worlds are rejected');
select like(pg_get_functiondef('public.kivelle_validate_group_participant()'::regprocedure),'%v_resident_world <> v_conversation.group_world_id%','participant resident world must match the group');
select like(pg_get_functiondef('public.kivelle_validate_group_conversation_world()'::regprocedure),'%group world must match every active participant%','group world cannot be changed across its roster');
