...
select is((select count(*)::integer from public.together_worlds where published),6,'the published Kivelle catalog contains exactly six worlds');
select is((select array_agg(slug order by slug) from public.together_worlds where published),array['eos-meridian','juniper-city','neon-kyo','northvale','port-vervelle','vespormoor']::text[],'the published catalog includes all six canonical worlds');
