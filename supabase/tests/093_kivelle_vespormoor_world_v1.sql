...
select is((select count(*)::integer from public.together_character_templates where id::text like '22000000-0000-4000-8010-%' and published and can_be_selected),45,'All 45 residents are selectable');
select is((select count(*)::integer from public.together_character_versions where id::text like '23000000-0000-4000-8010-%' and published_at is not null),45,'All residents have a published version');
select is((select count(*)::integer from public.together_character_world_presence where world_id='10000000-0000-4000-8000-000000000010' and presence_type='resident' and character_version_id::text like '23000000-0000-4000-8010-%'),45,'All residents have world presence');
...
select is((select count(*)::integer from public.together_character_voice_profiles where character_template_id::text like '22000000-0000-4000-8010-%' and active),45,'Every companion has a stable voice profile');
select is((select count(*)::integer from public.together_character_homes where character_version_id::text like '23000000-0000-4000-8010-%' and active),45,'Every companion has a private virtual home');
...
) complete_days),315,'Every companion has at least five authored blocks on all seven days');
...
select is((select count(*)::integer from public.together_story_arc_templates where specific_world_id='10000000-0000-4000-8000-000000000010' and active),7,'Seven dialogue-driven mystery arcs are active');
