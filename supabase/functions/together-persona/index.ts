import { createPersonaOnce } from '../_shared/persona-create.ts';
import { z } from 'zod';
...
const schema=z.discriminatedUnion('action',[
  z.object({action:z.literal('create'),requestId:z.string().uuid().optional(),...personaFields}),
...
serve(async(request,correlationId)=>{
  ...
  if(input.action==='create'){
    ...
    const {data,created}=await createPersonaOnce(db,user.id,input.requestId,{name:input.displayName,display_name:input.displayName,pronouns:input.pronouns??null,age:input.age??null,occupation:input.occupation??null,biography:input.biography??null,interests:input.interests,appearance_config:cleanAppearance(input.appearanceConfig),communication_config:input.communicationConfig,metadata:input.metadata,is_default:false});
    if(created)await track(db,user.id,'persona_created',{persona_id:data.id});
    return json({data,correlationId},created?201:200,correlationId);
  }
  ...
  if(input.action==='start_life'){
    ...
    const{data:existingLife}=await db.from('together_continuities').select('id,active_companion_instance_id').eq('user_id',user.id).eq('persona_id',persona.id).maybeSingle();if(existingLife){await activateLife(db,user.id,existingLife.id,existingLife.active_companion_instance_id??null,now);return json({data:await buildSnapshot(db,user.id),correlationId},200,correlationId);}
    const subscription=await resolveSubscriptionState(db,user.id);await enforceLifeLimit(db,user.id,subscription.capabilities);
    const{data,error}=await db.from('together_continuities').insert({user_id:user.id,persona_id:persona.id,kind:'alternate',title:input.title??`${persona.display_name}'s Life`,metadata:{createdFrom:'persona_picker',contextVersion:1}}).select('*,together_user_personas(*)').single();if(error||!data){if(error?.code==='23505'){const{data:raced}=await db.from('together_continuities').select('id,active_companion_instance_id').eq('user_id',user.id).eq('persona_id',persona.id).maybeSingle();if(raced){await activateLife(db,user.id,raced.id,raced.active_companion_instance_id??null,now);return json({data:await buildSnapshot(db,user.id),correlationId},200,correlationId);}}throw new AppError('INTERNAL_ERROR','That Alternate Life could not be started.',500,true);}await activateLife(db,user.id,data.id,null,now);await track(db,user.id,'alternate_life_created',{continuity_id:data.id,persona_id:persona.id,tier:subscription.tier});return json({data:await buildSnapshot(db,user.id),correlationId},201,correlationId);
  }
  if(input.action==='switch_life'){
    const continuity=await continuityById(db,user.id,input.continuityId);if(!continuity)throw new AppError('NOT_FOUND','That Kivelle Life is unavailable.',404);await activateLife(db,user.id,continuity.id,continuity.active_companion_instance_id??null,now);await track(db,user.id,'continuity_switched',{continuity_id:continuity.id,kind:continuity.kind});return json({data:await buildSnapshot(db,user.id),correlationId},200,correlationId);
  }
...
async function activateLife(db:SupabaseClient,userId:string,continuityId:string|null,characterId:string|null,now:string){
  const {data,error}=await db.from('together_profiles').update({active_continuity_id:continuityId,active_companion_instance_id:characterId,updated_at:now}).eq('user_id',userId).select('user_id').single();
  if(error||!data)throw new AppError('INTERNAL_ERROR','Your Life could not be switched. Please try again.',500,true);
}
