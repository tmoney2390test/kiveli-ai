...
serve(async(request,correlationId)=>{
  ...
  if(input.action==='update'){
    ...
    const changes:Record<string,unknown>={updated_at:now};if(input.displayName!==undefined){changes.display_name=input.displayName;changes.name=input.displayName;}if(input.pronouns!==undefined)changes.pronouns=input.pronouns;if(input.age!==undefined)changes.age=input.age;if(input.occupation!==undefined)changes.occupation=input.occupation;if(input.biography!==undefined)changes.biography=input.biography;if(input.interests!==undefined)changes.interests=input.interests;if(input.communicationConfig!==undefined)changes.communication_config=input.communicationConfig;if(input.appearanceConfig!==undefined){validatePersonaAvatarPath(user.id,input.appearanceConfig.avatarPath);changes.appearance_config=cleanAppearance(input.appearanceConfig);}if(input.metadata!==undefined)changes.metadata=input.metadata;
    const{data,error}=await db.from('together_user_personas').update(changes).eq('id',input.personaId).eq('user_id',user.id).select('*').single();if(error||!data)throw new AppError('NOT_FOUND','That Persona could not be updated.',404);const reconciliation=await reconcilePersonaIdentity({db,userId:user.id,personaId:data.id,before,after:data,now});await track(db,user.id,'persona_updated',{persona_id:data.id,changed_fields:reconciliation.changedFields,memories_superseded:reconciliation.memoriesSuperseded,reflections_invalidated:reconciliation.reflectionsInvalidated});return json({data,correlationId},200,correlationId);
  }
...
function validatePersonaAvatarPath(userId:string,path?:string|null){if(path&&!path.startsWith(`${userId}/persona-avatars/`)&&!path.startsWith(`${userId}/avatar-`))throw new AppError('VALIDATION_ERROR','That Persona photo path is invalid.',400);}
