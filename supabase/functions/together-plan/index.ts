...
serve(async(request,correlationId)=>{
  ...
  if(input.action==='create'){
    ...
    if(resolvedStartsAt){
      ...
      if(result.kind==='shared_plan'){
        ...
        const {data:scenarioHold,error:holdError}=await db.from('together_scenario_sessions').select('id').eq('user_id',user.id).eq('continuity_id',continuity.id).in('character_instance_id',[input.characterInstanceId,...(result.commitment.participant_instance_ids??[])]).eq('status','active').limit(1).maybeSingle();
        if(holdError)throw new AppError('INTERNAL_ERROR','Your plan was saved, but its scenario could not be checked.',500,true);
  ...
  const {data:scenarioHold,error:holdError}=await db.from('together_scenario_sessions').select('id').eq('user_id',user.id).eq('continuity_id',continuity.id).in('character_instance_id',[String(candidate.character_instance_id),...((result as {commitment?:{participant_instance_ids?:string[]}})?.commitment?.participant_instance_ids??[])]).eq('status','active').limit(1).maybeSingle();
  if(holdError)throw new AppError('INTERNAL_ERROR','The scenario could not be checked.',500,true);
  if(input.timingChoice==='now'&&!scenarioHold&&(result as Record<string,unknown>)?.kind==='shared_plan'){
