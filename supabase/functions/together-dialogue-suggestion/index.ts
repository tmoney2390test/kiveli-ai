import {activeScenarioContext} from '../_shared/kivelle-scenarios.ts';
import { z } from 'zod';
...
Deno.serve(async(request)=>{
  ...
  try{
    ...
    const instance=conversation.together_character_instances as Record<string,any>,storedRequestedMode=requestedConversationDialogueContentMode(profile,conversation),dialoguePolicy=resolvePrivateDialoguePolicy({access:adultAccess,requestedMode:storedRequestedMode,conversationMode:'direct',participants:[instance],safetyAllowed:true}),authorizedPrivateAdultText=dialoguePolicy.rollout.generationAllowed;
    let latestQuery=db.from('together_messages').select('id,role,content,created_at,conversation_sequence,provider_metadata,content_rating,visibility_scope').eq('conversation_id',input.conversationId).eq('user_id',user.id);
    ...
    const [context,activeScenario]=await Promise.all([buildKivelleConversationContext({db,userId:user.id,instance,conversation,userMessage:String(latest.content),lifeRun,now,authorizedPrivateAdultText}),activeScenarioContext(db,user.id,input.conversationId,input.characterInstanceId)]);
    ...
    const latestContentRating=messageContentRating(latest),spicyAnchor=authorizedPrivateAdultText&&dialoguePolicy.effectiveMode==='explicit'&&latestContentRating==='explicit';
    const generationInput:AutoDialogueInput={
      ...
      contentMode:dialoguePolicy.effectiveMode,latestContentRating,explicitContinuationAllowed:spicyAnchor,
      ...
      relationshipStage:String(context.relationship?.relationship_stage??instance.relationship_stage??'stranger'),intimacyOutcome:normalizeIntimacyOutcome(latest.provider_metadata?.intimacyOutcome),
      ...
      userName:personaName(context.persona),openThread:context.openThreads.find((thread)=>thread.eligible)?.displaySubject,upcomingCommitment:context.upcomingCommitments[0]?.title,activePlan:context.currentScene.activePlan?.title,activeDate:context.currentScene.activeDate?.title,activeStory:activeScenario?.title??(typeof context.activeStory?.title==='string'?context.activeStory.title:undefined),conversationFocus:activeScenario?`Selected starting premise (later conversation choices take precedence): ${activeScenario.setup}`:(typeof context.conversationFocus?.title==='string'?context.conversationFocus.title:undefined),emotionalTone:context.emotionalResidue?.tone,voiceHints:voiceHints(context.persona,context.userPatterns),
    };
...
function messageContentRating(message:Record<string,any>):AutoDialogueInput['latestContentRating']{const metadata=relationRecord(message.provider_metadata),value=message.content_rating??metadata.contentRating;return value==='safe'||value==='suggestive'||value==='explicit'?value:'unknown';}
