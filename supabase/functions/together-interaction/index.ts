...
async function completeAcceptedInteraction(input:any){
  ...
  const evidenceRecorded=await maybeRecordEvidence(db,userId,input.characterInstanceId,action.id,candidate,decision.relationshipEvidence,input.place.world.timezone,now);
