...
serve(async (request, correlationId) => {
  ...
  const result = { label: candidate.label, nextState, effects: candidate.effects, evidenceRecorded, reactionContext: { interactionKey: candidate.interactionKey, label: candidate.label, location: context.place.path } };
...
async function loadContext(input: { db: any; userId: string; continuityId: string; characterInstanceId: string; conversationId?: string; now: Date }) {
  ...
  const { data: currentScene } = await input.db.from('together_scene_sessions').select('*').eq('user_id', input.userId).eq('continuity_id', input.continuityId).eq('character_instance_id', input.characterInstanceId).is('ended_at', null).order('started_at', { ascending: false }).limit(1).maybeSingle();
  ...
  if (access === 'locked') throw new AppError('WORLD_LOCKED', 'That world is not available for this life yet.', 403);
  const [locationResult, nearbyResult, worldResult, presence] = await Promise.all([
  ...
  return { ...input, instance, conversation, activeScene: active.scene, sceneSession: currentScene as Row | null, place, location, nearby, world: { activityFamilies: worldResult.data?.activity_families ?? [], metadata: worldResult.data?.metadata ?? {} }, presence, character, relationship, profile: deriveCharacterInteractionProfile(character) };
}
...
function resolveCandidateSet(context: any) {
  ...
  const input = { character: context.character, interactionProfile: context.profile, relationship: context.relationship, world: context.world, location: context.location, scene, life, nearbyLocations: context.nearby, seed: `${context.characterInstanceId}:${context.location.id}:${context.sceneSession?.id ?? 'entered'}` };
...
async function syncConversationScene(db: any, userId: string, conversation: Row, scene: Row, now: Date) {
  ...
  const next = {
    version: 1, characterInstanceId: scene.character_instance_id, locationId: scene.location_id, worldId: scene.world_id,
  ...
  await db.from('together_conversations').update({ metadata: mergeConversationSceneMetadata(conversation.metadata ?? {}, next), updated_at: now.toISOString() }).eq('id', conversation.id).eq('user_id', userId);
}
