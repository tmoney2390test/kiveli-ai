...
async function syncStripeSubscription(db:Db,userId:string,snapshot:StripeSubscriptionSnapshot,event:StripeEvent):Promise<boolean>{
  ...
  const accessEndsAt=stripeSubscriptionHasAccess(snapshot.status,snapshot.periodEnd)?snapshot.periodEnd:null,record={user_id:userId,provider:'stripe',provider_customer_id:snapshot.customerId,provider_subscription_id:snapshot.id,provider_product_id:snapshot.productId,provider_price_id:snapshot.priceId,plan_key:mapping.tier,status:snapshot.status,billing_interval:mapping.billingInterval,current_period_start:snapshot.periodStart,current_period_end:snapshot.periodEnd,trial_end:snapshot.trialEnd,cancel_at_period_end:snapshot.cancelAtPeriodEnd,canceled_at:snapshot.canceledAt,access_ends_at:accessEndsAt,last_provider_event_created_at:event.created??null,metadata:{lastStripeEventId:event.id,apiObject:'subscription'},updated_at:new Date().toISOString()};
  const{error}=await db.from('together_billing_subscriptions').upsert(record,{onConflict:'provider,provider_subscription_id'});if(error)throw new AppError('INTERNAL_ERROR','Stripe subscription could not be synchronized.',500,true);
