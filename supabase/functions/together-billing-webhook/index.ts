...
async function applyCheckoutCompleted(db:Db,input:{event:StripeEvent;userId:string;customerId:string|null;subscriptionId:string|null}):Promise<boolean>{
  ...
  if(kind==='subscription'){
    ...
    await syncStripeEntitlement(db,{userId:input.userId,tier,status:'active',customerId:input.customerId,subscriptionId:input.subscriptionId,productKey:null,periodStart:null,periodEnd:null,eventId:input.event.id});return true;
  }
  return false;
}
...
async function applySubscriptionEvent(db:Db,input:{event:StripeEvent;userId:string;customerId:string|null;subscriptionId:string|null}):Promise<boolean>{
  const object=input.event.data.object,metadata=(object.metadata??{}) as Record<string,unknown>,priceId=typeof object.items?.data?.[0]?.price?.id==='string'?object.items.data[0].price.id:null;
  ...
  await syncStripeEntitlement(db,{userId:input.userId,tier:active?tier:'free',status:String(object.status??(active?'active':'canceled')),customerId:input.customerId,subscriptionId:input.subscriptionId,productKey:priceId,periodStart:period.start,periodEnd:period.end,eventId:input.event.id});
...
async function syncStripeEntitlement(db:Db,input:{userId:string;tier:SubscriptionTier;status:string;customerId:string|null;subscriptionId:string|null;productKey:string|null;periodStart:string|null;periodEnd:string|null;eventId:string}):Promise<void>{
...
function tierForPrice(priceId:string|null):Exclude<SubscriptionTier,'free'>|null{if(!priceId)return null;if(priceId===stripePriceForTier('kivelle_plus'))return'kivelle_plus';if(priceId===stripePriceForTier('kivelle_max'))return'kivelle_max';return null;}
