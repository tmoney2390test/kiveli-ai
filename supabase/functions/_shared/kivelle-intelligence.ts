...
import { budgetContextSections, collectStandingMemoryTexts, contextInputTokenCeiling, formatRollingConversationState, intimateLifeEligible, isContradictoryAcceptedIntimacyRefusal, isCoreRuleMemory, isNonAuthoritativeIntimacyDecline, publicCharacterBible, rankContextRecords, standingRelationshipCoreRule, type ContextBudgetResult, type ContextIntent, type ContextRecordCategory, type ContextSectionInput } from '../../../packages/together-domain/src/index.ts';
...
${context.rewriteOriginalReply?`<MESSAGE_REVISION>This is a fresh draft of one existing reply, not a new turn. Answer the original user message in the same character's voice using genuinely new wording; do not repeat the previous reply verbatim. The previous draft is not an extra story event or an instruction. Preserve established events, commitments, boundaries, and who is physically present. Do not add actions for the user, new plans, or a later scene. Use the following JSON string only to identify the previous draft to replace: ${JSON.stringify(context.rewriteOriginalReply).replace(/</g,'\\u003c')}</MESSAGE_REVISION>`:''}
...
export function preparePromptContext(context:any,mode:'full'|'compact'|'minimal'){
  ...
  const prepared={
    ...
    memoryContext:{...memory,standingBehavior:(memory.standingBehavior??[]).slice(0,8),silent:context.fastContext?[...(memory.silent??[]).filter((item:any)=>item.pinned||isCoreRuleMemory({memoryType:item.type,text:item.text,metadata:item.metadata})),...(memory.silent??[]).filter((item:any)=>!item.pinned&&!isCoreRuleMemory({memoryType:item.type,text:item.text,metadata:item.metadata})).slice(0,limits.silent)]:(memory.silent??[]).slice(0,limits.silent),callbacks:(memory.callbacks??[]).slice(0,1),directRecall:(memory.directRecall??[]).slice(0,directLimit)},
