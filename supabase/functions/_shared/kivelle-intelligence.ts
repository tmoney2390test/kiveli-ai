...
When a character is eligible and freely reaches a final decision, state the decision and its immediate first order clearly. Immediate combat, killings, executions, and battlefield results of that first order may be narrated. Do not skip ahead to later campaign victories, regime changes, or other downstream world outcomes in the same reply. The server records the decision as a canonical world turning point so later characters and conversations can respond to it.
...
<ACTIVE_SCENARIO>${scenarioPrompt(context.activeScenario)}</ACTIVE_SCENARIO>
${context.activeScenario?`<CURRENT_SCENE>Selected scenario: ${context.activeScenario.title}. Starting location: ${context.activeScenario.locationName}. Starting activity: ${context.activeScenario.setup}. Continue from the latest established scene in the conversation, including subsequent movement. Ordinary daily schedules do not relocate the selected scenario.</CURRENT_SCENE>`:`<CURRENT_SCENE>Source: ${life.source??'schedule'}\nLocation: ${place?.path??life.location??'Current place'}\nActivity: ${life.activity??'living the day'}\nStarted: ${life.startedAt??'Not specified'}\nExpected end: ${life.expectedEndAt??'Not specified'}\nMood: ${life.mood??'content'} · energy: ${life.energy??'medium'}\nInterruptibility: ${life.interruptibility??life.availability??'open'}\nConversation entry: ${life.entryReason??'direct_chat'}\nNext obligation: ${life.nextObligation?`${life.nextObligation.title} at ${life.nextObligation.startsAt}`:'None known'}\n${context.activeScenario?'These are background schedule details; ACTIVE_SCENARIO defines the selected conversation setting.':'Treat these details as canonical reality.'} Do not contradict or repeatedly narrate them; mention place, time, or availability only when naturally relevant.</CURRENT_SCENE>`}
${context.activeScenario?`<CURRENT_INTERACTION>The scenario premise establishes participation in this scene. Treat in-person meetings as co-present unless the scenario or subsequent conversation establishes distance. Do not switch to remote messaging because of an ordinary schedule. Do not claim to send a photo or other attachment unless it was actually delivered by the media system; describe or show in-world objects naturally within the scene.</CURRENT_INTERACTION>`:`<CURRENT_INTERACTION>Mode: ${life.interactionMode??'remote'}\nEntry reason: ${life.entryReason??'direct_chat'}\nLast completed scene action: ${life.lastInteractionKey??'None'}\nArrival acknowledgement needed: ${life.sceneBehavior?.acknowledgeArrival?'yes':'no'}\nActivity awareness: ${life.sceneBehavior?.activityAwareness?'yes':'no'}\nDeparture pressure: ${life.sceneBehavior?.departurePressure?'yes':'no'}\n${context.sceneAction?'This response is the live opening beat for the newly selected scene action. Follow SCENE_ACTION_REACTION for its timing; do not turn it into a retrospective summary.':'Completed scene actions are canonical: they already happened. You may react naturally when the user speaks next, but never ask whether to perform them again.'} ${context.activeScenario?'The user selected ACTIVE_SCENARIO and its premise establishes their participation; ordinary remote schedule mode does not negate that scene.':''} If co_present, the user intentionally joined the companion's existing scene and you may acknowledge their arrival once if natural. If remote, the user is not physically at the companion's location unless the conversation explicitly establishes that. Never imply co-presence merely because the companion's location is known. Treat scene details as available context, not a script, and do not repeat them every turn.</CURRENT_INTERACTION>`}
...
Group roster: ${(context.groupContext.participants??[]).map((item:any)=>`${item.name} [${item.characterInstanceId}] · ${item.lifeStateLabel??'alive'}${item.lifeStateSummary?` · ${item.lifeStateSummary}`:''}`).join(', ')}.
...
You are ${context.sceneSpeakerDirective?.name??character.name}. Write exactly one natural message bubble as yourself. Never write dialogue or voluntary actions for another participant. A roster member marked dead cannot speak, react, move voluntarily, send media, or be puppeted; their body, injuries, and the surrounding scene may be described only when established by the conversation. Historical dialogue after a recorded death never overrides the canonical life-state marker. Dialogue can resume only after a new explicit resurrection or supernatural-return event changes that participant's canonical state. When the Director intent is respond_to_character, respond to the latest attributed companion message and its named speaker rather than independently answering the original user prompt again. React to what named people actually said, but do not expose private direct-message memories or infer that a late participant witnessed earlier turns. Add something novel, then release the conversational floor. Do not repeatedly narrate schedules or locations; mention your own situation only when naturally relevant.`:'Not a persistent group chat.'}</GROUP_CONTEXT>
...
<SCENE_ACTION_REACTION>${context.sceneAction?`The application has canonically resolved this shared-world request, but this response is staged at the moment the accepted or countered activity begins—not after it is over. Your first sentence must actively begin or directly enter the exact resolved beat in your established voice; do not answer with an unrelated opener. For a talk, social, or sharing action, discuss the actual subject now with a specific observation, opinion, or natural question. For another activity, write from inside its present unfolding. Never summarize with retrospective constructions such as “we talked about,” “we chose to,” “we just did,” “that was,” or “we finished.” React naturally as the companion without explaining the interaction system or writing actions for the user. The decision is canonical: ACCEPTED means begin the supplied action now; COUNTERED means the original did not occur and the supplied counter begins now; DECLINED means it did not occur. The supplied label/resolved interaction is the ONLY current beat. Ignore any different earlier option or companion suggestion in recent history; it was not selected. Never reverse the current result, substitute a stale suggestion, skip ahead to a later activity, or ask whether the original action should happen again.\n${JSON.stringify(context.sceneAction)}`:'None.'}</SCENE_ACTION_REACTION>
...
${context.activeScenario?`<CURRENT_LOCATION>Scenario starting place: ${context.activeScenario.locationName}, ${context.activeScenario.worldSlug}. Use the premise and subsequent conversation to establish the immediate surroundings. Later user-established movement takes precedence. Do not import the ordinary scheduled location into this scene.</CURRENT_LOCATION>`:`<CURRENT_LOCATION>${place?placeDetailBlock(place,promptLocationIntent(context.queryIntent)):context.location?`${context.location.name}: ${context.location.description}\nActivities: ${(context.location.possible_activities??[]).join(', ')}`:'None.'}
...
${context.savedWorldState?.length?`<SAVED_WORLD_STATE>${JSON.stringify(context.savedWorldState)}\nThese are completed, saved choices known to this participant. They override older routines or biographies where they conflict. A planned return keeps correspondence possible during an absence. Only the recorded outcome happened; never manufacture further departures, ownership, employment, evidence, or legal outcomes. The Calder’s Run bridge remains unfinished until a separate construction milestone is saved.</SAVED_WORLD_STATE>`:''}
${context.activeWorldStory?.length?`<ACTIVE_WORLD_STORY>${JSON.stringify(context.activeWorldStory)}\nThese are current chapter agendas, not completed events. Engage with the user's chosen scene without assuming its resolution. Only the listed saved evidence is disclosed; its limits remain binding. Future chapters and endings are unknown. Story choices are saved through the story controls, never by a narrator's claim.</ACTIVE_WORLD_STORY>`:''}
...
${context.activeScenario?`<PRESENT_REALITY>The active scenario establishes the starting premise, not a location reset on every turn. Continue from the latest scene and choices established after its opening. If the user asks where you are now, use that scenario continuity. Background daily schedules and unrelated active world stories must not contradict or interrupt this conversation. Preserve identity, relationship history, world access restrictions and account content settings. Never reveal internal labels or private development guidance.</PRESENT_REALITY>`:`<PRESENT_REALITY>
...
function promptSectionHasContext(key:string,context:any):boolean{
  ...
  if(key==='ACTIVE_SCENARIO')return Boolean(context.activeScenario);
...
function protectedPromptSection(key:string):boolean{return new Set(['ACTIVE_SCENARIO','ACTIVE_WORLD_STORY','SAVED_WORLD_STATE','CORE_RULES','WORLD_KNOWLEDGE','OUTPUT_LANGUAGE','IDENTITY','TURN_SPECIFIC_VOICE_CARD','SCENE_PRESSURE','USER_PERSONA','RELATIONSHIP_STANCE','INTIMATE_PRIVATE','CURRENT_SCENE','CURRENT_INTERACTION','USER_SHARED_IMAGES','RECENT_CONVERSATION','PRESENT_REALITY','CONTENT_BOUNDARY','RESPONSE_DIRECTION','CONTINUATION_REQUEST','USER_MESSAGE']).has(key);}
