...
export async function createSharedPlan(db:any, input:CreatePlanInput) {
  ...
  const {data:plan,error}=await db.from('together_shared_plans').insert({user_id:input.userId,character_instance_id:input.characterInstanceId,title:resolved.title,activity_key:resolved.activityKey,location_id:resolved.location.id,starts_at:start.toISOString(),ends_at:end.toISOString(),status:'scheduled',source:input.source,source_conversation_id:input.sourceConversationId??null,source_message_id:input.sourceMessageId??null,note:input.note?.trim()||null,metadata}).select('*').single();
  if(error||!plan){
...
export async function updateSharedPlan(db:any,input:{userId:string;planId:string;note?:string;locationId?:string;conversationId?:string}){
  ...
  if(input.locationId){const resolved=await resolvePlanOption(db,input.locationId,plan.activity_key,undefined,Math.max(30,(new Date(plan.ends_at).getTime()-new Date(plan.starts_at).getTime())/60000));await validateAvailability(db,{userId:input.userId,characterInstanceId:plan.character_instance_id,characterVersionId:plan.together_character_instances.character_version_id,location:resolved.location,start:new Date(plan.starts_at),end:new Date(plan.ends_at),excludePlanId:plan.id});patch.location_id=resolved.location.id;patch.title=resolved.title;}
...
async function resolvePlanOption(db:any,locationId:string,activityValue:string,titleValue?:string,durationValue?:number){
  ...
  if(!location)throw new AppError('VALIDATION_FAILED','Choose a real City Life location.',400);
...
async function validateAvailability(db:any,input:{userId:string;characterInstanceId:string;characterVersionId:string;location:any;start:Date;end:Date;excludePlanId?:string}){
  ...
  const{data:profile}=await db.from('together_profiles').select('experience_timezone').eq('user_id',input.userId).maybeSingle();
  const timezone=safeTimezone(profile?.experience_timezone);
  if(!locationIsOpen(input.location,input.start,input.end,timezone))throw new AppError('LOCATION_CLOSED',`${input.location.name} is closed at that time.`,409,true);
  ...
  const[{data:conflicts},{data:dates},{data:schedules}]=await Promise.all([plans,db.from('together_date_sessions').select('id,scheduled_for,together_date_templates(name)').eq('user_id',input.userId).eq('character_instance_id',input.characterInstanceId).eq('status','upcoming'),db.from('together_schedule_templates').select('*').eq('character_version_id',input.characterVersionId)]);
  if(conflicts?.length)throw new AppError('PLAN_CONFLICT',`You already have ${conflicts[0].title} at that time.`,409,true);
  ...
  if(busy)throw new AppError('COMPANION_BUSY',`Your companion is busy with ${busy.activity} then. Choose a later time.`,409,true);
}
...
async function matchingDateSession(db:any,userId:string,instanceId:string,locationId:string,activityKey:string){
  ...
  return(sessions??[]).find((session:any)=>session.together_date_templates?.location_id===locationId&&normalize(session.together_date_templates?.name??'').includes(normalize(label).split(' ')[0]))??null;
}

function planCardMetadata(plan:any,locationName?:string){return{title:plan.title,startsAt:plan.starts_at,endsAt:plan.ends_at,status:plan.status,locationId:plan.location_id,location:locationName??'City Life',activityKey:plan.activity_key,note:plan.note??null};}
...
function durationFor(activity:string){if(/movie/.test(activity))return150;if(/trivia|music|dinner|karaoke|comedy/.test(activity))return120;if(/walk|shopping|gallery|books|records|photos/.test(activity))return90;if(/coffee|pastry/.test(activity))return60;return90;}
