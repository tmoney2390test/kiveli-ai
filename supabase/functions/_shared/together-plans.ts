...
export async function createSharedPlan(db:any, input:CreatePlanInput) {
  ...
  const {data:plan,error}=await db.from('together_shared_plans').insert({user_id:input.userId,continuity_id:continuity.id,character_instance_id:input.characterInstanceId,participant_instance_ids:roster.participantInstanceIds,title:resolved.title,activity_key:resolved.activityKey,world_id:resolved.location.world_id,location_id:resolved.location.id,starts_at:start.toISOString(),ends_at:end.toISOString(),world_timezone:availability.worldTimezone,user_timezone:availability.userTimezone,status:'scheduled',source:input.source,source_conversation_id:input.sourceConversationId??null,source_message_id:input.sourceMessageId??null,note:input.note?.trim()||null,metadata,time_precision:'exact',participation_mode:'live',grace_minutes:30,companion_state:'expected'}).select('*').single();
  if(error||!plan){
  ...
  if(groupPlan&&!input.deferSideEffects)await recordGroupPlanCommitment(db,{userId:input.userId,plan});
  if(!input.deferSideEffects){
    ...
    await track(db,input.userId,groupPlan?'group_plan_created':'plan_created',{planId:plan.id,source:input.source,conversionSource:conversionSource(input.source),characterInstanceId:input.characterInstanceId,conversationId:roster.groupConversationId,participantCount:roster.participantInstanceIds.length});
...
async function clearCancelledPlanContext(db:any,input:{userId:string;characterInstanceId:string;planId:string;conversationId?:string|null;now:string}){
  ...
  await db.from('together_proactive_messages').update({status:'cancelled',updated_at:input.now}).eq('user_id',input.userId).eq('status','queued').eq('context->>groupPlanId',input.planId);
}
...
function planCardMetadata(plan:any,locationName?:string){return{title:plan.title,startsAt:plan.starts_at,endsAt:plan.ends_at,status:plan.status,worldId:plan.world_id,locationId:plan.location_id,location:locationName??'Current place',activityKey:plan.activity_key,note:plan.note??null,immediate:plan.metadata?.immediate===true,timingChoice:plan.metadata?.immediate===true?'now':undefined,participantInstanceIds:plan.participant_instance_ids??plan.metadata?.participantInstanceIds??[plan.character_instance_id],participantNames:plan.metadata?.participantNames??[],groupPlan:plan.metadata?.groupPlan===true};}
