...
import {fetchRevenueCatSubscriber,normalizeRevenueCatSubscriber,readRevenueCatAdapterConfig,type NormalizedRevenueCatSubscription,type RevenueCatWebhookEvent} from './revenuecat.ts';
...
export async function syncRevenueCatUser(db:Db,userId:string,event:RevenueCatWebhookEvent,config:ReturnType<typeof readRevenueCatAdapterConfig>,secretApiKey:string) {
  const lease=crypto.randomUUID();
  const {data:claimed,error}=await db.rpc('kivelle_claim_billing_reconciliation',{p_user_id:userId,p_lease:lease});
  if(error||!claimed)throw new AppError('PROVIDER_UNAVAILABLE','Membership verification is already running. Try again shortly.',503,true);
  try { return await reconcileRevenueCatUser(db,userId,event,config,secretApiKey); }
  finally { await db.from('together_billing_reconciliation_leases').delete().eq('user_id',userId).eq('lease_id',lease); }
}
async function reconcileRevenueCatUser(db:Db,userId:string,event:RevenueCatWebhookEvent,config:ReturnType<typeof readRevenueCatAdapterConfig>,secretApiKey:string):Promise<{applied:boolean;verifiedNoPurchase:boolean}>{
  ...
  return {applied,verifiedNoPurchase:!current&&Object.keys(snapshot.subscriber.subscriptions).length===0&&Object.keys(snapshot.subscriber.entitlements).length===0};
}
