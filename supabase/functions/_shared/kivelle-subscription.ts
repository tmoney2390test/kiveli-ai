...
export async function resolveSubscriptionAccess(db:SupabaseClient,userId:string,now=new Date(),readOnly=false):Promise<KivelleSubscriptionAccess>{
  ...
  if(!row&&readOnly)row={tier:'free',entitlement_keys:[...entitlementsForTier('free')]};
