...
const cycle=(now=new Date())=>`${now.getUTCFullYear()}-${String(now.getUTCMonth()+1).padStart(2,'0')}`;
...
export async function resolveSubscriptionState(db:SupabaseClient,userId:string,now=new Date()):Promise<KivelleSubscriptionState>{
  ...
  const tier=normalizeSubscriptionTier(row.tier),capabilities=capabilitiesForTier(tier);
  if(row.tier!==tier||!sameKeys(row.entitlement_keys,capabilities.entitlements))await db.from('together_entitlements').update({tier,entitlement_keys:[...capabilities.entitlements],updated_at:now.toISOString()}).eq('user_id',userId);
  await ensureCreditGrants(db,userId,capabilities,now);
...
export async function ensureCreditGrants(db:SupabaseClient,userId:string,capabilities:KivelleCapabilities,now=new Date()):Promise<void>{
  ...
  const grantCycle=cycle(now),grantKey=`subscription:${capabilities.tier}:${grantCycle}`;
  const{error}=await db.rpc('kivelle_grant_subscription_credits',{p_user_id:userId,p_amount:capabilities.monthlyCreditGrant,p_cap:capabilities.subscriptionCreditRolloverCap,p_cycle:grantCycle,p_idempotency_key:grantKey,p_metadata:{tier:capabilities.tier}});
