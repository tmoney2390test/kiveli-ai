...
export async function ensureCreditGrants(db:SupabaseClient,userId:string,capabilities:KivelleCapabilities,now=new Date(),grantCycle=billingCycle(null,now)):Promise<void>{
  ...
  const grantKey=`subscription:${capabilities.tier}:${grantCycle}`;
  const{error}=await db.rpc('kivelle_grant_subscription_credits',{p_user_id:userId,p_amount:capabilities.monthlyCreditGrant,p_cap:capabilities.subscriptionCreditRolloverCap,p_cycle:grantCycle,p_idempotency_key:grantKey,p_metadata:{tier:capabilities.tier,cycle:grantCycle}});
