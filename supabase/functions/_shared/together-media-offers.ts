...
export async function createMediaOffer(db:SupabaseClient,input:CreateMediaOfferInput):Promise<Record<string,any>|null>{
  const[{data:instance},{data:profile},tier]=await Promise.all([
  ...
  const row={user_id:input.userId,continuity_id:String(instance.continuity_id),character_instance_id:input.characterInstanceId,conversation_id:input.conversationId??null,message_id:input.messageId??null,life_event_id:input.lifeEventId??null,date_session_id:input.dateSessionId??null,moment_id:input.momentId??null,story_arc_id:input.storyArcId??null,scene_session_id:input.sceneSessionId??null,shared_plan_id:input.sharedPlanId??null,offer_key:offerKey,source:input.source,status:'pending',content_level:normalizeOfferLevel(input.contentLevel),quality_tier:policy.qualityTier,shot_type:input.shotType??'scene',credit_action:policy.creditAction,credit_cost:policy.creditCost,title,companion_message:companionMessage,preview_metadata:{...(input.previewMetadata??{}),characterName:name,providerRequested:false,autoAcceptIncludedBenefit:policy.autoAccept},included_subscription_benefit:policy.includedSubscriptionBenefit,included_benefit_type:policy.includedBenefitType,subscription_tier_at_creation:tier,expires_at:expiresAt};
