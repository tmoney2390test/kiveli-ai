...
export async function listPendingMediaOffers(db:SupabaseClient,input:{userId:string;continuityId?:string|null;characterInstanceId?:string|null;adultPipelineAuthorized?:boolean}):Promise<Record<string,any>[]>{
  ...
  return presented.map((offer)=>{
    ...
    return{...offer,preview_metadata:{...((offer.preview_metadata??{}) as Record<string,unknown>),dailyPhotoAllowanceLimit:allowance.limit,dailyPhotoAllowanceRemaining:allowance.remaining,dailyPhotoBenefitDate:allowance.benefitDate,dailyPhotoAllowanceTimezone:allowance.timezone,dailyPhotoAllowanceResetsAt:allowance.resetsAt}};
  });
}
