...
export function presentRelationshipMilestone(proposal:RelationshipMilestoneProposal,input:{companionName:string;experienceTitle?:string}):PresentedRelationshipMilestone{
  ...
  switch(proposal.presentationKey){
    ...
    case'relationship.keep_in_touch':return{title:'Keep in touch?',body:`The conversation with ${name} has found enough of a rhythm that neither of you seems ready to leave it as a one-time meeting.`,prompt:'What do you say?',choices:[{id:'accept',label:'I\u2019d like that',tone:'primary'},{id:'defer',label:'Let\u2019s take it slowly',tone:'secondary'}]};
