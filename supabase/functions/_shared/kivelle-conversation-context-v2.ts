...
import { antiRepetitionGuidance, classifyChemistryResponseIntent, compileCharacterGoals, compileCharacterVoiceCard, compileRelationshipStance, compileResponseBrief, normalizeCharacterUserView, scoreConversationEngagement, type CharacterGoals, type CharacterUserView, type CharacterVoiceCard, type KivelleCapabilities, type PromptInteractionQuality, type RelationshipStance, type RelationshipState, type ResponseBrief, type SpiceLevel } from '../../../packages/together-domain/src/index.ts';
...
export async function buildTieredKivelleConversationContext(input:{db:SupabaseClient;userId:string;instance:Row;conversation:Row;userMessage:string;lifeRun:Row;semanticRows?:Row[];attachments?:Row[];now?:Date;correlationId?:string}):Promise<TieredConversationContext>{
  ...
  if(caps.recentTurnBudget>recent.length){const{data}=await input.db.from('together_messages').select('role,content,created_at,provider_metadata').eq('conversation_id',input.conversation.id).order('created_at',{ascending:false}).limit(caps.recentTurnBudget);if(data?.length)recent=data.reverse().map((item:Row)=>({role:String(item.role),content:String(item.content),...(item.provider_metadata&&typeof item.provider_metadata==='object'?{providerMetadata:item.provider_metadata as Record<string,unknown>}:{})}));}
  ...
  const chatPreferences=storedPreferences&&typeof storedPreferences==='object'&&!Array.isArray(storedPreferences)?storedPreferences as Row:{};
  const paidSpice=subscription.tier==='free'?undefined:chatPreferences.spiceLevel;
  const spiceLevel=normalizeSpice(paidSpice??base.character?.spice_level);
  const conversationStyle=chatPreferences.responseStyle==='paragraph'||chatPreferences.responseStyle==='texting'?chatPreferences.responseStyle:base.conversationStyle;
  const personality=(version?.personality_config??base.character?.personality_config??{}) as Record<string,unknown>;const relationshipState=toRelationshipState(base.relationship);
  const effectiveCharacter={...base.character,spice_level:spiceLevel,character_bible:version?.character_bible??{},life_config:version?.life_config??{},relationship_config:version?.relationship_config??{}};
  ...
  const eligibleOpenThread=base.openThreads.filter((thread)=>thread.eligible&&thread.followupCount<1&&!thread.lastFollowedUpAt).sort((a,b)=>b.importance-a.importance||(a.expectedAt??'9999').localeCompare(b.expectedAt??'9999'))[0];
  const handoffsEnabled=String(Deno.env.get('KIVELLE_CONVERSATIONAL_HANDOFFS_ENABLED')??'true').toLowerCase()!=='false';
  const baseBrief=compileResponseBrief({message:input.userMessage,interactionQuality,relationshipStance,...(responseIntent?{responseIntent}:{}),...(eligibleOpenThread?{eligibleOpenThread}:{}),nextCommitment:nextCommitment?.title??base.upcomingCommitments[0]?.title,activeStory:base.activeStory?.title,recentAssistantMessages:assistantMessages,recentTurns:recent.map((turn)=>({role:turn.role,content:turn.content})),now:input.now,handoffsEnabled});
  const director=await runKivelleDirector({context:{...base,commitments,character:effectiveCharacter,relationshipStance,characterGoals,recent,userMessage:input.userMessage},baseBrief,policy:caps.directorPolicy,interactionQuality,pendingMilestone:Boolean(base.progression),activeConflict:Number(base.relationship?.conflict??0)>=45,usageScope:{db:input.db,userId:input.userId,continuityId:String(input.instance.continuity_id??''),conversationId:String(input.conversation.id),characterInstanceId:String(input.instance.id),subscriptionTier:subscription.tier,contentMode:String(base.contentMode??'standard'),...(input.correlationId?{correlationId:input.correlationId}:{})}});
  const characterVoice=compileCharacterVoiceCard({bible:version?.character_bible??{},characterName:String(base.character?.name??''),occupation:String(base.character?.occupation??''),message:input.userMessage,mode:director.brief.mode,relationshipStage:String(base.relationship?.relationship_stage??base.relationship?.stage??'stranger'),trust:Number(base.relationship?.trust??0),interactionMode:String(base.currentScene?.interactionMode??'remote'),interactionQuality,recentAssistantMessages:assistantMessages});
