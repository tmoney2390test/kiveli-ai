...
import type { RelevantDialogueOpportunity } from './kivelle-dialogue-opportunities.ts';
...
export type KivelleConversationContext = {
  ...
  memories: Array<{ id:string; text:string; type:string; pinned:boolean; importance:number; characterInstanceId?:string }>;
  ...
  recent: Array<{ role:string; content:string; createdAt?:string; speakerCharacterInstanceId?:string|null; speakerName?:string|null; conversationSequence?:number|null; providerMetadata?:Record<string,unknown> }>;
  userMessage: string;
...
export async function buildKivelleConversationContext(input: {
  ...
  const [profile,entitlements, continuity, prefs, relationship, milestone, memories, threads, messages, schedules, events, plans, dates, stories, edges, instances, worlds, locations, media, moments, episodes, patterns, residue,relationshipPlaceRows,placeProfileRows] = await Promise.all([
    db.from('together_profiles').select('experience_timezone,interests,content_preferences,conversation_preferences,multimodal_preferences,privacy_settings').eq('user_id', userId).maybeSingle(),
    ...
    planningIntent?db.from('together_character_place_profiles').select('location_id,sentiment,confidence,opinion_summary,preferred_activities').eq('character_version_id',instance.character_version_id):emptyRows(),
  ]);
  ...
  const resolvedConversationScene=input.forceRemoteInteraction?{...sceneResolution,scene:null,expired:false}:sceneResolution;
  ...
  const memoryContext:ActivatedMemoryContext=personalizationEnabled
    ?await retrieveActivatedMemories({db,userId,characterInstanceId:String(instance.id),userMessage,intent,storedRows:memories.data??[],semanticRows:input.semanticRows??[],currentScene:{...currentScene,worldId:place?.world.id},relationship:relationship.data??{},recentAssistantMessages:(messages.data??[]).filter((item:Row)=>item.role==='assistant'),now})
  ...
  if(currentScene.sceneSessionId){
    ...
    const participantIds=(participantRows??[]).map((item:Row)=>String(item.character_instance_id));
    const[{data:participantRelationships},{data:socialStates}]=await Promise.all([
      participantIds.length?db.from('together_relationship_states').select('character_instance_id,trust,comfort,affinity,familiarity').eq('user_id',userId).in('character_instance_id',participantIds):Promise.resolve({data:[]}),
      participantIds.length>1?db.from('together_character_social_states').select('character_a_instance_id,character_b_instance_id,relationship_type,affinity,tension').eq('user_id',userId).or(`character_a_instance_id.in.(${participantIds.join(',')}),character_b_instance_id.in.(${participantIds.join(',')})`):Promise.resolve({data:[]}),
    ]);
  ...
  return {
    ...
    persona:personalizationEnabled
      ...
      :{display_name:(Array.isArray(continuity.data.together_user_personas)?continuity.data.together_user_personas[0]:continuity.data.together_user_personas)?.display_name},
    ...
    activeStory, memories:memoryRows.map((item)=>({id:String(item.id),text:String(item.text),type:String(item.type),pinned:Boolean(item.pinned),importance:Number(item.importance??0),characterInstanceId:String(instance.id)})),memoryContext,emotionalResidue,
    ...
    openThreads:personalizationEnabled?(threads.data??[]).map((thread:Row)=>threadContext(thread)):[], social,
...
function attributedRecentTurn(item:Row){
  const providerMetadata=item.provider_metadata&&typeof item.provider_metadata==='object'?item.provider_metadata as Record<string,unknown>:undefined;
...
function threadContext(thread:Row){const subject=String(thread.subject??thread.metadata?.subject??'something important');return{id:String(thread.id),subject,displaySubject:String(thread.display_subject??subject),followupPrompt:String(thread.followup_prompt??`I should tell you how my ${subject} went.`),expectedAt:thread.expected_at??null,eligible:Boolean(thread.follow_up_eligible),importance:Number(thread.importance??.5),lastFollowedUpAt:thread.last_followed_up_at??null,followupCount:Number(thread.followup_count??0)};}
