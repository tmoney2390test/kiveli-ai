...
import { classifyConversationQuery, conversationReferencesKnownCharacter, resolvePresentReality } from '../../../packages/together-domain/src/conversation.ts';
...
export type KivelleConversationContext = {
  ...
  conversationEpisodes:RelevantConversationEpisode[];
...
export async function buildKivelleConversationContext(input: {
  ...
  const continuityPersona=Array.isArray(continuity.data.together_user_personas)?continuity.data.together_user_personas[0]:continuity.data.together_user_personas;
  const personaId=String(continuity.data.persona_id??continuityPersona?.id??'');
  ...
  return {
    ...
    clock, currentScene:{...currentScene,schedulePaused}, life: {...currentScene,schedulePaused}, relationship:{...relationship.data,relationship_stage:instance.relationship_stage}, progression:milestone.data ?? null,
