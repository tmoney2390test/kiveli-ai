...
const ENGINE_VERSION='life_engine_v1';
...
export async function ensureCharacterSchedule(input:{db:SupabaseClient;userId:string;characterInstanceId:string;now?:Date;days?:number}){
  ...
  const fixed=[...(plansResult.data??[]).map((plan:Row):ScheduleBlock=>({activityKey:String(plan.activity_key),title:String(plan.title),locationId:plan.location_id?String(plan.location_id):null,startsAt:String(plan.starts_at),endsAt:String(plan.ends_at),priority:'user_commitment',visibility:'shared',source:'user_plan',interruptibility:'open',generationKey:`plan:${plan.id}`,metadata:{planId:plan.id}})),...legacyBlocks(legacyResult.data??[],instance,timezone,startDate,days,worldId)];
  const configuredProfile=lifeProfile(instance),preferredWorkSlug=configuredProfile.occupation?.primaryLocationSlug,preferredWorkLocation=preferredWorkSlug?(locationsResult.data??[]).find((location:Row)=>location.slug===preferredWorkSlug):null;
  const profile=preferredWorkLocation&&configuredProfile.occupation?{...configuredProfile,occupation:{...configuredProfile.occupation,primaryLocationId:String(preferredWorkLocation.id)}}:configuredProfile;
  const generated=generateScheduleWindow({characterInstanceId,seed:String(instance.simulation_seed??characterInstanceId),timezone,fromLocalDate:startDate,days,profile,locations,homeLocationId:String(base.id),activityTemplates:templates.length?templates:defaultActivities(),fixedCommitments:fixed,history:[...history,...existing],generationVersion:ENGINE_VERSION});
...
function toActivityTemplate(row:Row):ActivityTemplate{return{key:String(row.activity_key),title:String(row.title),category:String(row.category),validTimeWindows:Array.isArray(row.valid_time_windows)?row.valid_time_windows:[],durationMinutes:parseRange(row.duration_minutes,[45,90]),locationCategories:row.location_categories??[],locationSlugs:row.location_slugs??[],tags:row.tags??[],affinity:Number(row.affinity??.5),preferredWeeklyFrequency:parseRange(row.preferred_weekly_frequency,[0,2]),maximumWeeklyFrequency:Number(row.maximum_weekly_frequency??2),minimumGapHours:Number(row.minimum_gap_hours??18),energyRequirement:row.energy_requirement??undefined,socialRequirement:row.social_requirement??'either',priority:row.priority??'preferred_activity',visibility:row.visibility??'hidden',interruptibility:row.interruptibility??'open',activityLabel:row.metadata?.activityLabel??row.title};}
