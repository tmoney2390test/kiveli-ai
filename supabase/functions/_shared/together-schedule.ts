...
const ENGINE_VERSION='life_engine_v4_natural_language';
...
export async function resolveCharacterPresence(input:{db:SupabaseClient;userId:string;characterInstanceId:string;now?:Date;ensure?:boolean;routineOnly?:boolean}):Promise<ResolvedCharacterPresence|null>{
  ...
  const fallbackActivity=naturalizeCharacterActivity(preserveAuthoredState?String(instance.current_activity??'Having some unstructured time'):'Having some unstructured time at home');
...
function toActivityTemplate(row:Row):ActivityTemplate{const key=String(row.activity_key),title=naturalizeCharacterActivity(row.title,{activityKey:key}),activityLabel=naturalizeCharacterActivity(row.metadata?.activityLabel??title,{activityKey:key});return{key,title,category:String(row.category),validTimeWindows:Array.isArray(row.valid_time_windows)?row.valid_time_windows:[],durationMinutes:parseRange(row.duration_minutes,[45,90]),locationCategories:row.location_categories??[],locationSlugs:row.location_slugs??[],tags:row.tags??[],affinity:Number(row.affinity??.5),preferredWeeklyFrequency:parseRange(row.preferred_weekly_frequency,[0,2]),maximumWeeklyFrequency:Number(row.maximum_weekly_frequency??2),minimumGapHours:Number(row.minimum_gap_hours??18),energyRequirement:row.energy_requirement??undefined,socialRequirement:row.social_requirement??'either',priority:row.priority??'preferred_activity',visibility:row.visibility??'hidden',interruptibility:row.interruptibility??'open',activityLabel,upcomingHint:row.metadata?.upcomingHint?naturalizeCharacterActivity(row.metadata.upcomingHint):undefined,outcomeEligible:row.metadata?.outcomeEligible===true,outcomeProbability:Number(row.metadata?.outcomeProbability??0),outcomeVariants:Array.isArray(row.metadata?.outcomeVariants)?row.metadata.outcomeVariants.map(String):[],rare:row.metadata?.rare===true};}
function toBlock(row:Row):ScheduleBlock{const activityKey=String(row.activity_key),title=naturalizeCharacterActivity(row.title,{activityKey}),metadata=row.metadata??{},activityLabel=naturalizeCharacterActivity(metadata.activityLabel??title,{activityKey});return{id:String(row.id),activityKey,title,locationId:row.location_id?String(row.location_id):null,startsAt:String(row.starts_at),endsAt:String(row.ends_at),priority:row.priority,visibility:row.visibility,source:row.source,interruptibility:row.interruptibility,participantInstanceIds:row.participant_instance_ids??[],generationKey:String(row.generation_key),metadata:{...metadata,activityLabel,...(metadata.upcomingHint?{upcomingHint:naturalizeCharacterActivity(metadata.upcomingHint)}:{})}};}
function legacyBlocks(rows:Row[],timezone:string,startDate:string,days:number,worldId:string,homeLocationId:string){const out:ScheduleBlock[]=[];for(let day=0;day<days;day++){const date=addDays(startDate,day),weekday=new Date(`${date}T12:00:00Z`).getUTCDay();for(const row of rows){const metadata=row.metadata??{},authored=metadata.scheduleMode==='authored',rowWorld=String(row.together_locations?.world_id??'');if(!scheduleRunsOnDate(metadata,date)||Number(row.day_of_week)!==weekday||(row.location_id&&rowWorld!==worldId))continue;const title=authoredActivity(row,date),locationId=row.location_id?String(row.location_id):homeLocationId;out.push({activityKey:String(metadata.activityKey??row.activity),title,locationId,startsAt:localToUtc(date,Number(row.start_minute),timezone).toISOString(),endsAt:localToUtc(date,Number(row.end_minute),timezone).toISOString(),priority:metadata.priority??(/work|class|shift/i.test(String(row.activity))?'hard_obligation':'recurring_routine'),visibility:metadata.profileVisibility==='hidden'?'hidden':authored?'known':row.availability==='busy'?'known':'hidden',source:'recurring',interruptibility:row.availability==='busy'?'busy':row.availability==='limited'?'limited':'open',generationKey:`legacy:${row.id}:${date}`,metadata:{...metadata,legacyTemplateId:row.id,activityLabel:title}});}}return out;}
function authoredActivity(row:Row,date:string){const variants=Array.isArray(row.metadata?.activityVariants)?row.metadata.activityVariants.filter((value:unknown)=>typeof value==='string'&&Boolean(value.trim())):[];const selected=!variants.length?String(row.activity):String(variants[stableIndex(`${row.id}:${date}`,variants.length)]);return naturalizeCharacterActivity(selected,{activityKey:String(row.metadata?.activityKey??row.activity)});}
...
function sceneActivityLabel(state:Row|undefined,activityKey:string){const explicit=typeof state?.activityLabel==='string'?state.activityLabel.trim():'';if(explicit)return naturalizeCharacterActivity(explicit,{activityKey});return activityKey&&activityKey!=='together'?naturalizeCharacterActivity(activityKey,{activityKey}):'Spending time together';}
