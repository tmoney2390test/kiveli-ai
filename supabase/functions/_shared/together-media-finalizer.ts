...
function sanitizeProviderMetadata(value:Record<string,unknown>):Record<string,unknown>{const allowed=['status','hasNsfwContents','inferenceMs','outputCount','webhookReceivedAt','qualityVerdict','qualityReasonCodes','qualityProviderStatus','qualityErrorCode','qualityTimedOut','sfwExplicitConfirmation','sfwExplicitConfirmationProviderRequestId','sfwExplicitConfirmationProviderModel','sfwExplicitConfirmationInferenceMs','sfwExplicitConfirmationActualCostUsd'];return Object.fromEntries(allowed.filter((key)=>key in value).map((key)=>[key,value[key]]));}
