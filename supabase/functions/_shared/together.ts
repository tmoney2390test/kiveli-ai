...
function loadSnapshotCatalog(db:SupabaseClient){
  ...
  ]).then(([worlds,locations,characterWorldPresence,trips,photoOpportunities])=>({worlds,locations,characterWorldPresence,trips,photoOpportunities}));
  snapshotCatalogCache={expiresAt:Date.now()+SNAPSHOT_CATALOG_TTL_MS,value};
  void value.catch(()=>{if(snapshotCatalogCache?.value===value)snapshotCatalogCache=null;});
  return value;
}
...
export async function buildExploreCatalogSnapshot(db:SupabaseClient,userId:string):Promise<Record<string,unknown>>{
  ...
  const publishedLocations=await filterAccessibleLocations(db,userId,(locations.data??[]).filter((location)=>worldIds.has(String(location.world_id))));
  ...
  return{
    ...
    discoverableCharacters:await hydrateDiscoverableCharacters(db,(discoverable.data??[]).filter((row:Record<string,any>)=>!row.discovery_metadata?.requiredLocationState||publishedLocations.some((location:Record<string,any>)=>location.id===row.first_meeting?.location_id))),
...
export async function buildSnapshot(db: SupabaseClient, userId: string, requestedTimezone?: string | null): Promise<Record<string, unknown>> {
  ...
  if(!profileCheck.data)return buildOnboardingSnapshot(db,userId,null);
  if(!profileCheck.data.onboarding_completed_at)return buildOnboardingSnapshot(db,userId,profileCheck.data);
  let profileForSnapshot=profileCheck;
  if(requestedTimezone&&profileCheck.data.experience_timezone!==requestedTimezone){
    const updatedAt=new Date().toISOString();
    profileForSnapshot={...profileCheck,data:{...profileCheck.data,experience_timezone:requestedTimezone,updated_at:updatedAt}};
    waitUntil(Promise.all([
      db.from('together_profiles').update({experience_timezone:requestedTimezone,updated_at:updatedAt}).eq('user_id',userId),
      db.from('together_notification_preferences').update({timezone:requestedTimezone,updated_at:updatedAt}).eq('user_id',userId),
    ]).then(()=>undefined));
  }
  const catalog=loadSnapshotCatalog(db);
  const continuity=await activeContinuity(db,userId);
  ...
  const dailyMessageWindowStart=new Date();dailyMessageWindowStart.setUTCHours(0,0,0,0);
  ...
  const publishedLocations=await filterAccessibleLocations(db,userId,(locations.data??[]).filter((location)=>publishedWorldIds.has(String(location.world_id))));
  ...
  const visibleInstances:Array<Record<string,any>>=(instances.data??[]).map((instance:Record<string,any>):Record<string,any>=>{
    ...
    const authored=resolveAuthoredSnapshotPresence(instance,nowDate,schedules.data??[],publishedLocations,publishedWorlds,publishedCharacterPresence,scheduleEvents.data??[]);
  ...
  const snapshotCapabilities=capabilitiesForAccount(String(entitlements.data?.tier??'free'),entitlements.data?.metadata);
  const dailyMessageLimit=snapshotCapabilities.dailyMessageLimit;
  const dailyMessageUsed=dailyMessageLimit===null?null:Math.min(dailyMessageLimit,Math.max(0,Number(dailyMessages.count??0)));
  const dailyMessageAllowance={limit:dailyMessageLimit,used:dailyMessageUsed,remaining:dailyMessageLimit===null?null:Math.max(0,dailyMessageLimit-(dailyMessageUsed??0)),resetAt:dailyMessageWindowReset.toISOString()};
  const {memories:clientMemories,memoryCounts}=projectSnapshotMemories(durableMemories,snapshotCapabilities.entitlements);
  ...
  const [signed,discoverableCharacters,characterPlaceProfilesResult,currentPlaceContext]=await Promise.all([
    ...
    hydrateDiscoverableCharacters(db,(discoverable.data??[]).filter((row:Record<string,any>)=>!row.discovery_metadata?.requiredLocationState||publishedLocations.some((location:Record<string,any>)=>location.id===row.first_meeting?.location_id))),
...
export async function buildCharacterPresenceSnapshot(
  ...
  const [schedules, scheduleEvents, worlds, locations, presence, scenes, dates, plans, lifeEvents]=await Promise.all([
    db.from('together_schedule_templates').select('*').eq('character_version_id',versionId).order('day_of_week').order('start_minute'),
    db.from('together_character_schedule_events').select('*').eq('user_id',userId).eq('continuity_id',continuity.id).eq('character_instance_id',characterInstanceId).gte('ends_at',new Date(now-86400000).toISOString()).lte('starts_at',new Date(now+8*86400000).toISOString()).order('starts_at').limit(120),
    catalog.then((value)=>value.worlds),
    catalog.then((value)=>value.locations),
    catalog.then((value)=>({...value.characterWorldPresence,data:(value.characterWorldPresence.data??[]).filter((row:Record<string,unknown>)=>String(row.character_version_id)===versionId)})),
    db.from('together_scene_sessions').select('*').eq('user_id',userId).eq('continuity_id',continuity.id).eq('character_instance_id',characterInstanceId).is('ended_at',null).order('started_at',{ascending:false}).limit(1),
    db.from('together_date_sessions').select('*, together_date_templates(*)').eq('user_id',userId).eq('continuity_id',continuity.id).eq('character_instance_id',characterInstanceId).in('status',['scheduled','active']),
    db.from('together_shared_plans').select('*,together_plan_attendance(*),together_plan_participant_responses(*)').eq('user_id',userId).eq('continuity_id',continuity.id).eq('character_instance_id',characterInstanceId).in('status',['scheduled','active','started']).limit(20),
    db.from('together_life_events').select('*').eq('user_id',userId).eq('continuity_id',continuity.id).eq('character_instance_id',characterInstanceId).lte('starts_at',new Date(now+3*60*60*1000).toISOString()).order('starts_at',{ascending:false}).limit(20),
  ]);
...
async function buildOnboardingSnapshot(db:SupabaseClient,userId:string,profile:Record<string,unknown>|null):Promise<Record<string,unknown>>{
  const[worlds,locations,characterWorldPresence,discoverable,entitlements,preferences,personas]=await Promise.all([
    db.from('together_worlds').select('*').eq('published',true),
    db.from('together_locations').select('*'),
    db.from('together_character_world_presence').select('*'),
  ...
  const failed=[worlds,locations,characterWorldPresence,discoverable,entitlements,preferences,personas].find((result)=>result.error);
  ...
  const publishedLocations=(await filterAccessibleLocations(db,userId,(locations.data??[]).filter((location)=>publishedWorldIds.has(String(location.world_id))))).map(compactSnapshotLocation);
  const discoverableCharacters=await hydrateDiscoverableCharacters(db,(discoverable.data??[]).filter((row:Record<string,any>)=>!row.discovery_metadata?.requiredLocationState||publishedLocations.some((location:Record<string,any>)=>location.id===row.first_meeting?.location_id)));
  const entitlementKeys=(entitlements.data?.entitlement_keys??[]).map(String);
  return{
    profile:profile?projectClientProfile(profile):null,activePersona:personas.data?.[0]??null,activeContinuity:null,personas:personas.data??[],continuities:[],
...
export function compactSnapshotSchedule(row:Record<string,any>){const hidden=row.metadata?.source==='calders_run_authoring_v1'&&row.metadata?.profileVisibility==='hidden';return{...row,...(hidden?{location_id:null,activity:'Private time'}:{}),metadata:snapshotMetadata(row.metadata,SNAPSHOT_SCHEDULE_METADATA_KEYS)};}
function compactSnapshotScheduleEvent(row:Record<string,any>){const hidden=row.metadata?.source==='calders_run_authoring_v1'&&row.visibility==='hidden';return{...row,...(hidden?{location_id:null,title:row.activity_key==='travel'?'On the way':'Private time'}:{}),metadata:hidden?{}:snapshotMetadata(row.metadata,SNAPSHOT_SCHEDULE_EVENT_METADATA_KEYS)};}
...
async function fetchScheduleTemplates(db:SupabaseClient,characterVersionIds:string[]){
  if(!characterVersionIds.length)return{data:[],error:null};
  const chunkSize=100,rows:Array<Record<string,unknown>>=[];
  for(let index=0;index<characterVersionIds.length;index+=chunkSize){
    const page=await db.from('together_schedule_templates').select('*').in('character_version_id',characterVersionIds.slice(index,index+chunkSize)).order('character_version_id').order('day_of_week').order('start_minute');
...
function resolveAuthoredSnapshotPresence(instance:Record<string,unknown>,now:Date,schedules:Array<Record<string,any>>,locations:Array<Record<string,any>>,worlds:Array<Record<string,any>>,presences:Array<Record<string,any>>,events:Array<Record<string,any>>=[]){
  ...
  if(authored[0]?.metadata?.source==='calders_run_authoring_v1'){
    const event=events.filter(row=>row.character_instance_id===instance.id&&row.metadata?.source==='calders_run_authoring_v1'&&!row.metadata?.suppressedByPlanId&&new Date(row.starts_at)<=now&&new Date(row.ends_at)>now).sort((a,b)=>Number(b.metadata?.worldProgressVersion??0)-Number(a.metadata?.worldProgressVersion??0))[0];
    return event?{locationId:event.visibility==='hidden'?null:event.location_id,activity:event.visibility==='hidden'?'Taking some private time':event.title,energy:'medium',interruptibility:event.interruptibility}:null;
  }
