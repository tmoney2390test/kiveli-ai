...
export async function buildSnapshot(db: SupabaseClient, userId: string): Promise<Record<string, unknown>> {
  const [profile, worlds, locations, instances, relationships, milestones, dates, moments, memories, threads, conversations, events, proactive, entitlements, preferences] = await Promise.all([
  ...
  const failed = [profile, worlds, locations, instances, relationships, milestones, dates, moments, memories, threads, conversations, events, proactive, entitlements, preferences].find((result) => result.error);
