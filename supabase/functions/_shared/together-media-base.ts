...
export function configuredImageProvider():ImageGenerationProvider|null {
  ...
  if(selected==='venice'){
    const openAIKey=Deno.env.get('OPENAI_API_KEY');if(openAIKey)return new OpenAIImageProvider(openAIKey,Deno.env.get('KIVELLE_CREATOR_IMAGE_MODEL')??Deno.env.get('KIVELLE_IMAGE_MODEL')??'gpt-image-2');
    const geminiKey=Deno.env.get('GEMINI_API_KEY');if(geminiKey)return new GeminiImageProvider(geminiKey,Deno.env.get('KIVELLE_CREATOR_IMAGE_MODEL')??'gemini-3.1-flash-image');
    return null;
  }
  if(selected==='openai'){
