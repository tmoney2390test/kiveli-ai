...
export async function queueMediaRequest(db:SupabaseClient,input:QueueMediaInput):Promise<Record<string,unknown>|null>{
  ...
  const metadata={source:input.source,photoOpportunitySlug:opportunity?.slug??null,shotType,locationId:locationId??null,sceneSessionId:input.sceneSessionId??null,sceneActionId:input.sceneActionId??null,resolvedContentLevel:contentLevel,qualityTier,aspectRatio,requestKey:key,requestIntent:{subject:intent.subject,confidence:intent.confidence},requestHint:safeRequestText(input.requestText),sceneSummary:`${String(template.name)} ${shotType==='scene'?'shared a view from':'sent a photo while at'} ${String(place?.path??location?.name??'their current place')} during ${String(instance.current_activity)}.`,activity:String(instance.current_activity),mood:String(instance.current_mood),timeOfDay:place?.clock.daypart??timeOfDay(now),outfitKey,relationshipStage:String(instance.relationship_stage),relationshipDirection:String(relationship?.recent_direction??'steady'),placeContext:place?placeContextSnapshot(place):null};
