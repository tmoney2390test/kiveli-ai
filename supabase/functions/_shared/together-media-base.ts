...
export function buildImagePrompt(request:CanonicalImageGenerationRequest):string {
  ...
  const referenceRule=characterReference?'Image 1 defines the same fictional adult companion and is authoritative for identity.':'Use the canonical identity description exactly and keep it stable across images.';
  const referenceInstructions=request.referenceImages.map((reference,index)=>`Image ${index+1} ${reference.role==='character_identity'?'defines the fictional adult companion identity':reference.role==='location_environment'?'defines the canonical location environment and its architecture, materials, layout cues, recurring objects, and atmosphere':reference.role==='world_environment'?'defines the wider canonical world identity':reference.role==='outfit_continuity'?'defines same-day clothing continuity':reference.role==='previous_media'?'defines continuity from the approved previous media':'is a curated character-training identity reference'}.`).join(' ');
  return [
    ...
    'REFERENCE ROLES',`${referenceRule} ${referenceInstructions} Use reference environments as identity anchors while allowing a natural new camera angle rather than copying the source composition.`,
...
export async function queueMediaRequest(db:SupabaseClient,input:QueueMediaInput):Promise<Record<string,unknown>|null>{
  ...
  const hasLocationReference=referenceAssets.some((asset)=>asset.role==='location_canonical'||asset.role==='location_alternate'),hasWorldReference=referenceAssets.some((asset)=>asset.role==='world_canonical');
  const metadata={source:input.source,photoOpportunitySlug:opportunity?.slug??null,shotType,locationId:locationId??null,sceneSessionId:input.sceneSessionId??null,sceneActionId:input.sceneActionId??null,requestedContentLevel:requestedLevel,resolvedContentLevel:contentLevel,mediaPolicyReason:policy.reasonCode,qualityTier,aspectRatio,requestKey:key,requestIntent:{subject:intent.subject,confidence:intent.confidence},generationIntent:input.source==='user_request'&&input.requestText?{requestText:input.requestText.slice(0,400),requestedContentLevel:requestedLevel}:null,requestHint:safeRequestText(input.requestText),referenceAssets,locationReferenceResolution:hasLocationReference?'location':hasWorldReference?'world':'text',location_reference_fallback:!hasLocationReference&&hasWorldReference?'world':null,sceneSummary:`${String(template.name)} ${shotType==='scene'?'shared a view from':'sent a photo while at'} ${String(place?.path??location?.name??'their current place')} during ${String(instance.current_activity)}.`,activity:String(instance.current_activity),mood:String(instance.current_mood),timeOfDay:place?.clock.daypart??timeOfDay(now),outfitKey,relationshipStage:String(instance.relationship_stage),relationshipDirection:String(relationship?.recent_direction??'steady'),placeContext:place?placeContextSnapshot(place):null};
