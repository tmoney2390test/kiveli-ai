...
import { resolveMediaSceneBoundary } from '../../../packages/together-domain/src/media.ts';
...
export function buildImagePrompt(request:CanonicalImageGenerationRequest):string {
  ...
  return [
    ...
    'REFERENCE ROLES',`${referenceRule} ${referenceInstructions} Location references define the exact environment. World references define only the wider regional identity and must never replace the exact location. Allow a natural new camera angle rather than copying the source composition.`,
...
export async function queueMediaRequest(db:SupabaseClient,input:QueueMediaInput):Promise<Record<string,unknown>|null>{
  ...
  const metadata={source:input.source,photoOpportunitySlug:opportunity?.slug??null,shotType,locationId:locationId??null,sceneSessionId:input.sceneSessionId??null,sceneActionId:input.sceneActionId??null,requestedContentLevel:requestedLevel,resolvedContentLevel:contentLevel,mediaPolicyReason:policy.reasonCode,qualityTier,aspectRatio,requestKey:key,requestIntent:{subject:intent.subject,confidence:intent.confidence},generationIntent:input.source==='user_request'&&input.requestText?{requestText:input.requestText.slice(0,400),requestedContentLevel:requestedLevel}:null,requestHint:safeRequestText(input.requestText),referenceAssets,sceneBoundary:sceneBoundary.setting,locationReferenceResolution:hasLocationReference?'location':hasWorldReference?'world':'text',location_reference_fallback:!hasLocationReference&&hasWorldReference?'world':null,sceneSummary:`${String(template.name)} ${shotType==='scene'?'shared a view from':'sent a photo while at'} ${String(place?.path??location?.name??'their current place')} during ${String(instance.current_activity)}.`,activity:String(instance.current_activity),mood:String(instance.current_mood),timeOfDay:place?.clock.daypart??timeOfDay(now),outfitKey,relationshipStage:String(instance.relationship_stage),relationshipDirection:String(relationship?.recent_direction??'steady'),placeContext:place?placeContextSnapshot(place):null};
