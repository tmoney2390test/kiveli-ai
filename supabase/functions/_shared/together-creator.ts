...
function title(value:string){return value.replace(/\b\w/g,(letter)=>letter.toUpperCase())}function sentence(value:string){const clean=value.trim().replace(/[.!?]+$/,'');return `${clean[0]?.toUpperCase()??''}${clean.slice(1)}.`}function article(value:string){return /^[aeiou]/i.test(value)?'an':'a'}function clamp(value:number,min:number,max:number){return Math.max(min,Math.min(max,value))}
