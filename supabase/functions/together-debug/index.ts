...
serve(async (request, correlationId) => {
  ...
  if(input.action==='inspect_media_economics'){
    ...
    const[usage,offers,ledger,accounts]=await Promise.all([
      ...
      db.from('together_credit_accounts').select('permanent_balance,subscription_balance,subscription_expires_at').eq('user_id',user.id).maybeSingle(),
    ]);
...
function summarizeCreditLiability(rows:Array<Record<string,any>>,account:Record<string,any>|null){const delta=(kind:string,field:string)=>rows.filter((row)=>row.event_type===kind).reduce((sum,row)=>sum+Number(row[field]??0),0),expired=Math.abs(rows.filter((row)=>row.event_type==='adjustment'&&row.metadata?.reason==='post_subscription_grace_expired').reduce((sum,row)=>sum+Math.min(0,Number(row.subscription_delta??0)),0));return{welcomeAndPurchasedGranted:delta('welcome_grant','permanent_delta')+delta('purchase','permanent_delta'),subscriptionCreditsGranted:delta('subscription_grant','subscription_delta'),permanentCreditsSpent:Math.abs(Math.min(0,delta('spend','permanent_delta'))),subscriptionCreditsSpent:Math.abs(Math.min(0,delta('spend','subscription_delta'))),creditsRefunded:delta('refund','permanent_delta')+delta('refund','subscription_delta'),creditsExpired:expired,permanentPurchasedOutstanding:Number(account?.permanent_balance??0),subscriptionCreditsOutstanding:Number(account?.subscription_balance??0),subscriptionCreditsExpireAt:account?.subscription_expires_at??null,note:'Unused credits are liabilities, not provider cost. Purchased and welcome credits remain permanent; subscription adjustments record downgrade clamps and grace expiry.'};}
