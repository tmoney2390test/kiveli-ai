...
serve(async (request, correlationId) => {
  ...
  const { data: conversation } = await db.from('together_conversations').insert({ user_id: user.id, character_instance_id: chloe.id, kind: 'direct', title: 'Meeting Chloe', last_message_at: now }).select('*').single();
  ...
  await db.from('together_relationship_states').update({ trust: 30, comfort: 10, affinity: 12, familiarity: 8, updated_at: now }).eq('character_instance_id', chloe.id);
