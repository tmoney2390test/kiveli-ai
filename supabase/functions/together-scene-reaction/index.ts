...
import { ConfiguredDialogueProvider, ConfiguredModerationProvider, dialogueProviderName } from '../_shared/together-ai.ts';
...
Deno.serve(async(request)=>{
  ...
  try{
    ...
    const context=await buildKivelleConversationContext({db,userId:user.id,instance,conversation,userMessage:'React naturally to the shared action that just happened.',lifeRun,now});
    (context as Record<string,unknown>).sceneAction={id:action.id,key:action.interaction_key,label,result:action.result,location:context.place?.path??context.currentScene.location};
    const content=await dialogue.generate(context);
    const safe=await moderation.check(content);const reply=safe.allowed?content:`${String(instance.together_character_templates?.name??'Your companion')} pauses for a second, then smiles.`;
    const {data:message,error}=await db.from('together_messages').insert({conversation_id:conversation.id,user_id:user.id,character_instance_id:input.characterInstanceId,role:'assistant',content:reply,delivery_status:'complete',provider_metadata:{provider:dialogueProviderName(),source:'scene_action',sceneActionId:input.sceneActionId,sceneSessionId:action.scene_session_id,clientRequestId:input.clientRequestId}}).select('*').single();
