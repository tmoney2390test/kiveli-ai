...
import{adultVideoEnabledForSurface,configuredVideoPromptEnhancer,directVideoOpeningFrameRequest,resolveAnimatedVideoContentLevel,resolveDirectVideoContentDecision,resolveSourcePhotoVideoDecision,type DirectVideoContentDecision}from'../_shared/together-video-content.ts';
import { requireAiDataConsent } from '../_shared/kivelle-ai-consent.ts';

function adultVideoOn(access?:AdultAccessContext|null):boolean{
  return adultVideoEnabledForSurface({clientSurface:access?.client_surface,authorizedWebAdult:access?.authorized_web_adult});
}

const videoSettingsSchema=z.object({model:z.string().trim().min(3).max(100),sound:z.boolean(),resolution:z.enum(VIDEO_RESOLUTIONS),duration:z.number().int().min(1).max(20),expectedCredits:z.number().int().positive().optional()}).strict();
...
serve(async(request,correlationId)=>{
  ...
  if(input.action==='video_direct_options'){
    ...
    const adultVideo=adultVideoOn(adultAccess);
    const routes=await publishedVideoRoutes(db,publicVideoRoutes(configuredVideoRouteCatalog().filter((route)=>route.enabled&&route.sourceModes.includes('generated_first_frame')&&identityReferenceCount>0),{includeAdultCapable:adultVideo}));
  ...
  if(input.action==='video_direct_generate'){
    ...
    const contentDecision=resolveDirectVideoContentDecision({requestText:input.requestText,authorizedWebAdult:adultAccess.authorized_web_adult,adultVideoFeatureEnabled:adultVideoOn(adultAccess)});
    ...
    const route=resolveVideoRoute(input.settings.model,user.id,user.email,{preferredContentClass:contentDecision.adult?'adult_capable':'sfw'}),settings=validateVideoSettings(route,newVideoSettings(route,{resolution:input.settings.resolution,duration:input.settings.duration,sound:input.settings.sound}));if(!route.sourceModes.includes('generated_first_frame'))throw new AppError('VALIDATION_ERROR','Choose a direct-video model.',422);
    if(route.contentClass==='adult_capable'&&!adultVideoOn(adultAccess))throw new AppError('FORBIDDEN','That video model is unavailable for this session.',403,false);
  ...
  if(input.action==='video_options'){
    ...
    const adultRoutesAvailable=adultVideoOn(adultAccess),routes=await publishedVideoRoutes(db,publicVideoRoutes(configuredVideoRouteCatalog().filter((route)=>route.enabled&&route.sourceModes.includes('existing_photo')&&(sourceDecision.adult?route.contentClass==='adult_capable':route.contentClass==='sfw'||adultRoutesAvailable&&route.contentClass==='adult_capable')&&route.referenceImageRequirements.canonicalCharacterMin<=identityReferenceCount),{includeAdultCapable:adultRoutesAvailable}));
  ...
  if(input.action==='video_event'){
    ...
    const sourceDecision=await validateVideoSource(db,user.id,String(continuity.id),media,adultAccess);
    if(input.event==='model_selected'){if(!input.videoRouteId)throw new AppError('VALIDATION_ERROR','A video route is required.',422);const selected=resolveVideoRoute(input.videoRouteId,user.id,user.email,{preferredContentClass:sourceDecision.adult?'adult_capable':'sfw'}),compatible=sourceDecision.adult?selected.contentClass==='adult_capable':selected.contentClass==='sfw'||selected.contentClass==='adult_capable'&&adultVideoOn(adultAccess);if(!compatible)throw new AppError('FORBIDDEN','That video model is not compatible with this photo or session.',403,false);}
  ...
  if(input.action==='animate'){
    ...
    await enforceGenerationGuardrails(db,user.id,'provider_cost_only');
    await enforceVideoSubmissionAbuseLimit(db,user.id);
    const promptDecision=resolveDirectVideoContentDecision({requestText:input.prompt,authorizedWebAdult:adultAccess.authorized_web_adult,adultVideoFeatureEnabled:adultVideoOn(adultAccess)});
    ...
    const route=resolveVideoRoute(input.settings.model,user.id,user.email,{preferredContentClass:outputAdult?'adult_capable':'sfw'}),settings=validateVideoSettings(route,newVideoSettings(route,{resolution:input.settings.resolution,duration:input.settings.duration,sound:input.settings.sound})),routeUsesAdultEndpoint=route.contentClass==='adult_capable',routeCompatible=outputAdult?routeUsesAdultEndpoint:route.contentClass==='sfw'||routeUsesAdultEndpoint&&adultVideoOn(adultAccess);if(!route.sourceModes.includes('existing_photo')||!routeCompatible)throw new AppError('VALIDATION_ERROR',outputAdult?'Choose an Adult-capable model for this photo and prompt.':'Choose a video model available for this session.',422);const motionPreset=input.motionPreset??'subtle',creditCost=await publishedVideoCreditCost(db,route,settings,input.settings.expectedCredits),baselineCostUsd=videoProviderBaselineCostUsd(route,settings),canonical=await canonicalRequestForMedia(db,media),sourceAspectRatio=sourceVideoAspectRatio(media.width,media.height);
...
async function validateVideoSource(db:any,userId:string,continuityId:string,media:Record<string,any>,access:AdultAccessContext){
  ...
  const sourceDecision=resolveSourcePhotoVideoDecision({contentLevel:media.content_level,contentRating:media.content_rating,visibilityScope:media.visibility_scope,authorizedWebAdult:access.authorized_web_adult,adultVideoFeatureEnabled:adultVideoOn(access)});
  ...
  const policy=resolveMediaContentPolicy({requestedLevel:level,source:'user_request',automatic:false,ageVerified:sourceDecision.adult?Boolean(access.adult_eligible):Boolean(profile?.age_verified_at),characterAge:Number(template.age),fictionalCharacter:isFictionalCompanion(template,version),realPersonRequest:false,nonConsensualRequest:false,minorRelatedRequest:false,characterAllowsRequestedLevel:characterAllows,romanceEnabled:preferences.romanceEnabled!==false,suggestiveMediaEnabled:sourceDecision.adult,matureMediaEnabled:sourceDecision.adult,explicitMediaEnabled:sourceDecision.adult,adultVideoEnabled:sourceDecision.adult&&adultVideoOn(access),mediaType:'video',adultMediaFeatureEnabled:sourceDecision.adult&&adultVideoOn(access),adultPipelineAuthorized:sourceDecision.adult&&access.authorized_web_adult});
...
async function enhanceVideoPromptDraft(db:any,user:{id:string;email?:string|null},access:AdultAccessContext,input:z.infer<typeof videoPromptEnhancementSchema>,correlationId:string){
  ...
  const continuity=await activeContinuity(db,user.id);let route=resolveVideoRoute(input.routeId,user.id,user.email,{preferredContentClass:resolveDirectVideoContentDecision({requestText:input.prompt,authorizedWebAdult:access.authorized_web_adult,adultVideoFeatureEnabled:adultVideoOn(access)}).adult?'adult_capable':'sfw'});const settings=validateVideoSettings(route,newVideoSettings(route,{resolution:input.settings.resolution,duration:input.settings.duration,sound:input.settings.sound}));
  if(route.contentClass==='adult_capable'&&!adultVideoOn(access))throw new AppError('FORBIDDEN','That video model is unavailable for this session.',403,false);
  const promptDecision=resolveDirectVideoContentDecision({requestText:input.prompt,authorizedWebAdult:access.authorized_web_adult,adultVideoFeatureEnabled:adultVideoOn(access)});
  ...
  if(input.sourceMode==='existing_photo'){
    ...
    const outputAdult=['suggestive','mature','explicit'].includes(contentLevel);route=resolveVideoRoute(input.routeId,user.id,user.email,{preferredContentClass:outputAdult?'adult_capable':'sfw'});const compatible=outputAdult?route.contentClass==='adult_capable':route.contentClass==='sfw'||route.contentClass==='adult_capable'&&adultVideoOn(access);
  ...
  const enhancedDecision=resolveDirectVideoContentDecision({requestText:result.prompt,authorizedWebAdult:access.authorized_web_adult,adultVideoFeatureEnabled:adultVideoOn(access)});
