...
serve(async(request,correlationId)=>{
  ...
  if(input.action==='video_options'){
    ...
    const adultRoutesAvailable=adultVideoOn(adultAccess),routes=publicVideoRoutes(configuredVideoRouteCatalog().filter((route)=>route.enabled&&route.sourceModes.includes('existing_photo')&&(sourceDecision.adult?route.contentClass==='adult_capable':route.contentClass==='sfw'||adultRoutesAvailable&&route.contentClass==='adult_capable')&&route.referenceImageRequirements.canonicalCharacterMin<=identityReferenceCount),{includeAdultCapable:adultRoutesAvailable});
  ...
  if(input.action==='animate'){
    ...
    const route=resolveVideoRoute(input.settings.model,user.id,user.email),settings=validateVideoSettings(route,{resolution:input.settings.resolution,duration:input.settings.duration,sound:input.settings.sound}),routeUsesAdultEndpoint=route.contentClass==='adult_capable',routeCompatible=outputAdult?routeUsesAdultEndpoint:route.contentClass==='sfw'||routeUsesAdultEndpoint&&adultVideoOn(adultAccess);if(!route.sourceModes.includes('existing_photo')||!routeCompatible)throw new AppError('VALIDATION_ERROR',outputAdult?'Choose an Adult-capable model for this photo and prompt.':'Choose a video model available for this session.',422);const motionPreset=input.motionPreset??'subtle',creditCost=videoCreditCost(route,settings),baselineCostUsd=videoProviderBaselineCostUsd(route,settings),canonical=await canonicalRequestForMedia(db,media),sourceAspectRatio=sourceVideoAspectRatio(media.width,media.height);
...
async function enhanceVideoPromptDraft(db:any,user:{id:string;email?:string|null},access:AdultAccessContext,input:z.infer<typeof videoPromptEnhancementSchema>,correlationId:string){
  ...
  if(input.sourceMode==='existing_photo'){
    ...
    const outputAdult=['suggestive','mature','explicit'].includes(contentLevel),compatible=outputAdult?route.contentClass==='adult_capable':route.contentClass==='sfw'||route.contentClass==='adult_capable'&&adultVideoOn(access);
