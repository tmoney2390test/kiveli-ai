...
import{adultVideoFeatureEnabled,directVideoOpeningFrameRequest,resolveDirectVideoContentDecision,type DirectVideoContentDecision}from'../_shared/together-video-content.ts';
...
const schema=z.discriminatedUnion('action',[
  ...
  z.object({action:z.literal('animate'),sourceMediaId:z.string().uuid(),settings:videoSettingsSchema,videoRouteId:z.string().trim().min(3).max(100).optional(),motionPreset:z.enum(MOTION_PRESETS),requestId:z.string().trim().min(8).max(120)}).strict(),
  z.object({action:z.literal('video_direct_generate'),characterInstanceId:z.string().uuid(),conversationId:z.string().uuid().optional(),settings:videoSettingsSchema,videoRouteId:z.string().trim().min(3).max(100).optional(),motionPreset:z.enum(MOTION_PRESETS),aspectRatio:z.enum(['9:16','16:9']).default('9:16'),locationSource:z.enum(['current','home','place']).default('current'),locationId:z.string().uuid().optional(),requestText:z.string().trim().min(2).max(400),requestId:z.string().trim().min(8).max(120)}).strict(),
...
serve(async(request,correlationId)=>{
  ...
  if(input.action==='video_options'){
    ...
    const source=await validateVideoSource(db,user.id,String(continuity.id),media,adultAccess);
    ...
    const routes=publicVideoRoutes(configuredVideoRouteCatalog().filter((route)=>route.enabled&&route.sourceModes.includes('existing_photo')&&route.referenceImageRequirements.canonicalCharacterMin<=identityReferenceCount),{includeAdultCapable:source.adult});
  ...
  if(input.action==='video_event'){
    ...
    await validateVideoSource(db,user.id,String(continuity.id),media,adultAccess);
    if(input.event==='model_selected'){if(!input.videoRouteId)throw new AppError('VALIDATION_ERROR','A video route is required.',422);const selected=resolveVideoRoute(input.videoRouteId,user.id,user.email);if(selected.contentClass==='adult_capable'&&!adultAccess.authorized_web_adult)throw new AppError('FORBIDDEN','That video model is unavailable for this session.',403,false);}
  ...
  if(input.action==='animate'){
    ...
    await enforceVideoSubmissionAbuseLimit(db,user.id);
    const preferredContentClass:VideoContentClass=source.adult?'adult_capable':'sfw';
    const route=resolveVideoRoute(input.settings.model,user.id,user.email,{preferredContentClass}),settings=validateVideoSettings(route,{resolution:input.settings.resolution,duration:input.settings.duration,sound:input.settings.sound});if(!route.sourceModes.includes('existing_photo'))throw new AppError('VALIDATION_ERROR','Choose a model that can animate this photo.',422);if(source.adult&&route.contentClass!=='adult_capable')throw new AppError('VALIDATION_ERROR','Choose an Adult-capable video model for that photo.',422,false);if(!source.adult&&route.contentClass!=='sfw')throw new AppError('VALIDATION_ERROR','Choose a Safe for work model to animate this photo.',422);const creditCost=videoCreditCost(route,settings),baselineCostUsd=videoProviderBaselineCostUsd(route,settings),canonical=await canonicalRequestForMedia(db,media),sourceAspectRatio=sourceVideoAspectRatio(media.width,media.height);
    ...
    const canonicalReferences=canonical.referenceImages.filter((item)=>item.signedUrl&&['character_identity','location_environment','world_environment','outfit_continuity'].includes(item.role)).map((item)=>({url:String(item.signedUrl),role:item.role as 'character_identity'|'location_environment'|'world_environment'|'outfit_continuity'}));
    const sourceMetadata=(media.metadata??{}) as Record<string,unknown>,sourceIntent=sourceMetadata.generationIntent&&typeof sourceMetadata.generationIntent==='object'?sourceMetadata.generationIntent as Record<string,unknown>:null;
    const animatePrompt=typeof sourceIntent?.requestText==='string'&&sourceIntent.requestText.trim()?sourceIntent.requestText.trim():source.adult?'Continue this authorized adult pose with natural sexual motion. Preserve visible anatomy. Do not add clothing or censorship.':undefined;
    const payload=buildVideoProviderPayload(route,{sourceImageUrl:sourceSigned.signedUrl,canonicalReferences,sourceAspectRatio,motionPreset:input.motionPreset,...settings,userPrompt:animatePrompt,contentLevel:String(media.content_level??'standard') as 'standard'|'romance'|'suggestive'|'mature'|'explicit',adultAuthorized:source.adult,anonymousAdultPartner:sourceMetadata.anonymousAdultPartner===true,context:{companionName:canonical.companion.name,locationName:canonical.context.place?.location.name??canonical.context.location?.name,activity:canonical.context.activity}});
    ...
    await track(db,user.id,'video_generation_confirmed',{sourceMediaId:media.id,routeId:route.id,provider:route.provider,requestedModel:route.model,resolvedModel:route.model,motionPreset:input.motionPreset,durationSeconds:settings.duration,resolution:settings.resolution,soundRequested:settings.sound,creditCost,estimatedProviderCostUsd:baselineCostUsd,quotedProviderCostUsd:quote.amountUsd});
    const{data:reserved,error:reserveError}=await db.rpc('kivelle_reserve_video_generation_v4',{p_user_id:user.id,p_continuity_id:String(continuity.id),p_source_media_id:media.id,p_request_key:requestKey,p_route_id:route.id,p_motion_preset:input.motionPreset,p_provider:route.provider,p_model:route.model,p_requested_model:route.model,p_resolved_model:route.model,p_credit_cost:creditCost,p_quote_usd:quote.amountUsd,p_baseline_usd:baselineCostUsd,p_duration_seconds:settings.duration,p_resolution:settings.resolution,p_audio_behavior:settings.sound?'generated_audio':'silent',p_sound_requested:settings.sound,p_provider_audio_mode:route.audioMode,p_aspect_ratio:sourceAspectRatio,p_testing_selection:true,p_route_concurrency_limit:route.concurrencyLimit,p_adult_authorized:source.adult,p_adult_web_session_id:source.adult?adultAccess.web_session_id:null});
    ...
    waitUntil(kickMediaDispatcher());
    await track(db,user.id,'video_generation_submitted',{mediaId:video.id,sourceMediaId:media.id,routeId:route.id,provider:route.provider,requestedModel:route.model,resolvedModel:route.model,motionPreset:input.motionPreset,durationSeconds:settings.duration,resolution:settings.resolution,soundRequested:settings.sound,creditCost,estimatedProviderCostUsd:baselineCostUsd,quotedProviderCostUsd:quote.amountUsd,quoteCacheHit:quote.cacheHit,idempotent:Boolean((reserved as Record<string,unknown>)?.idempotent)});
    const visibleVideo=(await signMediaRows(request,db,user.id,adultAccess,[video]))[0];
    return json({data:{media:visibleVideo,creditCost,creditBalance:Number((reserved as Record<string,unknown>)?.total??0),route:safeVideoRouteOption(route)},correlationId},202,correlationId);
  }
...
async function validateVideoSource(db:any,userId:string,continuityId:string,media:Record<string,any>,access?:AdultAccessContext){
  ...
  const adult=['suggestive','mature','explicit'].includes(String(media.content_level??''))||media.visibility_scope==='web_adult'||!['safe','suggestive'].includes(String(media.content_rating??''));
  if(adult){
    if(!access?.authorized_web_adult||!adultVideoFeatureEnabled())throw new AppError('FORBIDDEN','Adult video is unavailable for this session.',403,false);
  }else if(media.visibility_scope!=='all'||!['safe','suggestive'].includes(String(media.content_rating??''))){
    throw new AppError('CONFLICT','Only a ready, app-safe Kivelle companion photo can be animated.',409);
  }
  ...
  if(!adult&&!['standard','romance'].includes(level))throw new AppError('FORBIDDEN','Only standard and romantic images can be animated.',403);
  const boundaries=(version.content_boundaries??{}) as Record<string,unknown>;
  const characterAllows=level==='standard'?boundaries.allows_standard!==false:level==='romance'?boundaries.allows_romance!==false:level==='suggestive'?boundaries.allows_suggestive===true||boundaries.allows_mature===true:level==='mature'?boundaries.allows_mature===true:boundaries.allows_explicit===true;
  const policy=resolveMediaContentPolicy({requestedLevel:level as 'standard'|'romance'|'suggestive'|'mature'|'explicit',source:'user_request',automatic:false,ageVerified:adult?Boolean(access?.authorized_web_adult&&profile?.adult_eligible_at):Boolean(profile?.age_verified_at),characterAge:Number(template.age),fictionalCharacter:isFictionalCompanion(template,version),realPersonRequest:false,nonConsensualRequest:false,minorRelatedRequest:false,characterAllowsRequestedLevel:characterAllows,romanceEnabled:preferences.romanceEnabled!==false,suggestiveMediaEnabled:adult,matureMediaEnabled:adult,explicitMediaEnabled:adult,adultVideoEnabled:adult,mediaType:'video',adultMediaFeatureEnabled:adultVideoFeatureEnabled(),adultPipelineAuthorized:adult});
  if(!policy.allowed)throw new AppError('FORBIDDEN','This photo is not eligible for video generation.',403);
  return{profile,instance,policy,adult};
}
