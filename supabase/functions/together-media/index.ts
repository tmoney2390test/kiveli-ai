...
const schema=z.discriminatedUnion('action',[
  ...
  z.object({action:z.literal('animate'),sourceMediaId:z.string().uuid(),settings:videoSettingsSchema,videoRouteId:z.string().trim().min(3).max(100).optional(),motionPreset:z.enum(MOTION_PRESETS),prompt:z.string().trim().min(2).max(400).default('Continue this photo with natural motion.'),requestId:z.string().trim().min(8).max(120)}).strict(),
...
serve(async(request,correlationId)=>{
  ...
  if(input.action==='animate'){
    ...
    const route=resolveVideoRoute(input.settings.model,user.id,user.email),settings=validateVideoSettings(route,{resolution:input.settings.resolution,duration:input.settings.duration,sound:input.settings.sound}),routeUsesAdultEndpoint=route.contentClass==='adult_capable',routeCompatible=outputAdult?routeUsesAdultEndpoint:route.contentClass==='sfw'||routeUsesAdultEndpoint&&adultAccess.authorized_web_adult&&adultVideoFeatureEnabled();if(!route.sourceModes.includes('existing_photo')||!routeCompatible)throw new AppError('VALIDATION_ERROR',outputAdult?'Choose an Adult-capable model for this photo and prompt.':'Choose a video model available for this session.',422);const creditCost=videoCreditCost(route,settings),baselineCostUsd=videoProviderBaselineCostUsd(route,settings),canonical=await canonicalRequestForMedia(db,media),sourceAspectRatio=sourceVideoAspectRatio(media.width,media.height);
    ...
    const canonicalReferences=canonical.referenceImages.filter((item)=>item.signedUrl&&['character_identity','location_environment','world_environment','outfit_continuity'].includes(item.role)).map((item)=>({url:String(item.signedUrl),role:item.role as 'character_identity'|'location_environment'|'world_environment'|'outfit_continuity'}));
    const payload=buildVideoProviderPayload(route,{sourceImageUrl:sourceSigned.signedUrl,canonicalReferences,sourceAspectRatio,motionPreset:input.motionPreset,...settings,userPrompt:input.prompt,contentLevel:outputContentLevel,adultAuthorized:outputAdult,anonymousAdultPartner,context:{companionName:canonical.companion.name,locationName:canonical.context.place?.location.name??canonical.context.location?.name,activity:canonical.context.activity}});
    ...
    await track(db,user.id,'video_generation_confirmed',{sourceMediaId:media.id,routeId:route.id,provider:route.provider,requestedModel:route.model,resolvedModel:route.model,motionPreset:input.motionPreset,durationSeconds:settings.duration,resolution:settings.resolution,soundRequested:settings.sound,contentLevel:outputContentLevel,creditCost,estimatedProviderCostUsd:baselineCostUsd,quotedProviderCostUsd:quote.amountUsd});
    const{data:reserved,error:reserveError}=await db.rpc('kivelle_reserve_video_generation_v7',{p_user_id:user.id,p_continuity_id:String(continuity.id),p_source_media_id:media.id,p_request_key:requestKey,p_route_id:route.id,p_motion_preset:input.motionPreset,p_provider:route.provider,p_model:route.model,p_requested_model:route.model,p_resolved_model:route.model,p_credit_cost:creditCost,p_quote_usd:quote.amountUsd,p_baseline_usd:baselineCostUsd,p_duration_seconds:settings.duration,p_resolution:settings.resolution,p_audio_behavior:settings.sound?'generated_audio':'silent',p_sound_requested:settings.sound,p_provider_audio_mode:route.audioMode,p_aspect_ratio:sourceAspectRatio,p_user_prompt:input.prompt,p_content_level:outputContentLevel,p_anonymous_adult_partner:anonymousAdultPartner,p_testing_selection:true,p_route_concurrency_limit:route.concurrencyLimit,p_adult_authorized:routeUsesAdultEndpoint,p_adult_web_session_id:routeUsesAdultEndpoint?adultAccess.web_session_id:null});
