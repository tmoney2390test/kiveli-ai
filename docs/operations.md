...
The portal remains usable when email delivery is unavailable. Initial support-inbox notifications require `RESEND_API_KEY`, `KIVELLE_SUPPORT_EMAIL_FROM` (or `KIVELLE_OPS_ALERT_FROM`), and an inbox (`KIVELLE_SUPPORT_EMAIL`, default `support@kivelli.app`). The Ops ticket detail displays notification status. Portal replies do not send email and email replies are not ingested into the portal; do not promise otherwise.
