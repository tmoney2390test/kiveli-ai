...
Customers open `/support` to submit requests, view recent tickets, read support replies, and add follow-ups. Ops staff with the support role open tickets in `/ops` and use **Send reply to portal** for customer-visible responses. **Save note** remains private. Public replies set first-response time and move a ticket to waiting; customer follow-ups reopen it. Closing/resolving a ticket is visible to the customer.

The portal remains usable when email delivery is unavailable. Resend Free handles transactional notifications using `RESEND_API_KEY`, `KIVELLE_SUPPORT_EMAIL_FROM`, and `KIVELLE_SUPPORT_EMAIL` (default `support@kivelli.app`). Zoho continues receiving mail. New tickets queue a support-inbox alert and customer confirmation; public support replies notify the customer, and customer follow-ups notify the inbox. Internal notes never notify customers. Emails link to the private portal without including ticket or chat content. Email replies are **not** ingested into the portal; customers should reply in their support thread.

Production is at the Edge environment-secret limit, so the dispatcher falls back to the encrypted Vault secret `kivelli_resend_api_key` through `kivelle_email_sender_key()`. This RPC is executable only by `service_role` and returns only that one named secret. The key is restricted in Resend to sending from `kivelli.app`. The default sender is `Kivelli Support <notifications@kivelli.app>`. Do not enable paid overages or remove other production secrets to configure mail.

Verified production RevenueCat activations queue one welcome per account/plan, including monthly/annual plan details, trial status, benefits/credit-balance links, and the original store's subscription-management link. Renewals and restores of an already active plan do not resend welcomes. Sandbox subscriptions and deleted/unconfirmed accounts do not receive these customer notifications. No historical email backfill is performed.

`together_email_outbox` records pending/sending/sent/failed/skipped state, attempts, provider ID and safe error code. `sent` means accepted by Resend, not confirmed inbox delivery; check Resend Emails for delivery/bounce events. The service-only dispatcher runs every five minutes, claims up to ten messages, and reserves attempts atomically against a conservative 90/day and 2,700/month cap. Resend Free's account-wide 100/day and 3,000/month quotas are shared with other projects; pay-as-you-go must remain off. Quota exhaustion leaves unattempted messages queued. Transient failures retry with the same Resend idempotency key for at most 23 hours; ambiguous older attempts fail for operator review rather than risking duplicate sends after Resend's 24-hour deduplication expires. Permanent recipient/provider errors fail for review. The initial inbox notification's status also appears on the Ops ticket.

Support replies are stored separately from internal ticket events, with service-only access and owner-scoped customer endpoints. Request IDs deduplicate ticket/reply retries. Test the database contract with `node scripts/test-support-portal-db.mjs` and the endpoint helper privacy behavior with `deno test --allow-env --no-lock --config supabase/functions/deno.json supabase/functions/_shared/support-portal.test.ts`.
